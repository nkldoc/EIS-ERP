...
if ($_REQUEST["type"] == "bg_budget_hdr_adjust") {
	...
	$sqlMain = "
        ...
        SELECT
            ...
            ,a.bg_budget_hdr_adjust_id
        INTO #TemData
        FROM dbo.bg_budget_hdr_adjust a
        WHERE a.i_enable = 1
            {$con};

        SELECT
            a.numrow
			,b.bg_budget_hdr_adjust_id
			,b.i_year
        ...
        FROM #TemData a
			INNER JOIN dbo.bg_budget_hdr_adjust b ON a.bg_budget_hdr_adjust_id = b.bg_budget_hdr_adjust_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"								=> $row["numrow"],
				"id"								=> $row["bg_budget_hdr_adjust_id"],
				"i_year"							=> $row["i_year"],
	...
	exit;
} else if ($_REQUEST["type"] == "bg_budget_dtl_adjust") {

	$sqlMain = "
		SET NOCOUNT ON
		SELECT
			ROW_NUMBER() OVER (ORDER BY b.c_code) AS numrow
			,a.bg_budget_dtl_adjust_id
			,a.bg_expense_id
			,a.f_increase
			,a.f_decrease
			,a.c_comment
		INTO #TemData
		FROM dbo.bg_budget_dtl_adjust a
			LEFT JOIN dbo.bg_expense b ON a.bg_expense_id = b.bg_expense_id AND b.i_enable = 1 AND b.i_delete = 2
		WHERE a.bg_budget_hdr_adjust_id = ?;
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"										=> $row["numrow"],
				"id"										=> $row["bg_budget_dtl_adjust_id"],
