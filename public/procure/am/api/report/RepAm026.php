...
function check_line($asset_name, $c_brand, $c_serial, $c_model, $asset_comment, $fix_len)
{
	$arr_row = array();
	...
	$arr_row[] = ceil((mb_strlen($c_model, 'utf-8')/$fix_len));
        $arr_row[] = ceil((mb_strlen($asset_comment, 'utf-8')/$fix_len));
	...
	for ($i = 1; $i <= $row; $i++)
	{
		...
		$data["data"][$i]["c_model"] = iconv_substr($c_model, (($i-1)*($fix_len)), $fix_len, 'utf-8');
                $data["data"][$i]["asset_comment"] = iconv_substr($asset_comment, (($i-1)*($fix_len)), $fix_len, 'utf-8');
	}
	return $data;
...
$data_hdr["4"]	= "<tr bgcolor=#A5BAD6>"
				...
				."		<th width=8% style=\"{$css_gl_top_bottom_left}\" nowrap>รวม <br />{$depreAt}</th>"
                                ...
                                ."		<th width=8% style=\"{$css_gl_top_bottom_left}\" nowrap>หมายเหตุ</th>"
                                ."		<th width=8% style=\"{$css_gl_top_bottom_all}\" nowrap>อายุการใช้งาน</th>"
				."	</tr>";
...
$sql = "SET NOCOUNT ON;  
        ...
        select 
                ...
                , cast(0 as decimal(18,2)) as i_period_year
                , cast('' as varchar(2000)) as asset_comment
        ...
        set a.asset_name = isnull(c.c_name, '-')
            ...
            , a.i_period_year = c.i_period_year
            , a.asset_comment = isnull(c.c_comment, '')
        ...
        select case when convert(datetime, b.ta_date, 102) <= convert(datetime, @last_day, 102)
                ...
                , a.i_period_year as i_period_year
                , isnull(a.c_comment, '') as asset_comment
                , cast(1 as tinyint) as i_type
        ...
        select a.dc_cost_id
                ...
                , c.d_depreciate
                , b.i_period_year
                , isnull(b.c_comment, '') as asset_comment
                , 2
        from am_tf_hdr a
                inner join am_tf_dtl b on a.am_tf_hdr_id = b.am_tf_hdr_id
        ...
        if ($cost_id != $row["dc_cost_id"]) // เปลี่ยนหน่วยงานใหม่
        {
            if ($cost_id > 0)
            {
                $data.="<tr bgcolor='#C6D2D1'>"
                        ...
                        ."<th align=right style=\"{$css_am_text_comment}\">".number_format($sum_cost1["f_acc_price"],2)."</th>"
...
//=====
                $data.="<tr bgcolor='#C6D2D1'>"
                            ...
                            ."<th align=right style=\"{$css_am_text_comment}\">".number_format($sum_cost2["f_acc_price"],2)."</th>"
                            ."<th align=right style=\"{$css_am_text_comment}\">&nbsp;</th>"
                            ."<th align=right style=\"{$css_am_text_comment_close}\">&nbsp;</th>"
                        ."</tr>";
                $sum_cost2 = array("c_cost_asset"=> 0, "f_depre_bal"=>0, "f_depre"=>0, "f_sum_depre"=>0, "f_acc_price"=>0);
...
//=====
                $data.="<tr bgcolor='#C6D2D1'>"
                            ...
                            ."<th align=right style=\"{$css_am_text_comment}\">".number_format($sum_cost["f_acc_price"],2)."</th>"
                            ."<th align=right style=\"{$css_am_text_comment}\">&nbsp;</th>"
                            ."<th align=right style=\"{$css_am_text_comment_close}\">&nbsp;</th>"
                        ."</tr>";
                $sum_cost = array("c_cost_asset"=> 0, "f_depre_bal"=>0, "f_depre"=>0, "f_sum_depre"=>0, "f_acc_price"=>0);
        ...
        if ($row["i_type"] == 1) // ทั่วไป
        {
            if ($dataAssetGroup["asset_type"] == "0") // ที่ดิน
            {
                $arr_data = check_line($row["asset_name"], $row["p_area"], $row["p_deed"], $row["p_num_area"], $row["asset_comment"], $fix_len_half_min);
            }
            else // สินทรัพย์
            {
                $arr_data = check_line($row["asset_name"], $row["c_brand"], $row["c_serial"], $row["c_model"], $row["asset_comment"], $fix_len_half_min);
            }
            ...
            $asset_comment = ($arr_data["data"][1]["asset_comment"] != "")? $arr_data["data"][1]["asset_comment"] : "&nbsp;";
            ...
            $data.="<tr>"
                        ...
                        ."<td align=left style=\"{$css_am_text}\" nowrap valign=top title=หมายเหตุ>{$asset_comment}</td>"
                        ."<td align=right style=\"{$css_am_text_close}\" nowrap valign=top title=อายุการใช้งาน>".$row["i_period_year"]."</td>"
                    ."</tr>"
                    ;
            $cal_line++;
            $row_count++;
            ...
            if ($arr_data["row"] > 1)
            {
                for ($ii=2; $ii <= $arr_data["row"]; $ii++)
                {
                    ...
                    $asset_comment = ($arr_data["data"][$ii]["asset_comment"] != "")? $arr_data["data"][$ii]["asset_comment"] : "&nbsp;";
                    $data.="<tr>"
                                ...
                                ."<td align=left style=\"{$css_am_text} nowrap valign=top title=หมายเหตุ>".$asset_comment."</td>"
				."<td align=right style=\"{$css_am_text_close}\">&nbsp;</td>"
                            ."</tr>"
                                ;
                    $cal_line++;
                    $row_count++;
        ...
        else // กรณีโอนย้าย หรือ ตัดจำหน่าย
        {
                ...
                $data.="<tr>"
                            ...
                            ."<td align=right style=\"{$css_am_text}\" nowrap valign=top title=รวม>".number_format($row["f_sum_depre"],2)."</td>"
                            ."<td align=right style=\"{$css_am_text}\" nowrap valign=top title=ราคาตามบัญชี>".number_format($row["f_acc_price"],2)."</td>"
                            ."<td align=right style=\"{$css_am_text}\" nowrap valign=top title=หมายเหตุ>&nbsp;</td>"
                            ."<td align=right style=\"{$css_am_text_close}\" nowrap valign=top	title=อายุการใช้งาน>".$row["i_period_year"]."</td>"
                        . "</tr>";
                $cal_line++;
                $row_count++;
...
$data.="<tr bgcolor='#C6D2D1'>"
            ...
            ."<th align=right style=\"{$css_am_text_comment}\">".number_format($sum_all1["f_acc_price"],2)."</th>"
            ."<th align=right style=\"{$css_am_text_comment}\">&nbsp;</th>"
            ."<th align=right style=\"{$css_am_text_comment_close}\">&nbsp;</th>"
        ."</tr>";
$data.="<tr bgcolor='#C6D2D1'>"
            ...
            ."<th align=right style=\"{$css_am_text_comment}\">".number_format($sum_all2["f_acc_price"],2)."</th>"
            ."<th align=right style=\"{$css_am_text_comment}\">&nbsp;</th>"
            ."<th align=right style=\"{$css_am_text_comment_close}\">&nbsp;</th>"
        . "</tr>";
$data.="<tr bgcolor='#C6D2D1'>"
