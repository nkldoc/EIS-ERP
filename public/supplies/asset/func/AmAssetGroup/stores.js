Ext.store = new Ext.data.JsonStore({
  autoDestroy: false,
  autoLoad: true,
  url: "api/List_AmAssetGroup.php",
  baseParams: {
    type: "am_asset_group_hdr",
    // i_read: user_right_read,
  }, // Permission i_read
  root: "data",
  idProperty: "id",
  totalProperty: "totalCount",
  fields: [{ name: "no" }, { name: "id" }, { name: "c_name" }, { name: "c_comment" }, { name: "i_enable" }, { name: "d_create" }, { name: "am_asset_count" }, { name: "d_update" }],
});
...
Ext.storeDtl = new Ext.data.JsonStore({
  ...
  autoLoad: false,
  url: "api/List_AmAssetGroup.php",
  baseParams: { type: "am_asset_group_list" },
  root: "data",
  idProperty: "id",
  totalProperty: "totalCount",
  ...
  listeners: {
    load: function (t, records, options) {
      ...
      Ext.i_primary = false;
      for (var i = 0; i < num; i++) {
        if (Ext.storeDtl.data.items[i].data.i_primary == 1) {
          Ext.i_primary = true;
        }
      }
    },
...
Ext.imp_assetall_dtl = new Ext.data.JsonStore({
  autoLoad: true,
  storeId: "myStoreCost",
  url: "api/All_AmAssetGroup.php",
  baseParams: { type: "imp_assetall_dtl", id: Ext.HDR_ID },
  root: "data",
  idProperty: "id",
  totalProperty: "totalCount",
  fields: ["id", "no", "c_name", "c_code"],
});
