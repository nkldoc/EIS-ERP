...
$arrParam = array();
$arrCountParam = array();
$sqlTempTable = "select gl_dc_book_doc_id
                    , c_name
                    , c_comment
                    , i_enable
                    , gl_dc_book_type_id
                    , dc_doc_id
                    , (select c_code from dc_doc where dc_doc_id=a.dc_doc_id) as c_dc_doc_name
                    , (select top 1 c_code+' '+c_name from dc_doc where dc_doc_id=a.dc_doc_id) as doc_name
	...
	{
		$temp = array("no" => ($i++), 
                                "id"                        => $row["gl_dc_book_doc_id"],
                                "c_name"                    => $row["c_name"], 
                                "c_comment"                 => $row["c_comment"], 
                                "i_enable"                  => $row["i_enable"],
                                "gl_dc_book_type_id"        => $row["gl_dc_book_type_id"], 
                                "dc_doc_id"        			=> $row["dc_doc_id"],  
                                "txtdc_doc_idID"        	=> $row["doc_name"], 
                                "c_dc_doc_name"				=> $row["c_dc_doc_name"],
