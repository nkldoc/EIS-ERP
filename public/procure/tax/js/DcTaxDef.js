...
Ext.QuickTips.init();
	...
	var storeGroup	= new Ext.data.JsonStore({
            ...
            baseParams: {type : 'storeDcProductGroup', limit:20000},
            root: 'data',
            idProperty: 'id',
	    fields: [ 'id', 'c_name', 'c_code_name' ]
	});
	
	var storeTax	= new Ext.data.JsonStore({
            autoDestroy: true,
            autoLoad: true,
            url: 'api/All_TaxCombo.php',
            baseParams: {type : 'storeDcTax', limit:20000},
            root: 'data',
            idProperty: 'id',
	    fields: [ 'id', 'c_name' ]
	});
	
	var storeAcc = new Ext.data.JsonStore({
            autoLoad: true,
            url: 'api/All_TaxCombo.php',
            baseParams: {type : 'storeDcAcc'},
            root: 'data',
            idProperty: 'id',
	...
	var gridMain = {
		region: 'center',
		title: 'แสดงรายการกำหนดอัตราภาษีหัก ณ ที่จ่ายของหมวดรายได้',
		xtype: 'grid',
		id:'tabpanel1',
		border: false,
		stripeRows: true,
		...
		tbar: [{	
                ...
                store     : new Ext.data.SimpleStore({
							  fields: [ "value", "text" ],
							  data: [
								[ 'class_name', "หมวดรายได้" ],
								[ 'c_name', "ชื่อประเภทภาษี" ]
							  ]
							})
				,
				valueField: "value",
			...
			,' ', '-', {
				...
				handler : function() {  
					...
					{ header: "ID System", sortable: true, hidden:true, dataIndex: 'id' },
					{ header: "กลุ่มรายได้", sortable: true, dataIndex: 'group_name',width:200, },
					{ id: 'c_name', header: "ชื่อประเภทภาษี", sortable: true, dataIndex: 'c_name' },
					{ header: "อัตราภาษีหัก ณ ที่จ่าย (%)", 
                                            sortable: true, 
                                            dataIndex: 'tax_rate',
                                            renderer: function(value, metaData, record, rowIndex, colIndex, store) {
	...
	var panelForm = {
		region: 'center',
		title: 'ข้อมูลกำหนดอัตราภาษีหัก ณ ที่จ่ายของกลุ่มรายได้',
		xtype: 'panel',
		id:'tabpanel2',
		border: false,
		disabled: true,
        ...
        items: [
		{
			...
			items: [{
				...
				name: 'c_name',
                                validator: function(val) {
                                    if (!Ext.isEmpty(val)) {
                                            ...
                                            return "กรุณากรอก ชื่อประเภทภาษี";
                                    }
                                },
			}, {
				xtype: 'combo',
				...
				hiddenName : 'dc_tax_id',
                                mode: "local",
                                triggerAction: "all",
                                emptyText: "--- เลือกอัตราภาษีหัก ณ ที่จ่าย ---",
                                forceSelection: true,
                                selectOnFocus: true,
                                validator: function(val) {
                                    if (!Ext.isEmpty(val)){
                                            ...
                                            return "กรุณาเลือกอัตราภาษีหัก ณ ที่จ่าย";
                                    }
                                }
			}, 
                        Ext.PopAccForm.mini, 
