...
if ($_REQUEST["type"] == "am_asset_tf_hdr") {
	...
	$sqlMain = "
		SET NOCOUNT ON
        SELECT
            ROW_NUMBER() OVER (ORDER BY a.am_asset_tf_hdr_id) AS numrow
            ,a.am_asset_tf_hdr_id
        INTO #TemData
        FROM {$DATABASE_NAME} am_asset_tf_hdr a
        WHERE 1 = 1
            {$con};

        SELECT
            a.numrow
			,b.am_asset_tf_hdr_id
        ...
        FROM #TemData a
		INNER JOIN {$DATABASE_NAME} am_asset_tf_hdr b ON a.am_asset_tf_hdr_id = b.am_asset_tf_hdr_id
