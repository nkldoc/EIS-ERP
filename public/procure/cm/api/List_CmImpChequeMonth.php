...
if( $_REQUEST["type"] == "cm_imp_cheque_month_hdr" ) {
...
// 	}
	...
	$sqlMain = "SET NOCOUNT ON
				...
				INTO #TemData
				FROM cm_imp_cheque_month_hdr a
				WHERE a.i_enable = 1 
					{$con}
	...
	if( sqlsrv_has_rows( $stmt ) ) {
		while( $row = $db->Fetch( $stmt ) ) {
			$temp = array(	"no"						=> $row["numrow"],
							"id"						=> $row["cm_imp_cheque_month_hdr_id"],
...
} else if( $_REQUEST["type"] == "cm_imp_cheque_month_dtl" ) {
	...
	$sqlMain = "SET NOCOUNT ON
				SELECT
					...
					,a.f_cr
					,a.f_balance
				FROM cm_imp_cheque_month_dtl a
				WHERE a.cm_imp_cheque_month_hdr_id=?
				ORDER BY a.cm_imp_cheque_month_dtl_id;";

	$arrParam[]	= $_REQUEST["cm_imp_cheque_month_hdr_id"];
	...
	if( sqlsrv_has_rows( $stmt ) ) {
		...
		while( $row = $db->Fetch( $stmt ) ) {
			...
			$temp = array(	"no"						=> $totalCount,
							...
							"d_cheque"					=> ($row["d_cheque"] != "")? $row["d_cheque"] : "",
							"d_cheque_show"				=> ($row["d_cheque"] != "")? $date->extDateBuddha($row["d_cheque"]) : "",
