...
$caption = "รายงานสัดส่วนของค่าเสื่อมราคาต่อราคาสินทรัพย์ ";
...
if (is_array($data_dtl) && count($data_dtl["data"]) > 0) {
	...
	foreach ($data_dtl["data"] as $index => $jObj) {
		if ($jObj["i_type"] == '1') {
			...
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='left' nowrap>" . $jObj["c_acc_code"] . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='center' nowrap>" . $jObj["c_acc_name"] . "</td>";
			...
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap>" . changeNumFormat($jObj["f_per"]) . "</td>";
			...
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap><b>" . changeNumFormat($jObj["f_per"]) . "</b></td>";
		}
		$tbody .= "</tr>";
	}
	$tbody .= "</tbody>";
			...
			<table id='tb_main' width="100%" class="table_report" style=" width: 2000px; display:none" border="0" cellspacing="1" cellpadding="0">
				<thead valign="top">
					<tr>
						...
						<th style="vertical-align:middle;" rowspan=0 nowrap>&nbsp;รหัสทรัพย์สิน&nbsp;</th>
						...
						<th style='vertical-align:middle;' rowspan=0 nowrap>&nbsp;ค่าเสื่อมประจำปี&nbsp;</th>
						<th style="vertical-align:middle;" rowspan=0 nowrap>&nbsp;สัดส่วน(%)&nbsp;</th>
...
$legend_data = array("ค่าเสื่อมประจำปี", "มูลค่าที่ได้มา"); // 
...
foreach ($data_dtl["data"] as $index => $jObj) {
	if ($jObj["i_type"] == '1') {
		$temp = array(
			"name" => $jObj["c_acc_name"],
			"value1" => floatval($jObj["f_depre"]),
			"value2" => floatval($jObj["f_unit_cost"] - $jObj["f_depre"]),
		);
		$data[] = $temp;
	}
}
