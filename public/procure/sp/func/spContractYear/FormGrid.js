Ext.onReady(function() {
  ...
  const DisbledButton = function(t, record) {
    ...
    // Ext.getCmp("i_year").disable();
    // Ext.getCmp("d_doc_date").disable();
  };
  ...
  const controllTab = function(record, butt) {
    ...
    if (butt == "add") {
      ...
      if(Ext.session.i_level != 1 && butt == "edit" ){
        return false;
      } else {
        // ============ formAdd ============ //
  ...
  const gridMain = new Ext.grid.GridPanel({
    ...
    columns: [
      ...
      { header: "ผู้ทำรายการล่าสุด", sortable: true, width: 180,dataIndex: "dc_user_update" },
      {
      ...
      { header: "หน่วยงานที่ทำรายการล่าสุด", sortable: true, dataIndex: "dc_user_cost_update" ,width: 180,},
      { width: 40, dataIndex: "" }
    ],
