...
Ext.onReady(function() {
	...
	/* =============================================== */
	title_panel		= "บันทึกเลขที่เอกสารใบเบิก/ตั้งหนี้ แบบพิเศษ";	
	/* =============================================== */
	
	Ext.store = new Ext.data.JsonStore({
		autoLoad: false,
		url: "api/List_BookImpRequestVSNNone.php",
		baseParams: { type: "imp_request_vsn_hdr_dtl_none", i_read: user_right_read }, // Permission i_read
	    root: "data",
	    idProperty: "id",
	...
	gridMain = new Ext.grid.GridPanel({
		...
		tbar: [{ // กล่องค้นหาข้อมูล 1
	        ...
	        items: [{ // แถวที่ 1
	        	...
	        	xtype: "buttongroup",
	        	frame: false,
	        	items: [{ xtype: "label", text: "วันที่นำเข้าใบเบิก พิเศษ : " }, { xtype: "tbspacer", width: 4 }, {
	        ...
	        buttonAlign: "left",
			buttons:[{
				text : "&nbsp;&nbsp;บันทึกเลขเอกสารใบเบิก/ตั้งหนี้&nbsp;&nbsp;",
				iconCls: "icon-save",
				handler : function() {
					new Ext.Window({
						id: "win-msg-save",
							...
							handler: function() {
								...
								if( msg == "" ) {
									...
									function sendCallback(value,callback) {
										...
										Ext.Ajax.request({
											url : "api/mn_BookImpRequestVSNNone.php",
											timeout: 18000,
											method: "POST",
											params : {
												mode: "SAVE",
		...
		columns: [
			...
			{ header: "เลขที่นำเข้าใบเบิก พิเศษ", sortable: false, align: "center", width:100, dataIndex: "c_code" }, 
			...
			{ header: "วันที่นำเข้าใบเบิก พิเศษ", sortable: true, align: "center", dataIndex: "d_doc_date",
				renderer: function(value, metaData, record, rowIndex, colIndex, store) {
					return (value != "")? shortThaiDate(value) : "";
				}
			}, 
			{ header: "เอกสารอ้างอิง", sortable: false, align: "center", width:100, dataIndex: "c_period_no" }, 
			{ header: "เลขที่ใบเบิก พิเศษ", sortable: false, align: "center", width:100, dataIndex: "c_request_desc" }, 
			{ header: "เลขที่ตั้งหนี้ พิเศษ", sortable: false, align: "center", width:100, dataIndex: "c_request" },
			{ header: "วันที่ใบเบิก/ตั้งหนี้", sortable: true, align: "center", dataIndex: "d_doc_dtl",
				renderer: function(value, metaData, record, rowIndex, colIndex, store) {
					return (value != "")? shortThaiDate(value) : "";
				}
			},
			 { id: "c_comment_dtl", header: "รายการ", sortable: false, dataIndex: "c_comment_dtl" },
		...
		autoExpandColumn: "c_comment_dtl"
	}); // gridMain
