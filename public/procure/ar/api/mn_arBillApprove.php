...
switch ($mode) {
        ...
        if (count($json) > 0) {
            foreach ($json as $key => $val) {
                ...
                $ar = $db->GetDataBySQL("
                    SELECT
                        ...
                        ,a.d_bill_date
                        ,CASE 
                            WHEN a.d_bill_date BETWEEN '{$_REQUEST["d_start"]}' AND '{$_REQUEST["d_end"]}' THEN 1
                            ELSE 0
                        END AS i_between
                    ...
                    WHERE a.ar_bill_hdr_id = ?
                    GROUP BY a.ar_bill_hdr_id, a.c_code_bill, a.d_bill_date, d.ar_treat_right_group_id;", array($val));
                if ($ar["ar_treat_right_group_id"] == "") { // ยังไม่ระบุกลุ่มสิทธิ์ในข้อมูลหลัก
                    $msg .= "กรุณาระบุกลุ่มสิทธิ์การรักษา : " . $ar["c_code_bill"] . "<br>";
                }
                if ($ar["i_between"] == 0) { // วันที่เรียกเก็บไม่อยู่ในช่วงเดือน

                    $msg .= "วันที่เรียกเก็บไม่อยู่ในช่วงเดือน : " . $ar["c_code_bill"] . "<br>";
                }
            }
            ...
            if ($msg == "") {
                ...
                if ($vval != "") {
                    ...
                    $sql = "
                        BEGIN TRANSACTION;
                        UPDATE dbo.ar_bill_dtl
                            SET " . substr($addField, 1) . "
                        WHERE ar_bill_hdr_id IN (" . substr($vval, 1) . ");
                        COMMIT;";
    ...
    case "BACK_DATA":
        ...
        $sql = "
            BEGIN TRANSACTION;
            UPDATE dbo.ar_bill_dtl SET " . substr($addField, 1) . " WHERE ar_bill_hdr_id = {$_REQUEST["id"]};
            COMMIT;";
