...
###################
 ...
 { 
	...
	$sqlMain	= "select a.* ,c.c_name
	from ({$sqlTempTable}) a inner join dc_product c on c.dc_product_id=a.dc_product_id
	order by a.i_seq ";
	...
	$temp = array("no" => ($i++),  
			...
			."<input type='hidden' name='i_seq".$row["{$table}_id"]."' value='".$row["i_seq"]."'>" 
			."<input type='hidden' name='ar_bill_invoice_dtl_id[]' value='".$row["ar_bill_invoice_dtl_id"]."'>" 
			."<input type='text' onclick='this.focus();' style='font-size:12px;width:90%;margin:1px;' name='items".$row["{$table}_id"]."' value='".$row["c_name"]."'>"
			."<br/>"
			."<input type='text' onclick='this.focus();' style='font-size:12px;width:90%;margin:0px;' name='c_comment".$row["{$table}_id"]."' value='".$row["c_comment"]."'>",
			...
			"f_quan"			=> "<input type='text' onclick='this.focus();' style='font-size:12px;width:55px;text-align: right;margin:-2px;' name='f_quan".$row["{$table}_id"]."' value='".number_format($row["f_quan"],2)."'>"
	...
	${$root}[] = array("no" => ($i++), 
						"id" 		=> 'grandTotal7', 
						"c_name" 	=> "<input type='text' onclick='this.focus();' style='font-size:12px;width:90%;margin:-2px;' name='c_invoice_item1' value='".$due."'>",  
						"f_total_cost" 		=> "<p style='font-weight:bold;'>".$total_conv_text."</p>"
						."<input type='hidden' name='f_net_text' value='{$total_conv_text}'>");
	${$root}[] = array("no" => ($i++), 
						"id" 		=> 'grandTotal8', 
						"c_name" 	=> "<input type='text' onclick='this.focus();' style='font-size:12px;width:90%;margin:-2px;' name='c_invoice_item2' value=''>",  
