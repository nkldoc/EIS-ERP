...
Ext.onReady(function () {
  ...
  const controllTab = function (record, butt) {
    ...
    if (butt == "edit" || butt == "view") {
      ...
      if (Ext.dataSelect.i_status_last == 2 && record.data.d_status_date == "") {
        Ext.file_edit_enable = 1;
      } else {
        Ext.file_edit_enable = 0;
      }
      console.log(Ext.file_edit_enable);
    ...
    if (Ext.file_edit_enable == 0) {
      Ext.getCmp("form-widgets").getForm().applyToFields({ disabled: true, readOnly: true });
      $("#form-widgets :input").attr("readonly", "readonly");
    }
  }; // controllTab
  ...
  const gridMain = new Ext.grid.GridPanel({
    region: "center",
    layout: "fit",
    title: "แสดงรายการสถานะบันทึกใบขอเบิก (หน่วยงาน)",
    id: "tabpanel1",
    border: false,
    stripeRows: true,
    loadMask: true,
    ...
    columns: [
      ...
      //       },
      {
        ...
        renderer: function (value, metaData, record, row, col, store, gridView) {
          ...
          if (vv == 2 && record.data.d_status_date == "") {
            return "<button style='font-size:11px; cursor:pointer; color: green;'>แก้ไขเอกสาร / แสดง</button>";
          }
          return "<button style='font-size:11px; cursor:pointer; color: blue;'>แสดง</button>";
        },
      },
      ...
      // },
      {
        ...
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
          ...
          if (record.data.i_is_url_pdf_hdr == null) {
            ...
            return '<button style="display: flex" onclick="Po_OpenPdf(\'' + value + "', '" + record.data.c_code + '\')" type="button">' + BtnText + "</button>";
      ...
      // },
      {
        ...
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
          ...
          if (vv == 2 && record.data.d_status_date == "") {
            color = "black";
          } else if (vv == 2 && record.data.d_status_date != "") {
            color = "#FFA80F"; //yellow
          } else if (vv == 3) {
            color = "red";
          } else {
  ...
  Ext.myComboStores = [Ext.bg_expense, Ext.dc_expense_budget_type, Ext.po_parcel_officer, Ext.po_reason_protest, Ext.dc_cost];
