...
const impData = function() {
  ...
  dataDtl.forEach(v => {
    ...
    // หน่วยงาน
    try {
      let index_id = Ext.po_expense.findExact("c_name_excel", "" + v.หมวดค่าใช้จ่าย + "");
      po_expense_id = Ext.po_expense.data.items[index_id].id;
    } catch (err) {}
        ...
        columns: [
          ...
          },
          {
            ...
            renderer: function(value, metaData, record, rowIndex, colIndex, store) {
              if (value != "" && value != undefined) {
                metaData.attr = "style='text-align: left;'";
                return getStoreItems(Ext.po_expense, value, "c_name");
              } else {
                metaData.attr = "style='color:red; text-align: center;'";
                return "-";
              }
