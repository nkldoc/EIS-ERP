...
if ($_REQUEST["type"] == "po_budget_hdr_overlap") {
	...
	$sqlMain = "
        ...
        SELECT
            ...
            ,a.po_budget_hdr_overlap_id
        INTO #TemData
        FROM dbo.po_budget_hdr_overlap a
        WHERE a.i_enable = 1
            {$con};

        SELECT
            a.numrow
			,b.po_budget_hdr_overlap_id
			,b.i_year
        ...
        FROM #TemData a
			INNER JOIN dbo.po_budget_hdr_overlap b ON a.po_budget_hdr_overlap_id = b.po_budget_hdr_overlap_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"								=> $row["numrow"],
				"id"								=> $row["po_budget_hdr_overlap_id"],
				"i_year"							=> $row["i_year"],
	...
	exit;
} else if ($_REQUEST["type"] == "po_budget_dtl_overlap") {

	$sqlMain = "
		SET NOCOUNT ON
		SELECT
			ROW_NUMBER() OVER (ORDER BY a.c_code_ref) AS numrow
			,a.po_budget_dtl_overlap_id
			,a.c_code_ref
			,a.po_expense_id
			,a.f_total
			,a.f_cancel
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"										=> $row["numrow"],
				"id"										=> $row["po_budget_dtl_overlap_id"],
