...
Ext.onReady(function() {
	...
	storePV	= new Ext.data.JsonStore({
	    autoDestroy: true,
		autoLoad: true,
	    url: "api/List_GlFromPV.php",
	    baseParams: { type: "dataPV" },
	    root: "data",
	    idProperty: "id",
	...
	//================================ gridPV ================================//
	function PVCellClick(grid, rowIndex, columnIndex, e) {
		
		var record = grid.getStore().getAt(rowIndex);
		
		if ( columnIndex==grid.getColumnModel().getIndexById("pvProcess") ) {

			//ปุ่มบันทึกบัญชี
			var i_enable				= record.get("i_enable");
			var cm_voucher_one_id		= record.get("id");
			var i_type_voucher			= record.get("i_type_voucher");
			var ap_code					= record.get("c_code_pv");
			
			// SAVE DATA 
			function SaveData( typeData ) {

				Ext.getCmp("contenterCenter").getEl().mask("Please wait...", "x-mask-loading");
				Ext.Ajax.request({
					url: "api/mn_GlFromPV.php",
					timeout: 18000,
					method: "POST",
					params: { 
						type: typeData,
			...
			if ( i_enable != Ext.CONF_STATUS_DISABLE ) {
				...
				switch( i_type_voucher ) {
				
					case Ext.FI_PV_TYPE_BR: // เงินยืม (BR)
//						SaveData( "FI_PV_TYPE_BR" );
						alert("เงินยืม (BR)");
						break;
						
					case Ext.FI_PV_TYPE_APS_CLEAR_BRT: // เงินเบิกหักล้างเงินยืมทดรอง (APS)
//						SaveData( "FI_PV_TYPE_APS_CLEAR_BRT" );
						alert("เงินเบิกหักล้างเงินยืมทดรอง (APS)");
						break;
						
					case Ext.FI_PV_TYPE_APS_MANY: // เงินเบิกกรณีผู้รับหลายคน (APS)
					case Ext.FI_PV_TYPE_APS_WELFARE: // เงินเบิกสวัสดิการ (APS)
					case Ext.FI_PV_TYPE_APS_CLEAR_BRM: // เงินเบิกหักล้างเงินยืมหมุนเวียน (APS)
//						SaveData( "FI_PV_TYPE_APS" );
						alert("เงินเบิกกรณีผู้รับหลายคน (APS)");
						alert("เงินเบิกสวัสดิการ (APS)");
						alert("เงินเบิกหักล้างเงินยืมหมุนเวียน (APS)");
						break;
						
					case Ext.FI_PV_TYPE_AP: // เงินเบิกค่าใช้จ่าย (AP ค่าใช้จ่าย + AP จัดซื้อ)
						SaveData( "SEND_GX_AP_EXP" );
						break;
				}
			}
		}
	...
	gridPV = new Ext.grid.GridPanel({
		region: "center",
		layout: "fit",
		title: "แสดงรายการ PV ทุกสถานะ ยกเว้นยกเลิก,ส่งกลับ (ที่ยังไม่เป็น GX/GL)",
		id: "tabpanelPV",
		border: false,
		stripeRows: true,
		loadMask: true,
		...
		{ // กล่องค้นหาข้อมูล 1
            ...
            items: [{ // แถวที่ 1
            	...
            	items: [{ xtype: "label", text: "วันที่ใบสำคัญจ่าย (PV) ระหว่างวันที่ : " }, { xtype: "tbspacer", width: 4 }, {
            		id: "d_doc_date1", xtype: "datefield", width: 145, 
    				listeners : {
            		...
            		id: "d_doc_date2", xtype: "datefield", width: 145, 
    				listeners : {
						afterrender : function() {
            	...
            	xtype: "buttongroup",
            	frame: false,
            	items: [{ xtype: "label", text: "ประเภทใบสำคัญจ่าย : " }, { xtype: "tbspacer", width: 4 },
        	        new Ext.form.ComboBox({
						...
						store: new Ext.data.SimpleStore({
			            	...
			            	data: [
			            	       [ "0", "- เลือกทั้งหมด -" ],
			            	       [ "1", "เงินเบิกค่าใช้จ่าย/จัดซื้อจัดจ้าง (AP)" ],
			            	       [ "2", "ใบยืมเงินทดรอง/หมุนเวียน (BR)" ],
			            	       [ "7", "เงินเบิกกรณีผู้รับหลายคน (APS)" ],
			            	       [ "8", "เงินเบิกสวัสดิการ (APS)" ],
			            	       [ "9", "เงินเบิกกรณีผู้รับหลายคน หักล้างเงินยืมทดรอง (APS)" ],
			            	       [ "10", "เงินเบิกกรณีผู้รับหลายคน หักล้างเงินยืมหมุนเวียน (APS)" ]
			            	      ]
						}),
						value: "0",
						valueField: "id",
    			...
    			handler : function() {
    				...
    				if( msg == "" ) {
						...
						if(Ext.getCmp("value-box").getValue() != "") {
							storePV.setBaseParam("value", Ext.getCmp("value-box").getValue());
							storePV.setBaseParam("filter", Ext.getCmp("filter").getValue());
						} else {
							storePV.setBaseParam("value", "");
							storePV.setBaseParam("filter", "");
						}
						
						storePV.setBaseParam("mode", "SEARCH");
						storePV.setBaseParam("d_doc_date1", Ext.util.Format.date(Ext.getCmp("d_doc_date1").getValue(), "Y-m-d"));
						storePV.setBaseParam("d_doc_date2", Ext.util.Format.date(Ext.getCmp("d_doc_date2").getValue(), "Y-m-d"));
						storePV.setBaseParam("i_type_voucher", Ext.getCmp("i_type_voucher").getValue());
						storePV.load();
		...
		columns: [
			...
			{ id: "pvProcess", header: "-", sortable: false, align: "center", width: 80, dataIndex: "id",
		    	renderer: function(value, metaData, record, row, col, store, gridView) {
					var i_enable = record.get("i_enable");
					if (i_enable == Ext.CONF_STATUS_DISABLE)
						return "";
			...
			{ header: "เลขที่ใบสำคัญจ่าย (PV)", sortable: true, align: "center", width: 120, dataIndex: "c_code_pv" },
			{ header: "เลขที่เอกสารสมุดรายวัน", sortable: true, align: "center", width: 120, dataIndex: "c_code_gx" },
			{ header: "ประเภทใบสำคัญจ่าย", sortable: true, align: "center", width: 200, dataIndex: "name_voucher" },
			{ header: "เลขที่ใบขอเบิก", sortable: true, align: "center", dataIndex: "c_code_ap" },
			{ header: "วันที่ใบสำคัญจ่าย (PV)", sortable: true, align: "center", width: 120, renderer:shortThaiDate, dataIndex: "d_doc_date_pv" },
			{ header: "เลขที่จัดทำใบสำคัญจ่าย", sortable: true, align: "center", width: 120, dataIndex: "c_code" },
			{ header: "วันที่จัดทำใบสำคัญจ่าย", sortable: true, align: "center", width: 120, renderer:shortThaiDate, dataIndex: "d_doc_date" },
			{ header: "ประเภทการจ่ายเงิน", sortable: true, align: "center", dataIndex: "name_cash_type" },
			{ header: "จำนวนเงินรวม", sortable: true, dataIndex: "f_net_cost",
				renderer: function(value, metaData, record, row, col, store, gridView) {
					metaData.attr = "style='font-weight:bold;' align='right'";
					return floatRenderer(value);
				}
			},
			{ header: "ผู้สร้างรายการ", sortable: true, align: "center", dataIndex: "dc_user_create" },
			{ header: "วันที่สร้าง", sortable: true, align: "center", renderer:shortThaiDate, dataIndex: "d_create" },
			...
			{ header: "หน่วยงานที่แก้ไข", sortable: true, align: "center", dataIndex: "dc_user_update_cost" }
		],
...
//										Ext.MessageBox.alert("Success", jsonData.msg);		// alert massage success
	...
	var center = new Ext.TabPanel({
		...
		id: "contenterCenter",
		defaults:{ autoScroll: true }, 
		items: [ gridPV, gridMain, panelForm ]
	});
	// SET ref Grid&Tab
	Ext.getCmp("tabpanelPV").on("cellclick", PVCellClick, this);
	Ext.getCmp("tabpanel1").on("cellclick", cellClick, this);

	// SetTab Controller Loads
	Ext.getCmp("contenterCenter").setActiveTab("tabpanelPV");
	/*====================== RENDER ======================*/
	new Ext.Viewport({
		layout: "border",
		items: [ center ]
