...
switch ($mode) {
    ...
    case "imp_debtor_pay_dtl":
        ...
        if (count($json) > 0) {
            ...
            if ($strID != "") {
                $con = " AND imp_debtor_pay_dtl_id NOT IN (" . substr($strID, 1) . ")";
            }

            $sql = "
                BEGIN TRANSACTION;
                DELETE imp_debtor_pay_dtl WHERE imp_debtor_pay_hdr_id = {$_REQUEST["hdr_id"]} {$con};
                COMMIT;";
            ...
            foreach ($json as $key => $row) {
                ...
                if ($row["dtl_id"] > 0) { // EDIT
                    ...
                    $sql = "
                        BEGIN TRANSACTION;
                        UPDATE imp_debtor_pay_dtl SET " . substr($addField, 1) . " WHERE imp_debtor_pay_dtl_id = {$row["dtl_id"]};
                        COMMIT;";
                    ...
                    $data["imp_debtor_pay_hdr_id"]                  = $_REQUEST["hdr_id"];
                    $data["c_hn"]                                   = $row["c_hn"];
                    $data["c_an"]                                   = $row["c_an"];
                    ...
                    $data["c_no_pay"]                               = $row["c_no_pay"];
                    $data["d_save_pay"]                             = $row["d_save_pay"];
                    $data["f_pay"]                                  = $row["f_pay"];
                    $data["c_receipt"]                              = $row["c_receipt"];
                    $data["d_receipt"]                              = $row["d_receipt"];
                    ...
                    $sql = "
                        BEGIN TRANSACTION;
                        INSERT INTO imp_debtor_pay_dtl (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
                        COMMIT;";
    ...
    case "EDIT":
        ...
        if ($msg == "") {
            ...
            if ($mode == "ADD") {
                ...
                $sql    = "
                    SET NOCOUNT ON
                    INSERT INTO imp_debtor_pay_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
                ...
                $sql        = "
                    BEGIN TRANSACTION;
                    UPDATE imp_debtor_pay_hdr SET " . substr($addField, 1) . " WHERE imp_debtor_pay_hdr_id = ?;
                    COMMIT;";
                $para        = $db->QueryParam($sql, $arrValue);
                $id            = $_REQUEST["id"];
