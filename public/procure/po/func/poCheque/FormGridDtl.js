...
const delteDtl = function(id) {
  ...
  Ext.Ajax.request({
    url: "api/mn_poWorking.php",
    method: "POST",
    params: { mode: "DELETE_CHEQUE", id: id },
    success: function(result, request) {
      Ext.getCmp("contenterCenter")
        .getEl()
        .unmask();
  ...
  const girdDtl = new Ext.grid.GridPanel({
    ...
    viewConfig: {
      ...
      getRowClass: function(record) {
        console.log(record.data.i_chk);
        if (record.data.i_type != 1) {
          if (record.data.i_chk == 1) {
            return "td-success";
          } else {
            return "td-error";
          }
    ...
    listeners: {
      render: function(panel) {
        new Ext.dd.DropTarget(formPanelDD.body.dom, {
          ddGroup: "gridDDGroup",
          notifyDrop: function(ddSource, e, data) {
            var records = ddSource.dragData.selections;
            var selectedRecord = ddSource.dragData.selections[0];
            if (records[0].data.i_type == 1) {
              formPanelDD.getForm().loadRecord(selectedRecord);
              return true;
            }
          }
        });
        ...
        renderer: function(value, metaData, record, row, col, store, gridView) {
          if (record.get("i_type") == 1) {
            if (value == 1) {
              ...
              return "<font color=bue>รอดำเนินการ</font>";
            }
          } else {
            metaData.attr = 'style="font-weight: bold;"';
          }
