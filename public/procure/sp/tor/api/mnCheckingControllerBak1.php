...
switch ($mode) {
    ...
    case "LIST_SUB_PERIOD_HDR":
        ...
        $sqlTempTable = "select cc.sp_check_period_hdr_id,s.sp_po_id
                ...
                --inner join " . DB_NMU . "dc_creditor bbb on bbb.dc_creditor_id = cc.dc_creditor_id
...
";
    ...
    case "LIST_PERIOD_DTL":
        ...
        $sqlMain = "SELECT a.f_net_unit_price ,a.f_net_total_price
                , (select sum(isnull(f_wip_total_price,0) + isnull(f_under_total_price,0)+ isnull(f_net_total_price,0)) from sp_tranf_item bbbb 
    ...
    case "LIST_TRANF_ITEM":
        ...
        $sqlMain = "select  a.sp_tranf_item_id,
                            ...
                            a.f_net_total_price,
                            isnull(a.i_edit,0) as i_edit,
                            (ISNULL(a.f_net_total_price, 0) + ISNULL(a.f_under_total_price, 0) + ISNULL(a.f_wip_total_price, 0)) AS f_net_total,
                            a.dc_acc_id,
    ...
    case "INSERT_CHECKING_DTL":
        ...
        if ($hdr_id > 0) {
            ...
            if ($_REQUEST['i_workin_process'] == 1 && $_REQUEST['i_product_type'] <> 2) { // งานระหว่างดำเนินการ
                $arrParam = array(STATUS_ENABLE);
                $dc_acc = "SELECT  a.dc_acc_id  FROM  " . DB_CENTER . "dc_acc a WHERE  a.c_code = '10110010101' and a.i_enable = ?";
    ...
    case "GENCHECKINGCODE":
        ...
        $arrParam[] =  !empty($data["f_vat"]) ? str_replace(',', '', $data["f_vat"]) : 0;  // f_vat_amt
        $arrParam[] =  !empty($data["f_tax_personalID2"]) ? str_replace(',', '', $data["f_tax_personalID2"]) : 0;  //f_tax_personal
        $arrParam[] =  !empty($data["f_warranty"]) ? str_replace(',', '', $data["f_warranty"]) : 0;
    ...
    case "UPDATE_DCACC":
        ...
        foreach ($Arr as $fldd) {
            ...
            $arrValue[] = $fldd["sp_tranf_item_id"]; // ตรวจสอบว่ามีค่า sp_tranf_item_id 714
...
case "updateMaterialType":
    // อัพเดทประเภทวัสดุ/ครุภัณฑ์ (i_pr_type1) ในหลายตาราง
    unset($arrValue);
    $arrValue = array();
    
    // รับ array ของ sp_tor_hdr_period_id และ i_product_type
    $sp_tor_hdr_period_ids = json_decode($_REQUEST["sp_tor_hdr_period_ids"] ?? '[]', true); // decode JSON เป็น array
    $i_product_types = json_decode($_REQUEST["i_product_types"] ?? '[]', true); // decode JSON เป็น array
    
    // กำหนด variables
    $i_pr_type1 = $_REQUEST["i_product_type"] ?? null; // ค่าเดียวสำหรับ fallback
    ...
    if (!is_array($sp_tor_hdr_period_ids) || !is_array($i_product_types) || count($sp_tor_hdr_period_ids) !== count($i_product_types)) {
        $validationError = "Invalid arrays for sp_tor_hdr_period_ids and i_product_types";
    }
    
    if ($validationError !== null) {
        $stmt = false;
        $sql = "Validation Error: {$validationError}";
        break;
    }
    
    // อัพเดทแต่ละ sp_tor_hdr_period และ sp_check_period_hdr ตาม array
    // และอัพเดท sp_tor_contract และ sp_tor ที่เกี่ยวข้องด้วยค่าจาก i_pr_type1 เดิมใน sp_tor_hdr_period
    
    // ดึงค่าเดิมของทั้งหมด sp_tor_hdr_period_id ครั้งเดียว ก่อนการ update ใดๆ
    $placeholders = implode(',', array_fill(0, count($sp_tor_hdr_period_ids), '?'));
    $sql_get_all_old = "SELECT sp_tor_hdr_period_id, i_pr_type1 FROM dbo.sp_tor_hdr_period WHERE sp_tor_hdr_period_id IN ($placeholders)";
    $all_old_result = $db->QueryParam($sql_get_all_old, $sp_tor_hdr_period_ids);
    $old_values_map = array();
    while ($row = $db->Fetch($all_old_result)) {
        $old_values_map[$row['sp_tor_hdr_period_id']] = $row['i_pr_type1'];
    }
    ...
    $stmt1 = true;
    $stmt2 = true;
    $contract_value = null; // Initialize ตั้งแต่ต้น
    
    for ($i = 0; $i < count($sp_tor_hdr_period_ids); $i++) {
        $period_id = $sp_tor_hdr_period_ids[$i];
        $value = isset($i_product_types[$i]) ? $i_product_types[$i] : $i_pr_type1; // ใช้ค่าจาก array หรือ fallback
        
        // ใช้ค่าเดิมจาก map ที่ดึงมาก่อน
        $contract_value = $old_values_map[$period_id] ?? null;
        
        // อัพเดท sp_tor_hdr_period
        $sql3 = "UPDATE dbo.sp_tor_hdr_period SET i_pr_type1 = ? WHERE sp_tor_hdr_period_id = ?";
        $arrValue3 = array($value, $period_id);
        $stmt3 = $db->QueryParam($sql3, $arrValue3);
        
        // อัพเดท sp_check_period_hdr
        $sql4 = "UPDATE dbo.sp_check_period_hdr SET i_pr_type1 = ? WHERE sp_tor_hdr_period_id = ?";
        $arrValue4 = array($value, $period_id);
        $stmt4 = $db->QueryParam($sql4, $arrValue4);
        
        // หา sp_tor_contract_id จาก sp_tor_hdr_period_id
        $sql_get_contract = "SELECT sp_tor_contract_id FROM dbo.sp_tor_hdr_period WHERE sp_tor_hdr_period_id = ?";
        $contract_result = $db->QueryParam($sql_get_contract, array($period_id));
        $contract_row = $db->Fetch($contract_result);
        if ($contract_row) {
            $contract_id = $contract_row['sp_tor_contract_id'];
            
            // อัพเดท sp_tor_contract ด้วยค่าเดิมจาก sp_tor_hdr_period
            $sql1 = "UPDATE dbo.sp_tor_contract SET i_pr_type1 = ? WHERE sp_tor_contract_id = ?";
            $arrValue1 = array($contract_value, $contract_id);
            $stmt1 = $db->QueryParam($sql1, $arrValue1);
            
            // อัพเดท sp_tor ผ่าน JOIN ด้วยค่าเดิมจาก sp_tor_hdr_period
            $sql2 = "UPDATE a SET a.i_pr_type1 = ?
                     FROM dbo.sp_tor a
                     INNER JOIN dbo.sp_tor_contract b ON b.sp_tor_id = a.tor_id
                     WHERE b.sp_tor_contract_id = ?";
            $arrValue2 = array($contract_value, $contract_id);
    ...
    // ตรวจสอบว่าอัพเดททั้งหมดสำเร็จหรือไม่
    if (!$stmt1 || !$stmt2 || !$stmt3 || !$stmt4) {
        $stmt = false;
    ...
    $ret_id = $sp_tor_contract_id;
    break;

case "updateBookingType1":
    // อัพเดทประเภทการจองเงิน (i_pr_type2) พร้อมส่งค่ากลับเหมือนประเภทวัสดุ/ครุภัณฑ์
    unset($arrValue);
    $arrValue = array();
    $ret_booking_type = null; // เตรียมตัวสำหรับส่งกลับค่าที่อัพเดท
    
    // รับค่าพารามิเตอร์จากหลาย format เพื่อความยืดหยุ่น
    $sp_tor_hdr_period_ids = !empty($_REQUEST["sp_tor_hdr_period_ids"]) ? json_decode($_REQUEST["sp_tor_hdr_period_ids"], true) : array();
    $i_pr_types = !empty($_REQUEST["i_pr_types"]) ? json_decode($_REQUEST["i_pr_types"], true) : array();
    
    // ถ้าไม่ได้รับ arrays ให้สร้าง arrays จากค่าเดี่ยว
    if (empty($sp_tor_hdr_period_ids)) {
        $sp_tor_hdr_period_ids = array($_REQUEST["sp_tor_hdr_period_id"] ?? null);
    }
    if (empty($i_pr_types)) {
        $i_pr_types = array($_REQUEST["i_pr_type"] ?? null);
    }
    ...
    // Validate
    $validationError = null;
    if (empty($sp_tor_hdr_period_ids) || empty($i_pr_types)) {
        $validationError = "Missing sp_tor_hdr_period_ids or i_pr_types";
    } else if (count($sp_tor_hdr_period_ids) !== count($i_pr_types)) {
        $validationError = "Array count mismatch: sp_tor_hdr_period_ids and i_pr_types";
    }
    ...
    // ไม่มีคอลัมน์ i_pr_type2 ใน sp_tor_hdr_period เพราะเก็บไว้ที่ระดับสัญญา/tor
    // จะใช้ค่าที่ส่งเข้ามาทำการอัพเดทตรงไปยังตารางสัญญาและ TOR
    ...
    // Mapping ชื่อประเภทการจองเงิน
    $bookingTypeNames = array(
        '1' => 'เงินแผน',
        '2' => 'เงินงวด'
    );
    
    for ($i = 0; $i < count($sp_tor_hdr_period_ids); $i++) {
        $period_id = intval($sp_tor_hdr_period_ids[$i]);
        $value = intval(!empty($i_pr_types[$i]) ? $i_pr_types[$i] : 0);
        // จองเงินไม่มีการดึงค่าจาก period จึงใช้ค่าเดียวกันทั้ง contract/tor
        $contract_value = $value;
        $booking_type_name = isset($bookingTypeNames[(string)$value]) ? $bookingTypeNames[(string)$value] : '';
        
        // ติดต่อเฉพาะตารางสัญญาและ TOR เท่านั้น – ไม่มีคอลัมน์ i_pr_type2 ใน period/header
        // ปรับสถานะตัวบ่งชี้ความสำเร็จให้ยังคงเป็น true
        $stmt3 = true;
        $stmt4 = true;
        ...
        if ($contract_row && !empty($contract_row['sp_tor_contract_id'])) {
            $contract_id = intval($contract_row['sp_tor_contract_id']);
            
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "UPDATE dbo.sp_tor_contract SET i_pr_type2 = ? WHERE sp_tor_contract_id = ?";
            $arrValue1 = array($contract_value, $contract_id);
            $stmt1 = $db->QueryParam($sql1, $arrValue1);
            
            // อัพเดท sp_tor ผ่าน JOIN (booking type)
            $sql2 = "UPDATE a SET a.i_pr_type2 = ?
                     FROM dbo.sp_tor a
                     INNER JOIN dbo.sp_tor_contract b ON b.sp_tor_id = a.tor_id
                     WHERE b.sp_tor_contract_id = ?";
            $arrValue2 = array($contract_value, $contract_id);
...
if ((isset($stmt) && $stmt) && (isset($stmt2) && $stmt2)) {
    ...
    if (isset($ret_booking_type)) {
        $re["i_pr_type2"] = intval($ret_booking_type);
    }
} else {
    // build message safely
    $sqlMsgParts = array();
    if (isset($sql) && $sql !== null) $sqlMsgParts[] = $sql;
    if (isset($sql2) && $sql2 !== null) $sqlMsgParts[] = $sql2;
    if (isset($sql3) && $sql3 !== null) $sqlMsgParts[] = $sql3;
