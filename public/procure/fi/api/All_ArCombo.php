...
// 			$value = (isset($_REQUEST['query']) && !empty($_REQUEST['query']))?$_REQUEST['query']:$value;
...
}else if($_REQUEST['type'] == 'voucherTypeStore') { //select * from //dc_vat where end_date is null and i_enabled='1'
	
	$sqlMain	= "select fi_pymt_voucher_type_id 
						, c_code
						, c_name 
					from vw_fi_pymt_voucher_type
					where isnull(i_enable,".STATUS_DISABLE.") = ?
					order by fi_pymt_voucher_type_id
				";
	$arrParam	= array(STATUS_ENABLE); 
	$stmt = $db->QueryParam($sqlMain, $arrParam);
	$i=0;
	if($stmt){
		while($row =$db->Fetch($stmt))
		{   $i++;
			$temp = array("id" => $row["fi_pymt_voucher_type_id"]
						, "c_code" => $row["c_code"] 
						, "c_name" => $row["c_name"]
	...
	$totalCount = $i;
	$debug='voucherTypeStore >>>'; 
	
}else if($_REQUEST['type'] == 'dcBankAccStore') { //select * from //dc_vat where end_date is null and i_enabled='1'
	
	$sqlMain	= "select a.dc_bank_acc_company_id 
						, a.dc_acc_id
						, a.c_code
						, a.c_name
					from dc_bank_acc_company a 
					where isnull(a.i_enable,".STATUS_DISABLE.") = ? and a.i_delete =2 and a.dc_area_id !=0 
					order by a.dc_bank_acc_company_id
				";
	$arrParam	= array(STATUS_ENABLE); 
	$stmt = $db->QueryParam($sqlMain, $arrParam);
	$i=0;
	if($stmt){
		while($row =$db->Fetch($stmt))
		{   $i++;
			$temp = array("id" => $row["dc_bank_acc_company_id"]
	...
	$totalCount = $i;
	$debug='dcBankAccStore >>>'; 
}else if($_REQUEST['type'] == 'dcBankBranchStore') { //select * from //dc_vat where end_date is null and i_enabled='1'
	
	$sqlMain	= "select a.dc_bank_branch_id  
						, a.c_code
						, a.c_name
					from vw_dc_bank_branch a 
					where isnull(a.i_enable,".STATUS_DISABLE.") = ? and a.dc_bank_id=?
					order by a.dc_bank_branch_id
				";
	$arrParam	= array(STATUS_ENABLE,@$_REQUEST["dc_bank_id"]); 
	$stmt = $db->QueryParam($sqlMain, $arrParam);
	$i=0;
	if($stmt){
		while($row =$db->Fetch($stmt))
		{   $i++;
			$temp = array("id" => $row["dc_bank_branch_id"] 
						, "c_code" => $row["c_code"] 
						, "c_name" => $row["c_code"].' '.$row["c_name"]
	...
	$totalCount = $i;
	$debug='dcBankBranchStore >>>'; 
	
}else if($_REQUEST['type'] == 'dcBankStore') { //select * from //dc_vat where end_date is null and i_enabled='1'
	
	$sqlMain	= "select a.dc_bank_id  
						, a.c_code
						, a.c_name
					from vw_dc_bank a 
					where isnull(a.i_enable,".STATUS_DISABLE.") = ?
					order by a.dc_bank_id
				";
	$arrParam	= array(STATUS_ENABLE); 
	$stmt = $db->QueryParam($sqlMain, $arrParam);
	$i=0;
