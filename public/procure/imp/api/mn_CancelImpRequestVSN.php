...
$arrValue	= array();
	...
	switch($mode) 
	{
		...
		case "CANCEL_RQ_NORMAL" 	: // ยกเลิกใบเบิกปกติ (ก่อนเป็น GX + GL) - ephis + VSN
		default						:	
			$msg		= "";
			$sql		= ""; 
			$c_cheques	= "";
			...
			if ($cancel_dtl_id>0)
			{
					...
					{   
						...
						if ($doc_dtl["c_sub_rq"]!="")					
						{ //GEN เลขที่ตั้งหนี้แบบปกติแล้ว ปรับไม่ใช้งาน
						...
						if( @$stmtLog ) { 
							$re = array("success"					=> true,
										"msg"						=> "บันทึกยกเลิกใบเบิกปกติเสร็จสิ้น $c_txt"
							);
						} else {
