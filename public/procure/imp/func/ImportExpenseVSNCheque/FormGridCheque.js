...
// save dtl ADD && EDIT
const saveCheque = function(mode) {
  let msg = "";
  let jsonArr = [];
  let sto = Ext.getCmp("gridEditorCheque").store.data.items;
  ...
  if (msg == "") {
    ...
    Ext.Ajax.request({
      ...
      success: function(result, request) {
        ...
        Ext.storeCheque.load({ params: { dtl_id: Ext.DTL_ID } });
...
const changePrice = function() {
  ...
  Ext.storeCheque.each(function(record, id) {
...
// Class Extend
formPanelCheque = function(args) {
  cellClickDtl = function(grid, rowIndex, columnIndex, e) {
    if (columnIndex == grid.getColumnModel().getIndexById("DELETE")) {
      Ext.storeCheque.removeAt(rowIndex);
      changePrice();
    }
  }; //cellClick

  formPanelCheque.superclass.constructor.call(this, {
    title: "ระบุเช็ค",
    id: "PanelCheque",
        ...
        tbar: [
          ...
          "-",
          {
            ...
            handler: function(grid, rowIndex, colIndex) {
              ...
              Ext.storeCheque.insert(0, myNewRecord);
            }
          }
        ],
        columns: [
  ...
  Ext.getCmp("gridEditorCheque").on("cellclick", cellClickDtl, this);
}; // formPanelCheque
Ext.extend(formPanelCheque, Ext.Panel, {});
