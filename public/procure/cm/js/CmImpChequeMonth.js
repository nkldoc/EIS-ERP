...
Ext.onReady(function() {
	...
	/*===============================================*/
	title_panel		= "นำเข้าข้อมูลเช็ค (รายเดือน)";
	/*===============================================*/
	...
	store = new Ext.data.JsonStore({
	    ...
	    autoDestroy: true,
		autoLoad: true, 
	    url: "api/List_CmImpChequeMonth.php",
	    baseParams: { type: "cm_imp_cheque_month_hdr", i_read: user_right_read }, //Permission i_read
	    root: "data",
	    idProperty: "id",
			...
			new Ext.Window({
				...
				buttons: [{
					...
					handler: function() {
						...
						Ext.Ajax.request({
							url: "api/mn_CmImpChequeMonth.php",
							method: "POST",
							params: {
								mode: "DELETE", 
								id: record.get("id")
				...
				Ext.Ajax.request({
					...
					success: function ( result, request ) {
						...
						if ( jsonData.success == true ) {
							...
							Ext_Show( jsonData.cm_imp_cheque_month_hdr_id );
	...
	cellClick	= function( grid, rowIndex, columnIndex, e ) {
		...
		if (columnIndex == grid.getColumnModel().getIndexById("edit")) {
			if (record.get("i_enable") == 1) { controllTab(record, "edit"); }
		} else if (columnIndex == grid.getColumnModel().getIndexById("delete")) {
			if (record.get("i_enable") == 1) { controllTab(record, "delete"); }
		}
	}; //cellClick
	...
	gridMain = new Ext.grid.GridPanel({
		...
		tbar: [{
            ...
            items: [{
            	...
            	xtype: "buttongroup",
            	frame: false,
            	items: [{ xtype: "label", text: "วันที่เอกสาร : " }, { xtype: "tbspacer", width: 4 }, {
   				...
   				$( "input[id^=chk]" ).each(function( i, val ) {
   					if(val.checked == true) {
   						...
   						jsonArr.push({
   							...
   							d_cheque: $( "#d_cheque\\["+val.value+"\\]" ).val(),
				...
				if( msg == "" ) {
					...
					$.ajax({
						...
						success: function(result) {
							...
							} else { Ext.Msg.alert("แจ้งเตือน", data.msg); } 
						}
					});
			...
			function imp_data(obj) {
				...
				if( obj.totalCount > 0 ) {
					$.each(obj.data , function( index, v ) {
						...
						addBody	+= "<td nowrap align='center'>"+shortThaiDate(v.d_cheque_show)+"</td>";
						...
						addBody	+= "<input type='hidden' id='d_cheque["+v.id+"]' value='"+v.d_cheque+"'>";
					...
					addBody	+= "<tr>";
					addBody	+= "<td align='right' colspan='5'><b>จำนวนเงินนำเข้าทั้งหมด</b></td>";
				...
				listeners: {
					afterrender: function( component ) {
						...
						$.ajax({
							...
							type: "POST",
							data: {
								type: "cm_imp_cheque_month_dtl",
								cm_imp_cheque_month_hdr_id: id
							},
							success: function(result) {
								...
								if( obj.totalCount > 0 ) {
									imp_data(obj);									
								} else {  $("#Ext_table > tbody:last").append( "<td colspan='8'>ไม่พบข้อมูล</td>" ); }
							}
						});
			        }
				},
