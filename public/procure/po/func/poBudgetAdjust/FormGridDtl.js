...
const saveDtl = function(mode) {
  ...
  sto.forEach(function(v) {
    jsonArr.push({
      bg_budget_dtl_adjust_id: v.data.id,
      bg_expense_id: v.data.bg_expense_id,
      f_increase: v.data.f_increase ? v.data.f_increase.replace(/,/g, "") : "",
      f_decrease: v.data.f_decrease ? v.data.f_decrease.replace(/,/g, "") : "",
      f_cancel: v.data.f_cancel ? v.data.f_cancel.replace(/,/g, "") : "",
        ...
        columns: [
          ...
          }),
          {
            ...
            renderer: function(value, metaData, record, rowIndex, colIndex, store) {
              if (value != "" && value != undefined) {
                ...
                let name = getStoreItems(Ext.bg_expense, value, "c_group_name");
                return name;
              } else {
          ...
          },
          {
            ...
            renderer: function(value, metaData, record, rowIndex, colIndex, store) {
              if (value != "" && value != undefined) {
                metaData.attr = "style='text-align: left;'";
                let name = getStoreItems(Ext.bg_expense, value, "c_name");
                return name;
              } else {
                metaData.attr = "style='text-align: center; color:red;'";
                return "-";
          ...
          {
            header: "จำนวนเงิน<br>(เพิ่ม)",
            sortable: false,
            align: "center",
            dataIndex: "f_increase",
            width: 110,
          ...
          {
            header: "จำนวนเงิน<br>(ลด)",
            sortable: false,
            align: "center",
            dataIndex: "f_decrease",
            width: 110,
            editor: new Ext.form.TextField({
              style: "text-align: right",
              listeners: {
