...
if ($_REQUEST["type"] == "bg_budget_hdr_transfer") {
	...
	$sqlMain = "
		SET NOCOUNT ON
        SELECT
            ROW_NUMBER() OVER (ORDER BY a.i_year DESC, a.d_doc_date DESC, a.dc_expense_budget_type_id) AS numrow
            ,a.bg_budget_hdr_transfer_id
        INTO #TemData
        FROM dbo.bg_budget_hdr_transfer a
        WHERE a.i_enable = 1
            {$con};

        SELECT
            a.numrow
			,b.bg_budget_hdr_transfer_id
			,b.i_year
        ...
        FROM #TemData a
			INNER JOIN dbo.bg_budget_hdr_transfer b ON a.bg_budget_hdr_transfer_id = b.bg_budget_hdr_transfer_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"								=> $row["numrow"],
				"id"								=> $row["bg_budget_hdr_transfer_id"],
				"i_year"							=> $row["i_year"],
	...
	exit;
} else if ($_REQUEST["type"] == "bg_budget_dtl_transfer") {

	$sqlMain = "
		SET NOCOUNT ON
		SELECT
			ROW_NUMBER() OVER (ORDER BY b.c_code) AS numrow
			,a.bg_budget_dtl_transfer_id
			,a.bg_expense_begin_id
			,a.bg_expense_end_id
			,a.f_total
			,a.c_comment
		INTO #TemData
		FROM dbo.bg_budget_dtl_transfer a
			LEFT JOIN dbo.bg_expense b ON a.bg_expense_begin_id = b.bg_expense_id AND b.i_enable = 1 AND b.i_delete = 2
		WHERE a.bg_budget_hdr_transfer_id = ?;
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"										=> $row["numrow"],
				"id"										=> $row["bg_budget_dtl_transfer_id"],
				"bg_expense_begin_id"						=> $row["bg_expense_begin_id"],
				"bg_expense_end_id"							=> $row["bg_expense_end_id"],
				"f_total"									=> $row["f_total"],
				"c_comment"									=> $row["c_comment"],
			);
