...
if ($_REQUEST["type"] == "deliveries") {
    ...
    $sqlTempTable = "SELECT a.sp_tor_contract_id
                    ...
                    INNER JOIN dbo.sp_emp bb on bb.sp_emp_id = cc.sp_emp_id 
                    {$ses}
                    WHERE a.parent_id = 0 and a.i_contract_status > 0 {$wh} and cc.i_type_bg <> 2 "; // cc.c_code is not null  and cc.i_type_bg in(1,8)   and  
    //    echo $sqlTempTable; 
    //    exit;
    $arrParam[] = $start;
    $arrParam[] = $limit;
    $sqlMain = "select a.* "
        . ", s.c_code as tor_code
                ...
                , convert(varchar,isnull((select top 1 d_create from sp_doc_gen where ref_id = a.sp_tor_contract_id ORDER by i_value desc   ),c.d_doc_date),120) as d_doc_create
                ...
                , s.f_type5_amt        
                , case when s.i_type_bg = 4 then  1  else  s.i_pr_type1 end as i_pr_type1  
                , s.i_pr_type2 
                , s.i_pr_type3
                , s.i_pr_type4 
                , s.i_pr_type5
                ...
                when s.i_type_bg = 13 then 3 
                else  0 end   as i_status_overlap  
                -- 0 ปกติ  1 กันเหลื่อมยังไม่ก่อหนี้    2 กันเหลื่อม ก่อหนี้แล้ว 3 ไม่จองเงิน   
                , s.dc_cost_id
                , s.i_is_notor  
                , s.i_purchase 
                , CASE
...
//    exit;
    ...
    while ($row = $db->Fetch($stmt)) {
        ...
        $temp = array(
            ...
            // contract
            //แหล่งเงินที่ 1   
            "dtl_po_expense_id1" => $row['po_expense_id'] ?? 0,
            "i_type_guarantee" => intval($row["i_type_guarantee"]),
