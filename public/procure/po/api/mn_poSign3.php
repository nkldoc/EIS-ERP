    ...
    case "SEND_DATA":
        ...
        if ($Arr) {
            ...
            foreach ($Arr as $id) {
                ...
                $date_back = $db->GetDataBySQL("SELECT DATEDIFF(DAY, d_doc_date, '{$_REQUEST["d_doc_date"]}') FROM dbo.po_working_item WHERE po_working_hdr_id = {$id} AND i_status = ?;", array($_REQUEST["i_status"] - 1));
                if (!$date_back) {
                    $date_back = $db->GetDataBySQL("SELECT DATEDIFF(DAY, d_doc_date, '{$_REQUEST["d_doc_date"]}') FROM dbo.po_working_item WHERE po_working_hdr_id = {$id} AND i_status = ?;", array($_REQUEST["i_status"] - 2));
                }
                if ($date_back >= 0) {

                    $po_working_item_id = $db->GetDataBySQL("SELECT po_working_item_id FROM dbo.po_working_item WHERE po_working_hdr_id = {$id} AND i_status = ?;", array($_REQUEST["i_status"]));
