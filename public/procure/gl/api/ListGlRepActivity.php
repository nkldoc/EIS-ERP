...
#################################
 function getAccDtl($hdrID){
	...
	where a.gl_rep_conf_id=?", array($hdrID));  
	if ($ret>0) return true; 
	else return false; 
}
$arrParam = array();
$arrCountParam = array();
$sqlTempTable = "select gl_rep_conf_id
					, gl_dc_activity_id
					, c_name
					, i_row
					, i_source
	...
	while($row =$db->Fetch($stmt))				
	{
		...
		$temp = array("no" => ($i++), 
                                "id"                        => $row["gl_rep_conf_id"],
								"gl_dc_activity_id"         => $row["gl_dc_activity_id"], 
                                "c_name"                    => $row["c_name"], 
								"i_source"                  => $row["i_source"],
								"i_source_item"             => $row["i_source_item"],
								"i_source_item1"             => $row["i_source"]==1?$row["i_source_item"]:1,
								"i_source_item2"             	=> $row["i_source"]==2?$row["i_source_item"]:1,
								"i_is_acc"             			=> getAccDtl($row["gl_rep_conf_id"])?true:false,
								"i_level"                  		=> $row["i_level"],
								"i_row"                  		=> $row["i_row"],
								"i_show"                  	=> $row["i_show"], 
