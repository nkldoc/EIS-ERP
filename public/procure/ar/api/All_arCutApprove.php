...
if ($_REQUEST["type"] == "GROUP_DATE") {
	...
	$sqlMain	= "
		SET NOCOUNT ON
		DECLARE @d_start AS DATE	= (SELECT MIN(d_action_date) FROM dbo.ar_cut_log a WHERE a.i_success_api = 1 AND a.i_success_ar = 1 AND a.i_success_approve = 0);
		DECLARE @d_end AS DATE		= (SELECT MAX(d_action_date) FROM dbo.ar_cut_log a WHERE a.i_success_api = 1 AND a.i_success_ar = 1 AND a.i_success_approve = 0);
