...
Ext.AppUx = function (app, menu) {
    ...
    function winProcess(rec) {
        ...
        new Ext.Window({
            id: "win-processID",
            title: "ผ่านรายการ PR ประเงินอุดหนุน/งวด เพื่อออกเลขสัญญา",
            modal: true,
            resizable: false,
            width: 550,
            layout: "form",
...
//                            Ext.status.process("ST0003", rec);
    ...
    Ext.processToContract = function(rec){
         ...
         Ext.Ajax.request({
            ...
            success: function (result, request) {
                ...
                callback: function (record, operation, success) {
                    if (success) {
                        //บันทึกแล้ว 
                          websocket = new WebSocket("ws://"+window.parent.Ext.ipServer+":9000/demo/server.php");
                          websocket.onopen = function (ev) { // connection is open   
                              var msg = {
                                  message:'<font color="#000">'+ rec.get('c_code')
                                    +' '+rec.get('c_name')+'\r'
                                    +' '+rec.get('f_total_amt')+'\r' 
                                    +' คำของจองเงินอุดหนุนได้รับอนุมิตแล้ว'
                                    +' </font>',
                                  name: rec.get('dc_user_id'),
                                  color: '#007AFF'
                              };
