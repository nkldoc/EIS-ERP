	...
	case "IMPORT_EXCEL":
		...
		if ($handle != "") {
			...
			while ($obj = fgetcsv($handle, 1000, ",")) {
				...
				if ($n >= 3) 
				{ //เช็คข้อมูลตั้งแต่บรรทัดที่ 3 ของไฟล์ Excel
					...
					if ($data_conv["0"] != "") { 
							...
							$i_row++;
							 ...
							 { //แถวที่มี ปฏิทิน เป็นแถวแรก
								...
								$temp								= array(); 
								$temp["c_type"]						= "MAIN";
								$temp["i_rank_show"]				= $data_conv["0"];
								$temp["c_comment"]					= $data_conv["1"];
								$temp["c_expense_group_main"]		= $data_conv["2"]; 
								$temp["c_creditor"]					= $data_conv["3"]; 
								$temp["d_doc"]						= $data_conv["4"];
								$temp["f_inv"]						= (trim($data_conv["5"],"")>0) ? str_replace(",", "", $data_conv["5"]) : "0";
								$temp["c_acc_code"]					= $data_conv["6"];
								$temp["c_acc_name"]					= $data_conv["7"];
								$temp["c_acc_code2"]				= $data_conv["8"];
								$temp["c_acc_name2"]				= $data_conv["9"]; 
								$temp["i_status"]					= 1;   
							...
							if ($k>0)
							{
								...
								$temp_dr 							= array(); 
								$temp_dr["c_type"]					= "SUB"; 
								$temp_dr["i_rank_show"]				= $data_conv["0"];//$k;
								$temp_dr["f_dr"]					= str_replace(",", "", $data_conv["5"]);
								$temp_dr["f_cr"]					= "0";								 
								$temp_dr["c_acc_code_imp"]			= str_replace(' ', '', $data_conv["6"]);
								$temp_dr["c_acc_name_imp"]			= ltrim($data_conv["7"]);
								$temp_dr["i_cal_gl"]				= "2";   
								$temp_dr["i_rank_dr"]				= "1"; 
								...
								$temp_cr 							= array(); 
								$temp_cr["c_type"]					= "SUB"; 
								$temp_cr["i_rank_show"]				= $data_conv["0"];//$k;
								$temp_cr["f_dr"]					= "0";
								$temp_cr["f_cr"]					= str_replace(",", "", $data_conv["5"]);								 
								$temp_cr["c_acc_code_imp"]			= str_replace(' ', '', $data_conv["8"]);
								$temp_cr["c_acc_name_imp"]			= ltrim($data_conv["9"]);
								$temp_cr["i_cal_gl"]				= "2";   
								$temp_cr["i_rank_dr"]				= "0"; 
			...
			if ($num_payment > 0) { // ไม่มีเลขที่ใบเบิก
				...
				foreach ($sss as $fld) { 
					...
					if ($fld["c_type"]=="MAIN")
					{
						...
						$dataA["c_acc_code"]					= $fld["c_acc_code"];
						$dataA["c_acc_name"]					= $fld["c_acc_name"];
						$dataA["c_acc_code2"]					= $fld["c_acc_code2"];
						$dataA["c_acc_name2"]					= $fld["c_acc_name2"]; 
						...
						$dataA["dc_creditor_id"]				= $db->GetDataBySQL("SELECT TOP 1 dc_creditor_id FROM NMU.dbo.dc_creditor WHERE replace(c_full_imp,' ','') =replace(?,' ','');", array($dataA["c_creditor"]));
						$dataA["i_send_jv"] 					= "2"; //1=ไม่ระบุคือยังไม่ map กับใบเบิกพิเศษ ,2=ไม่ลงบัญชี,3=ลงบัญชี
					...
					if ($msg != "") {
						...
						if ($_REQUEST["id"] == $arr_gen_code_IRCV["reference_id"]) {
							...
							if ($para) {
								...
								$msg	.= "เลขที่นำเข้าใบเบิกพิเศษ Vision Net : <b style='color:blue;'>" . $arr_gen_code_IRCV["c_code_gen"] . "</b><br>";
								$re = array(
									"success"	=> true,
									"msg"		=> $msg
								);
							...
							FROM ( 
								SELECT
									...
									where a.imp_request_vsn_hdr_id = @imp_request_vsn_hdr_id
										and a.dc_acc_id_dr>0 and a.f_dr>0
								...
								from vw_vsn_request_item_gl a 
								where a.imp_request_vsn_hdr_id = @imp_request_vsn_hdr_id
									and a.dc_acc_id_cr>0 and a.f_cr>0
