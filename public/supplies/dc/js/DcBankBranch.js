Ext.onReady(function() {
	...
	/*===============================================*/
	var title_panel	= "สาขาธนาคาร"; 
	/*===============================================*/
	var store = new Ext.data.JsonStore({
		storeId: "myStore",
	    autoDestroy: true,
		autoLoad: true,
	    url: "api/List_DcBankBranch.php",
	    root: "data",
	...
	var gridMain = new Ext.grid.GridPanel({
		...
		columns: [
			...
			{ id: "c_name", header: "ชื่อสาขา (ไทย)", sortable: true, dataIndex: "c_name" },
			{ header: "ชื่อสาขา (อังกฤษ)", sortable: true, dataIndex: "c_name_eng" },
			{ header: "ธนาคาร", sortable: true, width: 200, align: "center", dataIndex: "dc_bank_name" },
        ...
        items: [{
			...
			items: [{
				...
				items: [{
					...
					items: [{
		    			...
		    			mode: "local",
		    			triggerAction: "all",
		    			emptyText: "- กรุณาธนาคาร -",
						forceSelection: true,
						selectOnFocus: true
					}, {
						xtype: "textfield",
						fieldLabel: "รหัสสาขาธนาคาร",
						id: "branch_code",
			        	name: "branch_code",
						width: 350
					}, {
						xtype: "textfield",
						fieldLabel: "ชื่อสาขาธนาคาร(ไทย)",
						id: "c_name",
			        	name: "c_name",
						width: 350
					}, {
						xtype: "textfield",
						fieldLabel: "ชื่อสาขาธนาคาร(อังกฤษ)",
						id: "c_name_eng",
			        	name: "c_name_eng",
						width: 350
					}, {
						xtype: "checkbox",
						fieldLabel: "ประเภทการใช้งาน",
						id: "i_fin_biz",
						name: "i_fin_biz",
						boxLabel: "เป็นบัญชีที่ อสมท ติดต่อทำธุรกรรมทางการเงิน",
						checked: false,
						inputValue: 1
					}, {
						xtype: "textfield",
			...
			buttons: [{
				...
				handler: function() {
					...
					var msg		= "";
					var dc_bank_id	= Ext.getCmp("dc_bank_id").getValue().toString();
					
					if(dc_bank_id == "" || dc_bank_id == null) {
						msg	+= "- กรุณาเลือกธนาคาร<br>";
					}
					if(Ext.getCmp("c_name").getValue() == "")	{ msg	+= "- กรุณากรอกชื่อสาขาธนาคาร<br>"; }

					if (msg == "") {
						Ext.getCmp("tabpanel2").getEl().mask("Please wait...", "x-mask-loading");
						
						if(Ext.getCmp("i_enable").checked == true) { var i_enabled	= 1; }
						else { var i_enabled	= 2; }
						
						if(Ext.getCmp("i_fin_biz").checked == true) { var i_fin_biz	= 1; }
						else { var i_fin_biz	= 2; }
						...
						Ext.Ajax.request({
							...
							params : { 
								...
								branch_code: Ext.getCmp("branch_code").getValue(),
								c_name: Ext.getCmp("c_name").getValue(),
								c_name_eng: Ext.getCmp("c_name_eng").getValue(),
								i_fin_biz: i_fin_biz,
