Ext.onReady(function () {
  ...
  /*===============================================*/
  if (Ext.ITYPE_JV) {
    Ext.title_panel = "นำเข้าค่าใช้จ่าย (Vision Net)";
  } else {
    Ext.title_panel = "บันทึกบัญชีค่าใช้จ่าย (Vision Net)";
  }
  /*===============================================*/
  // pagingBar
  ...
  const controllTab = function (record, butt) {
    ...
    Ext.butt = butt;
    if (butt == "add") {
      Ext.i_system_vsn_show = true;
      Ext.i_system_ep_show  = false;   
      ...
      // ============ formAdd ============ //
      if ((record.get("i_system"))=="2")
      { //VSN
        Ext.i_system_vsn_show = true;
        Ext.i_system_ep_show  = false; 
      }
      else if ((record.get("i_system"))=="1")
      { //EP
        Ext.i_system_vsn_show = false;
        Ext.i_system_ep_show  = true;
      }
      else
      {
        Ext.i_system_vsn_show = false;
  ...
  cellClick = function (grid, rowIndex, columnIndex, e) {
    ...
    } else if (columnIndex == grid.getColumnModel().getIndexById("editdetail")) {
      if (record.get("i_is_post") > 1 && record.data.i_enable == 1) {
        controllTab(record, "editdetail");
      }
  ...
  const gridMain = new Ext.grid.GridPanel({
    ...
    columns: [
      ...
      },
      {
        ...
        dataIndex: "id",
        renderer: function (value, metaData, record, row, col, store, gridView) {
          if (!(record.get("i_is_post") > 1) && record.get("i_enable") == 1) {
      ...
      { header: "เลขที่ค่าใช้จ่าย", sortable: false, align: "center", width: 100, dataIndex: "c_code" },
      {
      ...
      {
        header: "รหัสอ้างอิงค่าใช้จ่าย (GX)",
        id: "print",
        sortable: true,
        dataIndex: "c_gx_code",
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
      ...
      },
      {
        ...
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
          if (value == 1) {
            return "<span style='color:#44c3ff;'>e-phis</span>";
          } else {
            return "<span style='color:red;'>vsn</span>";
          }
        },
      },
      { header: "เอกสารค่าใช้จ่ายอ้างอิง", sortable: true, width: 130, dataIndex: "c_expense_vsn_period_no" },
      ...
      { header: "รหัสหน่วยงาน", sortable: true, width: 130, dataIndex: "dc_cost_acc" },
      {
        header: "สถานะใช้งาน",
        sortable: true,
        align: "center",
  ...
  let myComboStores = [Ext.dc_expense_budget_type, Ext.dc_expense_budget_type_all, Ext.vw_dc_bank_acc_company_full1, Ext.vw_dc_bank_acc_company_full2, Ext.dc_expense_group_vsn, Ext.dc_expense_acc_vsn_full,Ext.imp_request_vsn_dtl_gx,Ext.imp_request_ephis_dtl_gx];
