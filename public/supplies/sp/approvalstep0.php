...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <head>
        ...
        <style>
            ...
            .dtlRow{
                padding:2px;
                font:normal 13px/16px 'Mitr', sans-serif;
            }
.x-grid3-row-selected {
    background-color: #fff !important;
    font:normal 20px/22px 'Mitr', sans-serif !important; 
...
// --------config ---------------------------------------------------------------
            Ext.loadConfig = ()=> {
                Ext.Ajax.request({
                    url: '/supplies/sp/app/approved/conf/config.json',
                    method: 'GET',
                    success: function (response) {
                        var configArray = Ext.decode(response.responseText);
                        Ext.config = configArray;
                        Ext.menu_name = 'บันทึกใบขอซื้อจ้าง <font color="/blue/">(ลงนาม เอกสารภายใน)</font>';
                        Ext.menu_code = configArray.status_step_document[1].c_code; // {APSTEP11,APSTEP21}
        ...
        <script type="text/javascript" src="./../js/RowExpander.js?_dc=<?= __VPRODUCT_; ?>"></script> 
        <script type="text/javascript" src="./uiData/configStoreUi.js?_dc=<?= __VPRODUCT_; ?>"></script> 
<!--gen_new--> 
        <script type="text/javascript" src="app/spApproveDocument/app.js?_dc=<?PHP echo  __VPRODUCT_; ?>"></script>  
    </head>
