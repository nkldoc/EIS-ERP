...
switch ( $mode ) {
 ...
 //start Log
        $db -> logs ( "po-DcEmpPo"
            ...
            , "dc_emp_id"
            , $data[ 'id' ]
            , "DELETE dbo.po_emp_item where dc_emp_id=?"
            , array ( $data[ 'id' ] )
            , $_SESSION ) ;
        $stmt2 = $db -> QueryParam ( "DELETE dbo.po_emp_item where dc_emp_id=?" , array ( $data[ 'id' ] ) ) ;

        $chkarr = $data[ 'chk' ] ?? null ;
        if ( is_array ( $chkarr ) ) {
            $arrParam = array () ; // row , status_id, seq
            foreach ( $chkarr as $k => $v ) {
                foreach ( $v as $kk => $vv ) {
                    $po_status_hdr_id = $kk ?? null ; // row
                    $i_seq            = $vv ?? null ;
                ...
                $sql = "insert into dbo.po_emp_item (po_status_hdr_id, dc_emp_id, i_seq
