...
if($saveGroupMenu){
	...
	if ($groupId > 0)
	{
		...
		select mm.dc_menu_id
			...
			, ".$ww_field_i_read_overall." as i_read_overall 
		...
		from dc_menu mm
				{$strTableRight}
		where i_delete=@iDelete AND i_enable = 1
		
		select dc_menu_id
			, c_name
			, (SELECT TOP 1 dc_menu_id FROM dc_menu WHERE c_code =  a.parent_code AND i_delete=@iDelete) AS parent_id
			, i_level
...
$stmt = $db->QueryParam($sql, array(DELETE_FALSE, $id));
	...
	while($row =$db->Fetch($stmt))
	{
		...
		$temp = array(
				...
				"i_read_overall"    =>$row["i_read_overall"],
