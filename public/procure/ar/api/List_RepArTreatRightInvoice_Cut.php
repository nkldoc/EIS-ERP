...
function List_QueryParam()
{
	...
	$sqlMain = "
		...
		SELECT
			...
			,b.c_code_bill
			,CONVERT(VARCHAR, b.d_bill_date, 120) AS d_bill_date
			,a.c_code_cut
			,CONVERT(VARCHAR, a.d_doc_date, 120) AS d_cut_date
			,b.c_hn
			,b.c_an
			,b.c_patient
		...
		WHERE a.i_enable = 1
			...
			{$con}
		ORDER BY a.d_doc_date, d.c_name, c.c_name, a1.c_name, b.d_service_date;";
	...
	if ($stmt) {
		...
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				...
				"d_bill_date"							=> ($row["d_bill_date"] != "") ? $date->extDateBuddha($row["d_bill_date"]) : "",
