...
$table	= "cm_imp_cheque_hdr";
$key_id	= "cm_imp_cheque_hdr_id";
...
switch ( $mode ) {
		...
		if($handle != "") {
			...
			while ($obj = fgetcsv($handle, 1000, ",")) {
				...
				if($n >= 2) { //เช็คข้อมูลตั้งแต่บรรทัดที่ 2 ของไฟล์ Excel
					...
					$temp["no"]					= $totalCount;
					$temp["id"]					= $totalCount;
					$temp["c_doc"]				= $data_conv["0"];
					$temp["c_name"]				= $data_conv["1"];
					$temp["f_dr"]				= str_replace(",","",$data_conv["2"]);
					$temp["f_cr"]				= str_replace(",","",$data_conv["3"]);
					$temp["f_balance"]			= str_replace(",","",$data_conv["4"]);
	...
	case "GENCODE" :
		...
		$db->QueryParam("DELETE cm_imp_cheque_dtl WHERE cm_imp_cheque_hdr_id = ?;", array($_REQUEST["id"]));
		// ========================= add dtl ========================= //
		$Arr	= json_decode($_REQUEST["data"], true);
		foreach( $Arr as $fld ) {
			
			// ================== INSERT DTL ================== //
			$data["cm_imp_cheque_hdr_id"]					= $_REQUEST["id"];
			...
			$sql	= "INSERT INTO cm_imp_cheque_dtl (".substr($addField, 1).") VALUES (".substr($addValue,1).");";
			...
			$re	= array("success"=>true, "cm_imp_cheque_hdr_id"=>$_REQUEST["id"]);
		}
		...
		if( $mode == "GENCODE" ) {
			...
			if( $msg != "" ) {
				$re	= array("success" => false, "msg" => $msg, "cm_imp_cheque_hdr_id" => $_REQUEST["id"]);
			} else {
	...
	case "DELETE" :
		...
		if( $c_code != "0" && $c_code != "" ) { // ปรับสถานะเป็นไม่ใช้งาน
			...
			$db->QueryParam("DELETE cm_imp_cheque_dtl WHERE {$key_id}=?;", array($_REQUEST["id"]));
