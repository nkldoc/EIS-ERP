...
if ($dc_cost_id > 0)
{
    $sqlWhere .= " and a.dc_cost_acc_id = ?";
    $arrParam[] = $dc_cost_id;
}

$sql = "select (select c_code+' '+c_name from dc_cost where dc_cost_id = d.dc_cost_acc_id) as cost_acc_name
        ...
        from am_tf_hdr a 
                ...
                inner join dc_acc f on e.dc_acc_conf_recv_id = f.dc_acc_id
        where  a.c_code_gen like 'BT%' 
                ...
                {$sqlWhere}
        order by cost_code, acc_code, a.c_code_gen";
...
{
    if ($tempCostName != $row["cost_acc_name"])
    {
        if ($sumAcc_cost > 0)
        {
            $str .= "<tr bgcolor='#f0f4fa'>"
                    ."<td align='right' colspan='6'>รวมบัญชีสินทรัพย์ {$tempAccCode}: {$tempAccName}</td>"
        ...
        if ($sumCost_cost> 0)
        {
            $str .= "<tr bgcolor='#C6D2D1'>"
                ."<td align='right' colspan='6'>รวมศูนย์ต้นทุนทางบัญชี : {$tempCostName}</td>"
        ...
        $str .= "<tr bgcolor='#C6D2D1'>"
                ."<td align='left' colspan='12'>ศูนย์ต้นทุนทางบัญชี : {$row["cost_acc_name"]}</td>"
                ."</tr>";

        $tempCostName = $row["cost_acc_name"];
    }
    ...
    if ($tempAccCode != $row["acc_code"])
    {
        ...
        $str .= "<tr bgcolor='#f0f4fa'>"
                ."<td align='left' colspan='12'>บัญชีสินทรัพย์ : {$row["acc_code"]} {$row["acc_name"]}</td>"
                ."</tr>";
...
$str = '<table cellspacing="0" cellpadding="0" width="100%" border="0" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
                        ...
                        <tr><th colspan="12"><b>ศูนย์ต้นทุนทางบัญชี :</b> '.$costName.'</th></tr>
			<tr><th colspan="12">ตั้งแต่วันที่ '.$strDateBetween.'</th></tr>
                        <tr><td colspan="12" align="right">หน่วย : บาท</td></tr>
		</table>
		<table cellspacing="0" cellpadding="3" width="100%" border="1" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
                    <tr bgcolor="#A5BAD6">
                        ...
                        <th align="center" nowrap><b>รหัสหน่วยงาน</b></th>
                        <th align="center" nowrap><b>ชื่อหน่วยงาน</b></th>
