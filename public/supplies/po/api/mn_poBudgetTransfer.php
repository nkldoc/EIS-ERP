...
switch ($mode) {
	...
	case "EDIT":
		...
		$data["i_transfer"]								= $_REQUEST["i_transfer"];
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				SET NOCOUNT ON
				INSERT INTO bg_budget_hdr_transfer (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE bg_budget_hdr_transfer SET " . substr($addField, 1) . " WHERE bg_budget_hdr_transfer_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
	...
	case "SAVE_DTL":
		...
		if ($msg == "") {
			foreach ($Arr as $fldd) {
				...
				$data["bg_expense_begin_id"]							= $fldd["bg_expense_begin_id"];
				$data["bg_expense_end_id"]								= $fldd["bg_expense_end_id"];
				...
				if ($fldd["bg_budget_dtl_transfer_id"] > 0) { // EDIT
					...
					$arrValue[] = $fldd["bg_budget_dtl_transfer_id"];
					$sql		= "UPDATE bg_budget_dtl_transfer SET " . substr($addField, 1) . " WHERE bg_budget_dtl_transfer_id = ?";
					$db->QueryParam($sql, $arrValue);
				} else { // ADD

					$data["bg_budget_hdr_transfer_id"]						= $_REQUEST["id"];
					$data["dc_user_create_id"]								= $_SESSION["user_id"];
					$data["dc_user_create_cost_id"]							= $_SESSION["dc_cost_id"];
					$data["d_create"]										= date("Y-m-d H:i:s");
					...
					$sql = "
						SET NOCOUNT ON
						INSERT INTO bg_budget_dtl_transfer (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
						SELECT @@IDENTITY as id;";
