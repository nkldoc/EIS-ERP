...
function List_QueryParam() {
	...
	$conDCCheque	= "";
	$conDateCancel	= "";
	if($_REQUEST["page"] == "GlRepCancelChequeIMP") { // รายงาน ยกเลิกบัญชีย่อยเงินฝากธนาคาร
		...
		$sqlMain = "SET NOCOUNT ON
					...
					FROM imp_expense_hdr a
						...
						INNER JOIN imp_cancel_doc_expense r ON r.gl_tran_hdr_bank_id_source = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I'
						and r.i_type_doc=1 and r.i_enable=1
					...
					FROM imp_expense_vsn_hdr a
						...
						INNER JOIN imp_cancel_doc_expense r ON r.gl_tran_hdr_bank_id_source = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I'
						and r.i_type_doc=2 and r.i_enable=1
	...
	else if($_REQUEST["page"] == "GlRepCancelChequeBTN") { // รายงาน ยกเลิกบัญชีเงินฝากธนาคาร(ไม่มีรายละเอียดค่าใช้จ่าย)
		$conDateCancel	= "";
		$for_id = explode ( ";", $_REQUEST ["dc_bank_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
		...
		$sqlMain = "SET NOCOUNT ON
					...
					WHERE a.c_code IS NOT NULL
						...
						and r.i_type_doc=3 and r.i_enable=1
						{$conDate} {$conCheque} {$conFixedAcc2}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, a.d_save_jv_date,a.f_money 
					UNION ALL
					SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
