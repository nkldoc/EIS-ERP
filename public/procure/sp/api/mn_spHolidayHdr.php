...
switch ($mode) {
	...
	case "EDIT":
		...
		if ($mode == "ADD") {
			$holiday = $db->GetDataBySQL("SELECT a.* FROM dbo.sp_holiday_hdr a WHERE a.i_year = ? AND a.i_enable = 1", array($_REQUEST["i_year"]));
			if (empty($holiday)) {
				...
				$sql	= "
				SET NOCOUNT ON
				INSERT INTO sp_holiday_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE sp_holiday_hdr SET " . substr($addField, 1) . " WHERE sp_holiday_hdr_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
