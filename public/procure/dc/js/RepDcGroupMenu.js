Ext.onReady(function() {
  ...
  /*===============================================*/
  var title_panel = "รายงานต้นแบบกลุ่มการใช้งานระบบ";
  /*===============================================*/
  ...
  // หมวดผังบัญชี
  var store_dc_menu_hdr = new Ext.data.JsonStore({
    autoDestroy: true,
    autoLoad: true,
    url: "api/All_RepDcGroupMenu.php",
    baseParams: { type: "dc_menu_hdr" ,all : "all" },
    root: "data",
    idProperty: "id",
    fields: [{ name: "id" }, { name: "c_name", type: "string"}],
    listeners: {
      load: function(t, records, options) {
        Ext.getCmp("dc_menu_hdr_id").setValue("0");
      }
    }
  });
  ...
  LookReport = function(type) {
    ...
    if (Ext.getCmp("dc_menu_hdr_id").getValue()=="0")
    {
      msg = "กรุณาเลือกต้นแบบอย่างน้อย 1 รายการ";
    }

    if (msg == "") {
      var href = "report/Rep_DcGroupMenu.php";
      var resultUrl = "";

      resultUrl += "&type=" + type; 
      resultUrl += "&i_enable=" + Ext.getCmp("i_enable").getValue();
      resultUrl += "&dc_menu_hdr_id=" + Ext.getCmp("dc_menu_hdr_id").getValue();
