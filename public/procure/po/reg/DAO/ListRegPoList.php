...
if ( ! $util -> get ( $sort ) ) {
    $sort = "dc_emp_id,i_enable, c_code" ;
}

#################################
$arrParam = array();
$arrCountParam = array();
$sqlTempTable = "select dc_emp_id
                    , dc_cost_id
                    , (select top 1 c_code+' '+c_name from vw_dc_cost where dc_cost_id=a.dc_cost_id) as c_cost_name
                    , dc_title_id
                    , c_code
                    , c_title
                    , c_name
                    , case when c_title = '-ไม่ระบุ-' then '' else c_title end+' '+c_name as c_full_name
                    , c_ref_value
                    , c_tax_value
                    , c_tel_home
                    , c_tel_office
