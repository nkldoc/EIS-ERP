...
$txtBuild = $_REQUEST["txts-dc_building_id"];
$dc_ins_town_hdr_id = $_REQUEST["dc_ins_town_hdr_id"];
$txtTownHdr = $_REQUEST["txts-dc_ins_town_hdr_id"];
...
function genReportByType($i_type, $dc_building_id, $dc_ins_town_hdr_id, $type_name, $txtBuild, $txtTownHdr, $i_start_month, $i_start_year)
{
    ...
    $sql = "select e.c_code as build_code
            ...
            where a.i_enable= ? 
                ...
                and a.i_is_method= ?
                {$sqlWhere}
            group by e.c_code, e.c_name, d.dc_ins_town_hdr_id, d.c_name
    ...
    while ($row = $db->Fetch($stmt))
    {
        ...
        $str .= "<tr>"
                ...
                ."<td align='left' nowrap>&nbsp;{$row["c_name"]}</td>";
