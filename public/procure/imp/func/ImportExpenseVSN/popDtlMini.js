...
const PopDtlMini = function(record) {
  ...
  new Ext.Window({
    title: "รายละเอียดค่าใช้จ่าย (vision net)",
    id: "win-pop-dtl",
    modal: true,
          ...
          success: function(result) {
            ...
            if (obj.debug == true) {
              $.each(obj.data, function(index, v) {
                ...
                v.c_import_rq_code  = (v.c_import_rq_code==null)        ? "" : v.c_import_rq_code ;
                v.c_request           = (v.c_request==null)             ? "" : v.c_request ;
                v.c_import_rq_jv_code = (v.c_import_rq_jv_code==null)   ? "" : v.c_import_rq_jv_code ;
                v.c_acc_item_import   = (v.c_acc_item_import==null)     ? "" : v.c_acc_item_import ;
                // GEN TBODY
                ...
                addBody += "<td>" + v.dc_expense_group_vsn_name + "</td>";
                addBody += "<td>" + v.dc_expense_acc_vsn_name + "</td>";
                addBody += "<td align='center'><font color=green>" + v.c_acc_name + "</font></td>";
                addBody += "<td align='center'>" + v.c_approve + "</td>";
                addBody += "<td nowrap align='center' style='color: red;'>" + v.d_doc_show + "</td>";
                ...
                addBody += "<td nowrap align='right'>" + floatRenderer(floatMinus(v.f_prov_fund, 2)) + "</td>";
                ...
                addBody += "<td nowrap align='center'>" + v.c_import_rq_code + "</td>";
                addBody += "<td nowrap align='left'>" + v.c_request + "</td>";
                addBody += "<td nowrap align='center'>" + v.c_import_rq_jv_code + "</td>";
                addBody += "<td nowrap align='left'>" + v.c_acc_item_import + "</td>";
    ...
    html:
      ...
      "<td>เลขที่นำเข้าใบเบิก</td>" +
      "<td>เลขที่ใบเบิก</td>" +
      "<td>เลขที่บันทึกบัญชีตั้งหนี้จากใบเบิก</td>" + 
      "<td>รายการ(ใบเบิก)</td>" + 
  ...
  var myFunc = function(index, v = null) {
    new Ext.form.TextField({
      id: "dtl_imp_expense_vsn_dtl_id[" + index + "]",
      hidden: true
    });
    ...
    if (v != null) {
      Ext.getCmp("dtl_imp_expense_vsn_dtl_id[" + index + "]").setValue(v.id);
