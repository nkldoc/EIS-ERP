...
function List_QueryParam()
{
	...
	$sqlMain = "
		...
		FROM dbo.po_working_hdr aa
			INNER JOIN dbo.po_working_dtl bb ON aa.po_working_hdr_id = bb.po_working_hdr_id AND aa.i_status_last >= 5
				AND bb.po_expense_id IS NOT NULL
			INNER JOIN dbo.vw_po_expense_with_parent cc ON bb.po_expense_id = cc.po_expense_lv4_id
			INNER JOIN dbo.po_working_item dd ON aa.po_working_hdr_id = dd.po_working_hdr_id AND dd.i_status = 5
		WHERE aa.i_enable = 1
		...
		FROM #tempBudget a
			...
			FULL JOIN #tempWorking d ON a.po_expense_id = d.po_expense_id;
	...
	if ($stmt) {
		...
		while ($row = $db->Fetch($stmt)) {
			if ($row["i_level"] == 1) {
				...
				$f_budget_adjust	+= $row["f_budget_adjust"];
				$f_working			+= $row["f_working"];
				$f_sum				+= ($row["f_budget"] + $row["f_budget_adjust"]) - $row["f_working"];
			}
			...
			if (
				...
				if ($row["f_budget"] + $row["f_budget_adjust"] > 0 && $row["f_working"] > 0) {
					$f_percent = (($row["f_budget"] + $row["f_budget_adjust"]) / $row["f_working"]);
				} else {
					$f_percent = 0;
				}
				$temp = array(
					...
					"f_budget_adjust"				=> $row["f_budget_adjust"],		// โอนเปลี่ยนแปลงตามบัญชีจัดสรร
					"f_working"						=> $row["f_working"],			// หักงบประมาณ
					"f_sum"							=> ($row["f_budget"] + $row["f_budget_adjust"]) - $row["f_working"], // คงเหลืองบประมาณ
					"f_percent"						=> $f_percent
				);
				${$root}[]	= $temp;
			}
		...
		$temp = array(
			...
			"f_budget_adjust"				=> $f_budget_adjust,	// โอนเปลี่ยนแปลงตามบัญชีจัดสรร
			"f_working"						=> $f_working,			// หักงบประมาณ
			"f_sum"							=> $f_sum,				// คงเหลืองบประมาณ
		);
		${$root}[]	= $temp;
	}
