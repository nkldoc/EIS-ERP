...
#################################
	...
	if($mode=="SEARCH"){
		...
		{
			$wh 				.= " and i_enable = ?";
			$arrParam[]  		= $_REQUEST['status'];
			$arrCountParam[]	= $_REQUEST['status'];		
		} 
		if ($filter!="" && $value!="")
		{
			$wh 				.= " and ".$filter." like ?";
	...
	$sqlTempTable = "select dc_cnt_type_id 
					, c_code 
					, c_name   
					, c_comment
					, i_enable
				...
				where isnull(i_delete,".DELETE_FALSE.") = ? $wh".$util->viewAcc($i_read);
