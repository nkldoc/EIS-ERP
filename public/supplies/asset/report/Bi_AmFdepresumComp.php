...
include("../api/List_BiAmFdepresumComp.php");
...
function changeNumFormat($val)
{
	if ($val > 0) {
		...
		$val = "<font color=red>(" . number_format(abs($val), 0) . ")</font>";
	} else {
		$val = "-";
	}
	return $val;
...
if (is_array($data_dtl) && count($data_dtl["data"]) > 0) {
	...
	foreach ($data_dtl["data"] as $index => $jObj) {
		if ($jObj["i_type"] == '1') {
			...
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='center' nowrap>" . $jObj["acc_code"] . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='left' nowrap>" . $jObj["acc_name"] . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap>" . changeNumFormat($jObj["y1"]) . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap>" . changeNumFormat($jObj["y2"]) . "</td>";
			...
			$tbody .= "<tr>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' colspan=3 nowrap><b>รวม</b></td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap><b>" . changeNumFormat($jObj["y1"]) . "</b></td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='right' nowrap><b>" . changeNumFormat($jObj["y2"]) . "</b></td>";
		}
		$tbody .= "</tr>";
	}
	$tbody .= "</tbody>";
...
<body>
	<div>
		...
		echo "<div align='center'><strong> ระหว่างปี  " . ($_REQUEST['i_year1'] + 543) . " และ " . ($_REQUEST['i_year2'] + 543) . " </strong></div>";
			...
			<table id='tb_main' width="100%" class="table_report" style=" width: 2000px; display:none" border="0" cellspacing="1" cellpadding="0">
				<thead valign="top">
					<tr>
						...
						<th style="vertical-align:middle;" rowspan=0 nowrap>&nbsp;ปี&nbsp <?php echo ($_REQUEST['i_year1'] + 543); ?>&nbsp;</th>
						<th style='vertical-align:middle;' rowspan=0 nowrap>&nbsp;ปี&nbsp <?php echo ($_REQUEST['i_year2'] + 543); ?>&nbsp;</th>
...
$legend_data = array(strval($_REQUEST['i_year1'] + 543), strval($_REQUEST['i_year2'] + 543)); // 
$legend_color = array("#516b91", "#FF9E12");
