...
switch ($mode) { 
    ...
    $ss_id1 = $db->Fetch($para1);
        ...
        $code_dc = (string)$c_code;
    
    
//
//        $stmt = $db->QueryParam("EXEC dbo.SP_GEN_CODE_MONTH ?,?,?,?,?;", array($code_dc, YEARBG, $data['dc_user_create_id'], $data['dc_user_create_cost_id'], $ret_id));
...
//        }  
    ...
    // print_r($data);exit();
           ...
           /*     
        if($ss_id){ 
        $stmt = $db->QueryParam("select a.sp_gl_monthly_dtl_id from dbo.sp_gl_monthly_dtl a where sp_gl_monthly_hdr_id =?", array($sp_gl_monthly_hdr_id));
        if ($stmt) { 
            ...
            $data['dc_user_create_cost_id'] = $_SESSION['dc_cost_id'];
            $data['dc_user_create_id'] = $_SESSION['user_id'];
            $ret_id = $row['sp_gl_monthly_dtl_id']; 
            $code_dc = (string) $c_code;
            $sql = "EXEC dbo.SP_GEN_CODE ?,?,?,?,?;";
    
            $stmt = $db->QueryParam($sql, array($code_dc, YEARBG, $data['dc_user_create_id'], $data['dc_user_create_cost_id'], $ret_id));
            $arr_gen_code = $db->Fetch($stmt);
            $c_codex = $arr_gen_code["c_code_gen"];
            $ref_id = $arr_gen_code["reference_id"]; 
                if ($ret_id == $ref_id) {
                  $sql2 = "UPDATE dbo.sp_gl_monthly_dtl"
                                . " SET c_code = '{$c_codex}' "
                                . ", dc_user_update_id =?"
                                . ", dc_user_update_cost_id =?"
                                . ", d_update =?"
