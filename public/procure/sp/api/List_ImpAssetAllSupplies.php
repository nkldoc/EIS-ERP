...
if ($_REQUEST["type"] == "imp_assetall_supplies_hdr") {
	...
	$sqlMain = "
		SET NOCOUNT ON
        SELECT
            ROW_NUMBER() OVER (ORDER BY a.imp_assetall_supplies_hdr_id) AS numrow
            ,a.imp_assetall_supplies_hdr_id
        INTO #TemData
        FROM {$DATABASE_NAME} imp_assetall_supplies_hdr a
        WHERE a.i_enable = 1
            {$con};

        SELECT
            a.numrow
			,b.imp_assetall_supplies_hdr_id
        ...
        FROM #TemData a
		INNER JOIN {$DATABASE_NAME} imp_assetall_supplies_hdr b ON a.imp_assetall_supplies_hdr_id = b.imp_assetall_supplies_hdr_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"                => $row["numrow"],
				"id"                =>	$row["imp_assetall_supplies_hdr_id"],
	...
	exit;
} else if ($_REQUEST["type"] == "imp_assetall_supplies_dtl") {

	$sqlMain = "SET NOCOUNT ON
			SELECT
				ROW_NUMBER() OVER (ORDER BY c_code) AS numrow
				,imp_assetall_supplies_dtl_id
				...
				,from_file --null
			INTO #DATA
			FROM {$DATABASE_NAME} imp_assetall_supplies_dtl
			WHERE imp_assetall_supplies_hdr_id = ?
			...
			SELECT 
				null AS numrow
				,null AS imp_assetall_supplies_dtl_id
	...
	if (sqlsrv_has_rows($stmt)) {
		...
		while ($row = $db->Fetch($stmt)) {
			...
			$temp = array(
				...
				"id"                        => ++$no,
				"i_type"                    => $row["i_type"],
				"imp_assetall_dtl_id"       =>	$row["imp_assetall_supplies_dtl_id"],
				"c_code"                    =>	$row["c_code"],
