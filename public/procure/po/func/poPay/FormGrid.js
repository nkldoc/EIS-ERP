...
Ext.onReady(function () {
  ...
  const search = function () {
    ...
    if (msg == "") {
      if (Ext.getCmp("value-box").getValue() != "") {
        ...
        Ext.store.setBaseParam("date_start", Ext.getCmp("date_start").getValue());
        Ext.store.setBaseParam("date_end", Ext.getCmp("date_end").getValue());
      } else {
  ...
  const gridMain = new Ext.grid.GridPanel({
    ...
    tbar: [
      {
        ...
        items: [
          ...
          },
          {
            ...
            frame: false,
            items: [
              { xtype: "label", text: "ค้นหาตามวันที่ทำทะเบียนจ่าย : " },
              { xtype: "tbspacer", width: 4 },
    ...
    columns: [
      ...
      //       },
      {
        ...
        dataIndex: "id",
        renderer: function (value, metaData, record, row, col, store, gridView) {
          if (Ext.I_STATUS == record.get("i_status_last")) {
            return "<button style='font-size:11px; cursor:pointer; color: blue;'>แสดง</button>";
          }
          if (parseInt(record.get("i_status_last")) == 3 && Ext.I_STATUS != 3) {
          } else if (parseInt(Ext.I_STATUS) == 3) {
  ...
  let myComboStores = [Ext.bg_expense, Ext.dc_expense_budget_type];
