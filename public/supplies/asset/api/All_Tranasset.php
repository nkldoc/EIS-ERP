...
if ($_POST["type"] == "am_asset_hdr") {
	...
	$arrParam      = array();
	$arrCountParam = array();
	$con .= @$_REQUEST['dc_cost_id'] > 0 ? " AND a.dc_cost_id = ". $_REQUEST['dc_cost_id'] : "";
	...
	$sqlTempTable = "select 
					...
					from am_asset_hdr a
					left join am_asset_tf_dtl b on a.am_asset_hdr_id = b.am_asset_hdr_id
	...
	$sqlMain = "
		...
		SELECT 
			...
			,a.dc_cost_id --ส่วนงาน J
			,(select top 1 c_name from NMU_DATACENTER..dc_cost aaa where aaa.dc_cost_id =a.dc_cost_id)   AS segment
	...
	$sqlMain	= "SELECT *,c_code+' : '+c_name as c_full FROM nmu_datacenter..dc_cost ";
	$sqlMain	.= " WHERE i_delete = ?  and i_enable=1  ";
	//$sqlMain	.= " and i_gl_department>1 and i_fund_type not in (4,5,6,7) $ww_i_read ";
	$sqlMain	.= " ORDER BY c_code";
