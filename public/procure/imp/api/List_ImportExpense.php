...
if ($_REQUEST["type"] == "imp_expense_hdr") {
	...
	$sqlMain = "SET NOCOUNT ON
					...
					INTO #TemData
					FROM dbo.imp_expense_hdr a
						LEFT JOIN dbo.gl_tran_hdr b ON a.gl_tran_hdr_id = b.gl_tran_hdr_id
						LEFT JOIN dbo.gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE 1=1 
						{$con}
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"								=> $row["numrow"],
				"id"								=> $row["imp_expense_hdr_id"],
				"c_code"							=> ($row["c_code"] != "") ? $row["c_code"] : "",
				"c_code_bank"						=> ($row["c_code_bank"] != "") ? $row["c_code_bank"] : "",
				"i_post"							=> $row["i_post"],
				"c_expense_vsn_period_no"			=> $row["c_expense_period_no"],
				"c_doc"								=> $row["c_doc"],
	...
	exit;
} else if ($_REQUEST["type"] == "imp_expense_dtl") {

	$sqlMain = "
			SET NOCOUNT ON
			SELECT
				ROW_NUMBER() OVER (ORDER BY a.imp_expense_dtl_id) AS numrow
				,a.imp_expense_dtl_id
				,a.i_cal_gl
				,a.c_booking        
				,a.i_type_year
				,a.c_budget_year
				,a.dc_expense_group_id
				,a.dc_expense_id
				,c.c_name AS dc_expense_group_name
				,b.c_name AS dc_expense_name
				,ISNULL(CASE
					WHEN a.i_type_year = 1 THEN b.c_acc_name
					ELSE b.c_acc_name_overlap
				END,'') AS c_acc_name
				,a.c_approve
				,CONVERT(VARCHAR, a.d_pay, 120) AS d_pay
				,a.f_inv
				,a.f_tax_personal
				,a.f_tax_corporate
				,a.f_social_security
				...
				,STUFF((
					SELECT
						aa.c_cheque+''+'<br/>'
					FROM tb_cheque aa
					WHERE aa.imp_expense_dtl_id = a.imp_expense_dtl_id
...
*/
			...
			FROM dbo.imp_expense_dtl a
				LEFT JOIN (
					...
					FROM dbo.dc_expense aa
				) b ON a.dc_expense_id = b.dc_expense_id
				LEFT JOIN dc_expense_group c ON a.dc_expense_group_id = c.dc_expense_group_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				...
				"dc_expense_group_name"				=> ($row["dc_expense_group_name"] != "") ? $row["dc_expense_group_name"] : "",
				"dc_expense_name"					=> ($row["dc_expense_name"] != "") ? $row["dc_expense_name"] : "",
				"c_acc_name"						=> ($row["c_acc_name"] != "") ? $row["c_acc_name"] : "",
				"c_approve"							=> $row["c_approve"],
				"d_pay"								=> ($row["d_pay"] != "") ? $date->extDateBuddha($row["d_pay"]) : "",
				"d_pay_show"						=> ($row["d_pay"] != "") ? $date->shot_date_from_db($row["d_pay"]) : "",
				"f_inv"								=> $row["f_inv"],
