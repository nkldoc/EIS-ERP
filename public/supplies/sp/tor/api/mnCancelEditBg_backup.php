...
switch ($mode) {
   ...
   case "cancelReserveMoney":
    ...
    $arrParam[] = $_POST["pr_id"];
    $arrParam[] = $_POST["dc_budget_type_id"];
    $arrParam[] = $_POST["bg_expense_id"];
// i_enable	d_create	d_update	dc_cost_acc_id	c_comment dc_budget_type_id	bg_expense_id
    
    $sql = "UPDATE NMU_EIS.dbo.bg_reserve_money
             SET i_enable = 2, 
                c_comment = ?,
                dc_user_update_cost_id = ?,
                d_update = ?
            WHERE pr_id = ? AND  dc_budget_type_id = ? AND  bg_expense_id = ?";
