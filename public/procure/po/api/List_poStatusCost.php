...
include("../../lib/database/DatabaseServerNMU.php");
...
if ($_REQUEST["type"] == "po_working_hdr") {
	...
	// 		default:	$con = "";
	// 	}
	if ($_SESSION["i_type_user"] == 3) {
		$con	.= " AND a.dc_user_create_id = {$_SESSION['user_id']}";
	}
	if (@$_REQUEST["i_cost"] == 1) {
		$con	.= " AND a.dc_user_create_id IN (SELECT dc_user_id FROM dc_user WHERE i_type_user = 3)";
	}
	...
	$sqlMain = "
		SET NOCOUNT ON
		SELECT
			ROW_NUMBER() OVER (ORDER BY (case when b.dc_approve_id > 0 then 1 else 0 end), a.i_status_last, d.d_doc_date DESC, b.i_protest DESC, a.c_code_ref) AS numrow
			,a.po_working_hdr_id
			, a.i_status_last
		INTO #TemData
		FROM dbo.po_working_hdr a
			INNER JOIN dbo.po_working_dtl b ON a.po_working_hdr_id = b.po_working_hdr_id
			INNER JOIN dbo.po_working_item bb ON bb.po_working_hdr_id = a.po_working_hdr_id AND bb.i_status = a.i_status_last AND bb.i_enable = 1
		...
		FROM #TemData a
			...
			LEFT JOIN (
				...
				inner join (
					select po_working_hdr_id, MAX(i_status) as max_status
					from po_working_item
					where i_enable = 1
					group by po_working_hdr_id
				) bb on aa.po_working_hdr_id = bb.po_working_hdr_id and bb.max_status = i_status
			)s2 ON s2.po_working_hdr_id = a.po_working_hdr_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			...
			if ($row["i_status_last"] == 2 && $row["d_status_date"] == "") {
				$c_stats_last = "รอส่งใบเบิก";
			} else if ($row["i_status_last"] == 2 && $row["d_status_date"] != "") {
				$c_stats_last = "รออนุมัติฏีกา";
			} else if ($row["i_status_last"] == 3) {
				$c_stats_last = "รอคืนทักท้วง";
			} else {
				$c_stats_last = "อนุมัติฏีกาเรียบร้อย";
			}
			$temp = array(
				"no"								=> $row["numrow"],
