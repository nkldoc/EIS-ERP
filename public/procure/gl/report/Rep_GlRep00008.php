	...
	while ($data_h = $db->Fetch($stmt_h)) {
		...
		if ($_REQUEST["i_show_month"] == 2) {
			...
			for ($ii = $mm1; $ii <= $mm2; $ii++) {
				...
				if ($ii == $mm1 && $_REQUEST["i_approve"] == 1) {
					$thead2 .= "<th nowrap style='vertical-align:middle; background-color:#ffe8c6;'>จำนวนฎีกา</th>";
					$rowAll++;
					$i++;
				}
				...
				if ($ii == $mm2 && $_REQUEST["i_approve"] == 1) {
					$thead2 .= "<th nowrap style='vertical-align:middle; background-color:#d9f5b9;'>จำนวนฎีกา</th>";
					$rowAll++;
					$i++;
				}
...
function gen_month($style, $param, $em = "td")
{
	...
	for ($vv = 1; $vv <= 2; $vv++) {
		...
		if (@$_REQUEST["i_approve"] == 1) {
			$tt .= "<{$em} " . $style . " nowrap align='center'>" . (@$param[$ind]["f_approve"]) . "</{$em}>";
		}
		for ($ii = $mm1; $ii <= $mm2; $ii++) {
		...
		if ($sum > 0) {
			$tt .= "<{$em} " . $style . " nowrap align='right'>" . number_format($sum, 2) . "</{$em}>";
		} else if ($sum < 0) {
			$tt .= "<{$em} " . $style . " nowrap align='right'>(" . number_format(abs($sum), 2) . ")</{$em}>";
		} else {
			$tt .= "<{$em} " . $style . " align='right' nowrap>-</{$em}>";
		}
	}
	return $tt;
}
...
if (is_array($data_dtl) && count($data_dtl["data"]) > 0) {
	...
	foreach ($data_dtl["data"] as $index => $jObj) {
		...
		if (@$jObj["i_type"] == 1) {
			$tbody .= "<tr class='t1'>";
			$tbody .= "<td nowrap><b style='border-bottom: 3px double #e0e0e0;'>" . $jObj["d_date"] . "</b></td>";
			...
			$nbsp	= "";
			for ($ii = 2; $ii < $jObj["i_type"]; $ii++) {
				$nbsp .= "&nbsp;&nbsp;&nbsp;";
			}
			$tbody .= "<tr class='t2'>";
			$tbody .= "<td nowrap>" . $nbsp . "<b>" . $jObj["c_name"] . "</b></td>";
			for ($ii = 1; $ii <= $rowAll; $ii++) {
				$tbody .= "<td></td>";
			}
			$tbody .= "<td></td>";
			...
			$tbody .= "<tr class='t3'>";
			$tbody .= "<td nowrap>" . $nbsp . $jObj["c_name"] . "</td>";
			...
			if ($vv > 0) {
				$tbody .= "<td nowrap align='right'>" . number_format($vv, 2) . "</td>";
			} else if ($vv < 0) {
				$tbody .= "<td nowrap align='right'>(" . number_format(abs($vv), 2) . ")</td>";
			} else {
			...
			@$jObj["i_type"] == 6 || $jObj["i_type"] == 9 || $jObj["i_type"] == 12 || $jObj["i_type"] == 15
			|| $jObj["i_type"] == 18 || $jObj["i_type"] == 21
		) {
			// หัก ส่งคืน
			$bg	= "";
			if ($jObj["i_type"] == "6") {
				$bg = "background:#FDFE86;";
			} else if ($jObj["i_type"] == "9") {
				$bg = "background:#59CE5D;";
			} else if ($jObj["i_type"] == "12") {
				$bg = "background:#FA7A74;";
			} else if ($jObj["i_type"] == "15") {
				$bg = "background:#D281F1;";
			} else if ($jObj["i_type"] == "18") {
				$bg = "background:#82E1FF;";
			} else if ($jObj["i_type"] == "21") {
				$bg = "background:#EBEBEB;";
			}

			$td	= ($jObj["i_type"] == "6") ? "td" : "th";

			$style	= "style='text-align:right;border-bottom: 1px solid #e0e0e0; $bg'";
			$vv		= 0;

			$tbody .= "<tr class='t2'>";
			$tbody .= "<{$td} nowrap align='right' style='text-align:right;'>" . $jObj["c_name"] . "</{$td}>";
			foreach ($ArrHeadID as $budget_id => $val) {
				$dd = @$jObj["data"]["budget_id"][$budget_id];
			...
			if ($vv > 0) {
				$tbody .= "<{$td} " . $style . " nowrap align='right'>" . number_format($vv, 2) . "</{$td}>";
			} else if ($vv < 0) {
				$tbody .= "<{$td} " . $style . " nowrap align='right'>(" . number_format(abs($vv), 2) . ")</{$td}>";
			} else {
				$tbody .= "<{$td} " . $style . " align='right' nowrap>-</{$td}>";
			}

			$tbody .= "</tr>";
		} else if (
			@$jObj["i_type"] == 7 || @$jObj["i_type"] == 10 || @$jObj["i_type"] == 13
			|| @$jObj["i_type"] == 16 || @$jObj["i_type"] == 19 || @$jObj["i_type"] == 22
		) {

			$bg	= "";
			if ($jObj["i_type"] == "7") {
				$bg = "background:#FBFD2D;";
			} else if ($jObj["i_type"] == "10") {
				$bg = "background:#32E93E;";
			} else if ($jObj["i_type"] == "13") {
				$bg = "background:#F84F46;";
			} else if ($jObj["i_type"] == "16") {
				$bg = "background:#C648F7;";
			} else if ($jObj["i_type"] == "19") {
				$bg = "background:#64DAFF;";
			} else if ($jObj["i_type"] == "22") {
				$bg = "background:#C3C3C3;";
			}
			...
			$tbody .= "<tr class='t2'>";
			$tbody .= "<{$td} style='text-align: right; border-bottom: 1px solid #e0e0e0;' nowrap align='right'>" . $jObj["c_name"] . "</{$td}>";
			foreach ($ArrHeadID as $budget_id => $val) {
				$dd = @$jObj["data"]["budget_id"][$budget_id];
				$tbody .= gen_month($style, $dd, $td);
				$vv += sum_row($dd);
			...
			@$jObj["i_type"] == 8 || @$jObj["i_type"] == 11 || @$jObj["i_type"] == 14
			...
			$style	= "style='text-align: right; {$bg}'";
			$vv		= 0;
			...
			if ($vv > 0) {
				$tbody .= "<th " . $style . " nowrap align='right'>" . number_format($vv, 2) . "</th>";
			} else if ($vv < 0) {
				$tbody .= "<th " . $style . " nowrap align='right'>(" . number_format(abs($vv), 2) . ")</th>";
			} else {
				$tbody .= "<th " . $style . " align='right' nowrap>-</th>";
			}
...
<style>
	.table_report tbody .t1 td,
	.table_report tbody .t2 td,
	.table_report tbody .t2 th {
		border: 0px solid;
		border-left: 1px solid #e0e0e0;
		border-right: 1px solid #e0e0e0;
	}

	.table_report tbody .t3 td {
		border: 0px solid;
		border-left: 1px solid #e0e0e0;
		border-right: 1px solid #e0e0e0;
