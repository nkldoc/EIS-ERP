...
switch ($mode) {
	...
	case "EDIT":
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				SET NOCOUNT ON
				INSERT INTO po_budget_income_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE po_budget_income_hdr SET " . substr($addField, 1) . " WHERE po_budget_income_hdr_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
	...
	case "SAVE_DTL":
		...
		if ($msg == "") {
			foreach ($Arr as $fldd) {
				...
				if ($fldd["po_budget_income_dtl_id"] > 0) { // EDIT
					...
					$arrValue[] = $fldd["po_budget_income_dtl_id"];
					$sql		= "UPDATE po_budget_income_dtl SET " . substr($addField, 1) . " WHERE po_budget_income_dtl_id = ?";
					$db->QueryParam($sql, $arrValue);
				} else { // ADD

					$data["po_budget_income_hdr_id"]						= $_REQUEST["id"];
					$data["dc_user_create_id"]								= $_SESSION["user_id"];
					$data["dc_user_create_cost_id"]							= $_SESSION["dc_cost_id"];
					$data["d_create"]										= date("Y-m-d H:i:s");
					...
					$sql = "
						SET NOCOUNT ON
						INSERT INTO po_budget_income_dtl (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
						SELECT @@IDENTITY as id;";
