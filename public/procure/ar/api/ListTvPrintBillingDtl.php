...
###################
 ...
 { 
	...
	$sqlMain	= "select b.f_quan as f_quan  
		...
		,(select convert(varchar, d_end_credit, 120) from ar_bill_invoice_hdr where ar_bill_invoice_hdr_id=a.ar_bill_invoice_hdr_id) as d_end_credit
...
//
    ...
    /*             $billing = $so->dtlBilling($row["{$table}_id"]);
		...
		//==================
		$due = "ชำระที่ฝ่ายการเงิน บมจ. อสมท ภายในวันที่  ".$date->long_date_from_db($row["d_end_credit"]);
	...
	if($typePrint=="noneGroup"){
		...
		${$root}[] = array("no" => ($i++), 
						"id" 		=> 'grandTotal0', 
						"c_name" 	=> ""	 
                ."<input type='text' style='font-size:12px;width:90%;margin:1px;' name='itemsSum' value='".$c_product ."'>"
                ."<br/>"
                ."<input type='text' style='font-size:12px;width:90%;margin:0px;' name='c_comment' value=''>",
