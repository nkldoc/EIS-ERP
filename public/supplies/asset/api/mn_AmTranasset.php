...
switch ($mode) {
	...
	case "EDIT":
		...
		$data["dc_cost_id_new"]		  = $_REQUEST["dc_cost_id_new"];
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				...
				SET NOCOUNT ON
				INSERT INTO {$DATABASE_NAME} am_asset_tf_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
...
// /********************/
			...
			$sql		= "UPDATE {$DATABASE_NAME} am_asset_tf_hdr SET " . substr($addField, 1) . " WHERE am_asset_tf_hdr_id = ?";
