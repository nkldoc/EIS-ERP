...
if ($_REQUEST['type'] == 'storeSpEmp') {
    ...
    $sqlTempTable = "select b.c_name as c_department
		 ...
		 , ROW_NUMBER() OVER (ORDER BY a.dc_department_id asc) as row
                ...
                where b.dc_department_id =? "; //  and a.i_level=3 and b.dc_department_id = " . $_SESSION['dc_department_id'];
    ...
    $sqlTempTable = "select b.c_name as c_department
		 ...
		 , ROW_NUMBER() OVER (ORDER BY a.dc_department_id asc) as row
                from dbo.sp_emp a
                  inner join sp_department b on b.dc_department_id = a.dc_department_id
                where a.i_level=2
                and a.dc_department_id  in  (2,3,6)  
                and a.i_enable=?";
...
//    print_r($_SESSION);
    ...
    //         . " where dc_cost_id=?  and d_end_date  >= '" . date('Y-m-d') ."'"  ;
        ...
        SELECT 
         ...
         ,a.dc_cost_id
         ,(SELECT TOP 1 c_name FROM NMU.dbo.dc_cost aa WHERE aa.dc_cost_id = a.dc_cost_id) AS dc_costTxt
         ,b.c_code
         ,b.c_name
         ,a.f_total AS f_overlap
         ,i_year
         ,(SELECT TOP 1  ISNULL(SUM(ISNULL(f_amt,0.00)),0.00)FROM NMU.dbo.vw_bg_reserve_overlap aa WHERE aa.i_reserve = 2 AND a.c_code_ref = aa.c_code_overlap) AS f_overlap_reserve
         ,(SELECT TOP 1  ISNULL(SUM(ISNULL(f_amt,0.00)),0.00)FROM NMU.dbo.vw_bg_reserve_overlap aa WHERE aa.i_reserve = 3 AND ISNULL(aa.i_finish,0) = 0 AND a.c_code_ref = aa.c_code_overlap) AS f_overlap_reserve_income
         ,(SELECT TOP 1  ISNULL(SUM(ISNULL(f_amt,0.00)),0.00)FROM NMU.dbo.vw_bg_reserve_overlap aa WHERE aa.i_reserve = 3 AND ISNULL(aa.i_finish,0) = 1 AND a.c_code_ref = aa.c_code_overlap) AS f_overlap_reserve_finish
         ,ISNULL((SELECT TOP 1 aa.f_total FROM #tmp_working aa WHERE aa.c_booking = a.c_code_ref),0) AS f_working
         ,a.d_end_date
        FROM NMU.dbo.vw_bg_budget_overlap a
        LEFT JOIN NMU.dbo.bg_expense b ON a.bg_expense_id = b.bg_expense_id
    ...
    if ($mode == "SEARCH") {
        ...
        LEFT JOIN (SELECT po_working_hdr_id ,SUM(f_return) AS f_return FROM NMU.dbo.po_return WHERE i_enable = 1 
