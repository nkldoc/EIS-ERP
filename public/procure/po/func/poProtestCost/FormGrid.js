...
Ext.onReady(function () {
  ...
  const controllTab = function (record, butt) {
    ...
    if (butt == "edit" || butt == "view") {
      ...
      Ext.getCmp("d_receive_date").setReadOnly(true);
  ...
  const gridMain = new Ext.grid.GridPanel({
    region: "center",
    layout: "fit",
    title: "แสดงรายการรับทักท้วง (หน่วยงาน)",
    id: "tabpanel1",
    border: false,
    stripeRows: true,
    loadMask: true,
      ...
      //       },
      {
        ...
        renderer: function (value, metaData, record, row, col, store, gridView) {
          ...
          if (parseInt(record.get("i_status_last")) == 3 && Ext.I_STATUS != 3) {
            ...
            return "<button style='font-size:11px; cursor:pointer; color: green;'>ส่งทักท้วง</button>";
          } else if (parseInt(record.get("i_status_edit")) != 1) {
