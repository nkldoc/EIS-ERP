Ext.onReady(function() {
  ...
  /* =============================================== */
  title_panel = "แยกรายละเอียดใบเบิก (Vision Net)";
  /* =============================================== */
  ...
  store = new Ext.data.JsonStore({
    ...
    autoDestroy: true,
    autoLoad: false,
    url: "api/List_ImpRequestItemVSN.php",
    baseParams: { type: "imp_request_vsn_dtl", i_read: user_right_read },  
    root: "data",
    idProperty: "id",
  ...
  Ext.store_dc_acc_last = new Ext.data.JsonStore({
    autoLoad: true, 
    url: "api/All_ImpRequestItemVSN.php",
        ...
        buttons: [
          {
            ...
            handler: function() {
              ...
              Ext.Ajax.request({
                url: "api/mn_ImpRequestItemVSN.php",
                method: "POST",
                params: {
                  mode: "DELETE",
                  imp_request_vsn_dtl_id: record.data.id
                },
    ...
    SaveDtl = function() {
      ...
      $("input[id^=no]").each(function(i, val) {
        ...
        jsonArr.push({
          ...
          c_acc_code_imp: Ext.getCmp("c_acc_code_imp[" + index + "]").getValue(),
          c_acc_name_imp: Ext.getCmp("c_acc_name_imp[" + index + "]").getValue() 
        });
      });
      ...
      if (msg == "") {
        ...
        $.ajax({
          ...
          data: {
            mode: "ADD_ITEM",
            imp_request_vsn_hdr_id: vv.data.hdr_id,
            imp_request_vsn_dtl_id: vv.data.id,
              ...
              if (obj.debug == true) {
                $.each(obj.data, function(index, v) {
                  ...
                  if (vv.data.c_gx_code != "") {
                    ...
                    addBody += "<input id='no[" + index + "]' type='hidden' value='" + index + "'>";
                    addBody += "<input id='item_id[" + index + "]' type='hidden' value='" + obj.data.imp_request_vsn_item_id + "'>";
      ...
      tbar: [
        ...
        { xtype: "tbfill" },
        {
          ...
          html: '<div style="font-weight:bold; background:#f5f5f5; padding:5px;">' 
                  + "<div style='font-size: 15px; text-align: left;'>เลขที่ใบเบิก : " + vv.data.c_request 
                  + "<br>เลขที่ตั้งหนี้ :: " + vv.data.c_request_desc 
                  + "<br>รายการ ::: " + vv.data.c_comment + "</div>" 
                + "</div>"
        }
      ],
      html:
        ...
        "<td width=100 nowrap>จำนวนเงิน DR</td>" +
        "<td width=100 nowrap>จำนวนเงิน CR</td>" + 
        "<td nowrap>รหัสบัญชีที่นำเข้า</td>" +        
        "<td nowrap>ชื่อบัญชีที่นำเข้า (MIS/Vision Net)</td>" +
  ...
  gridMain = new Ext.grid.GridPanel({
    ...
    columns: [
      ...
      }),
      {
        ...
        dataIndex: "id",
        renderer: function(value, metaData, record, row, col, store, gridView) {
          if ((record.data.i_status<3) && (record.data.i_group_show=="0")) {
