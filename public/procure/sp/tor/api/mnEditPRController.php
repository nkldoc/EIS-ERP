      ...
      , f.po_expense_id
       ...
       -- , isnull((select top 1  imp_assetall_supplies_hdr_id from NMU_ASSET.dbo.imp_assetall_supplies_hdr where sp_check_period_hdr_id = a.sp_check_period_hdr_id  and   i_enable = 1),0) as imp_assetall_supplies_hdr_id
        ...
        from ({$sqlTempTable}) a "
                ...
                . " left join " . DB_CENTER . "dc_map_send_ap dc on  dc.sp_check_period_hdr_id = c.sp_check_period_hdr_id and dc.i_enable = 1 and dc.i_sys = 3 "
...
";
    ...
    case "CANCEL_RESERVE_PR1":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 1 AND i_sys = 3;
    ...
    case "CANCEL_RESERVE_PO1":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_contract_id'])) {
            ...
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 2 AND i_sys = 3;
