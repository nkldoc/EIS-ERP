...
function List_QueryParam() {
	...
	switch ($_REQUEST["i_show_acc"]) {
		...
		default		:	//บัญชีคุม Level 2/3/4/5
			...
			$ww_acc_show 			= " WHERE i_group IN (".GL_ACC_GROUP4_REVENUE.",".GL_ACC_GROUP5_EXPENSE.") AND i_level IN (".GL_ACC_1ST_LEVEL.",".$_REQUEST["i_show_acc"].",".GL_ACC_6TH_LEVEL.") AND i_enable = ".STATUS_ENABLE." AND i_delete = ".DELETE_FALSE."; ";
			$ww_report_show 		= " SELECT
											a.dc_acc_id
											,a.c_code
											,a.c_name
											,CASE 
												WHEN (a.i_group=".GL_ACC_GROUP4_REVENUE.") THEN ISNULL((SELECT SUM(b.f_cr) FROM @Gl_money b WHERE b.c_sub=a.c_sub GROUP BY b.c_sub),0) - ISNULL((SELECT SUM(b.f_dr) FROM @Gl_money b WHERE b.c_sub=a.c_sub GROUP BY b.c_sub),0)
												WHEN (a.i_group=".GL_ACC_GROUP5_EXPENSE.") THEN ISNULL((SELECT SUM(b.f_dr) FROM @Gl_money b WHERE b.c_sub=a.c_sub GROUP BY b.c_sub),0) - ISNULL((SELECT SUM(b.f_cr) FROM @Gl_money b WHERE b.c_sub=a.c_sub GROUP BY b.c_sub),0)
												ELSE 0
											END AS f_money
											,a.i_group
											,".$CFG_ROW_DATA." AS i_type
