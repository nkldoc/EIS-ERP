...
if ($_REQUEST["type"] == "po_working_dtl") {
    ...
    $sqlTempTable = "select a.po_working_hdr_id
                        ...
                        where a.i_import=2 and" . $keyin . " aa.i_success != 1 and  a.i_enable = ? " . $conDtl . $con;
    ...
    $sqlMain = "select a.* "
        ...
        . " inner join dbo.po_working_item s2 on s2.po_working_hdr_id = a.po_working_hdr_id and s2.i_status = a.i_status_last and s2.i_enable = 1"
        . " WHERE s.i_success != 1 AND a.row > ? and a.row <= ? ORDER BY a.row";
    /* echo $sqlMain ;
    print_r ( $arrParam ) ;
    exit () ;*/
