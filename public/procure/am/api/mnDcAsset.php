...
switch ($mode) {
    case "AddChild" : // เพิ่มเมนูย่อย
        $sql = "SELECT c_code_tree FROM {$table} WHERE dc_cost_id = ? and i_delete <> 1";
        ...
        $sqlAddChild = "DECLARE @maxLenght as int;
                        ...
                        IF @parentIsLast = 1 /*Parent Node เป็นระดับล่างสุด*/
                            BEGIN
                                UPDATE dbo.am_tree SET c_code_tree = left(c_code_tree + @tempCode, @maxLenght);
                            END

                        UPDATE dbo.am_tree SET i_last = 0 WHERE dc_cost_id = @dc_cost_parent_id;
				
                        INSERT INTO dbo.am_tree (c_code_tree, dc_area_id, dc_cost_parent_id, dc_cost_acc_id
    ...
    case "AddBefore" : // เพิ่มก่อนหน้ารายการที่เลือก
        $sql = "SELECT c_code_tree, dc_cost_parent_id, i_last FROM {$table} WHERE dc_cost_id = ?  and i_delete <> 1";
        ...
        $sqlAddBefore = "DECLARE @maxLenght AS INT;
                        ...
                        INSERT INTO dbo.am_tree (c_code_tree, dc_area_id
                                        , c_code, c_name, c_address
...
//         exit;
        ...
        $sqlDel = " DECLARE @maxLenght AS INT;
                    ...
                    UPDATE dbo.am_tree SET i_delete = @i_delete, dc_user_update_id = @user_id, dc_user_update_cost_id = @user_cost_id, d_update = GETDATE() WHERE {$keyName} = @ref_id;";
    ...
    case "Move" : // ย้ายรายการที่เลืก
        ...
        switch($i_move)
        {
            case "Before" : // ย้ายไปไว้ก่อนหน้ารายการที่เลือก
                $sql = "SELECT c_code_tree, dc_cost_parent_id, i_level FROM {$table} WHERE dc_cost_id = ? and i_delete <> 1";
                ...
                $sql = "SELECT c_code_tree, dc_cost_parent_id FROM {$table} WHERE dc_cost_id = ? and i_delete <> 1";
                ...
                $sqlAddBefore = "   DECLARE @maxLenght AS INT;
                                    ...
                                    SELECT @targetCode = LEFT(c_code_tree,@leftTargetLenght), @targetLv = i_level FROM dbo.am_tree WHERE dc_cost_id = @targetID WHERE i_delete <> 1;
                                    ...
                                    SELECT @maxLenght= MAX(LEN(c_code_tree)) FROM dbo.am_tree WHERE i_delete <> 1;
            ...
            case "After" : // ย้ายไปไว้ต่อท้ายรายการที่เลือก
                ...
                $sqlAddAfter = "DECLARE @maxLenght AS INT;
                                ...
                                DECLARE @targetParentCode AS VARCHAR(250);
                                DECLARE @targetLv AS INT;
                                SELECT @targetCode = LEFT(c_code_tree,@leftTargetLenght), @targetLv = i_level FROM dbo.am_tree WHERE dc_cost_id = @targetID and i_delete <> 1;
                                SET @targetParentCode = LEFT(@targetCode, (LEN(@targetCode)-2));
