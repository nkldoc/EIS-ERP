...
function controllTab(record,butt){ 
    ...
    if(butt=='add'){  
        ...
        //Ext.getCmp('frm-i_is_type').fn(null);
...
formAdd	 = function() {
        formAdd.superclass.constructor.call(this, {
            ...
            items:[{
                    ...
                    columns: [230,230],
                    items: [
                        { boxLabel: 'มีรายการภาษีเงินได้สำหรับจัดซื้อจัดจ้าง', checked: true, name: 'i_is_type', inputValue: Ext.DC_TAX_IS_INCOME },
                        { boxLabel: 'ไม่มีรายการภาษีเงินได้สำหรับจัดซื้อจัดจ้าง ', name: 'i_is_type', inputValue: Ext.DC_TAX_IS_INCOME_NONE }
                    ],listeners: {
                        change : function(obj, value){   
                            if(value.inputValue==Ext.DC_TAX_IS_INCOME_NONE){
                                Ext.getCmp('frm-dc_tax_income_id').disable();
                                Ext.getCmp('frm-dc_tax_income_id').label.update('');
                            }else{  
                                Ext.getCmp('frm-dc_tax_income_id').enable(); 
                                Ext.getCmp('frm-dc_tax_income_id').label.update('รายการภาษีเงินไดั ');
                            }   
                        }
                    }
                },{
                    xtype: 'combo',
                    fieldLabel: 'รายการภาษีเงินไดั ',
                    id: 'frm-dc_tax_income_id',
                    store: Ext.storeDcTaxIncomeParent,
                    valueField: 'id',
                    displayField: 'c_name',
                    submitValue : true,
                    hiddenName : 'dc_tax_income_id',
                    mode: "local",
                    triggerAction: "all",
                   ...
                   items: [{
                            fieldLabel: 'สำหรับบุคคลธรรมดา',
                            boxLabel: 'เป็นรายการลดหย่อนภาษีสำหรับบุคคลธรรมดา', 
                            inputValue: '1',
                            uncheckedValue: '0',
                            name: 'i_dec_person'
                        } , {
                    ...
                    fieldLabel: 'การคิดภาษีหัก ณ ที่จ่าย <br />(ระบบเจ้าหนี้/บริหารการเงิน ตรวจจ่าย)',
                    xtype: 'radiogroup',
                    columns: [80,100,100],
                    items: [
                            { boxLabel: 'ยังไม่กำหนด', checked: true, name: 'i_type_tax', inputValue: Ext.TAX_NOT },
                            { boxLabel: 'นิติบุคคล', name: 'i_type_tax', inputValue: Ext.TAX_JURISTIC_PERSON },
                            { boxLabel: 'บุคคลธรรมดา', name: 'i_type_tax', inputValue: Ext.TAX_NORMAL_PERSON }
                            ]
                }, { 
...
Ext.storeDcTaxIncomeParent = new Ext.data.JsonStore({
    //autoDestroy: true,
    autoLoad: true,
    url: 'api/All_TaxCombo.php',
    baseParams: {type : 'storeDcTaxIncomeParent', i_all:1},
    root: 'data',
    idProperty: 'id',
    fields: [ 'id', 'c_name']
});
...
Ext.QuickTips.init();
    var gridMain = {
        ...
        { id: 'c_name', header: "ประเภทกิจการ", width:210, sortable: true, dataIndex: 'c_name' }, 
        { header: "หมายเหตุ", width:100, sortable: true, dataIndex: 'c_comment' }, 
        { header: "ภาษีเงินได้สำหรับจัดซื้อจัดจ้าง", width:180, sortable: true, dataIndex: 'c_type', align:'center'}, 
        { header: "การคิดภาษีหัก ณ ที่จ่าย", width:120, sortable: true, dataIndex: 'c_type_tax', align:'center' }, 
        {
            header: "ลดหย่อนภาษีแก่บุคคลธรรม", width:160,sortable: true,
            dataIndex: 'i_dec_person',
            align: 'center',
            renderer: function(value, metaData, record, row, col, store, gridView){ 
                return (value==1) ?'<img src="../images/icons/yes.gif");/>':'<img src="../images/icons/no.gif");/>'; 
            }
        },
        {
            header: "Status",  
