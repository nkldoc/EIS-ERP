...
{ 
        $data['d_contract_date'] 	= (isset($data['d_contract_date']) && $data['d_contract_date']!='')?$date->bc_to_ad($data['d_contract_date']):null;
...
*/		
	...
	break;
		...
		foreach($fld as $value)
		{
			...
			{ 
				...
				$upField .= ", {$value} = ?";
				$arrParam[] = 0; 
            }else if($value=='c_contract_no' 
			|| $value=='d_contract_date'
			|| $value=='c_comment'){
				$upField .= ", {$value} = ?";
				$arrParam[] = null; 
			}
	...
	if($data["statusBu"]=='del'){
		...
		$sqlM = "select * from gl_dc_period where c_mm='{$c_mm}' and c_yyyy='{$c_yyyy}' and i_system=3 and i_last_period=1 and i_status=?";			
		$fm = $db->GetDataBySQL($sqlM, array(2)); //check Close 2 check Open 1;	
		...
		if($fm['i_status']==2) //
		{
			...
			$mth = $date->l_month_thai["{$c_mm}"];
			$yth = ($c_yyyy/1)+543;
			$msg = "ไม่สามารถยกเลิก Invoice ทางบัญชีได้ปิดการวางบิล ไม่สามารถยกเลิกหรือแก้ไขได้ภายใน  เดือน {$mth} ปี พ.ศ. {$yth}";
		}
