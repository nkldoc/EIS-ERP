...
if( $_REQUEST["type"] == "imp_expense_approve" ) {
	...
	$sqlMain = "SET NOCOUNT ON
				SELECT
					...
					,aa.gl_tran_hdr_rq_id as gl_tran_hdr_id
					,bb.c_request
					,bb.c_request_desc 
					...
					,bb.i_status
					,ISNULL(aa.gl_tran_hdr_rq_id,cc.gl_tran_hdr_id_source) as gl_tran_hdr_id_source
					,cc.gl_tran_hdr_id_cancel 
					,NULL as c_code_source 
					...
					,bb.c_acc_code+' '+bb.c_acc_name as c_acc_dr_full
					,bb.c_acc_code2+' '+bb.c_acc_name2 as c_acc_cr_full
				INTO #TemData
				FROM  ".$tb_hdr_name." aa
				...
				WHERE aa.i_status=2 and aa.i_enable=1 and left(aa.c_code,1)='I' and aa.i_type_request=2
	...
	if( sqlsrv_has_rows( $stmt ) ) {
		while( $row = $db->Fetch( $stmt ) ) {
			$temp = array(	"no"								=> $row["numrow"],
							...
							"c_acc_dr_full"						=> $row["c_acc_dr_full"],
							"c_acc_cr_full"						=> $row["c_acc_cr_full"]
			);
