Ext.onReady(function () {
  ...
  Ext.dc_user = new Ext.data.JsonStore({
    ...
    listeners: {
      load: function (t, records, options) {
        Ext.getCmp("dc_approve_id").setValue("0");
      },
    },
  });
  ...
  Ext.po_parcel_officer = new Ext.data.JsonStore({
    ...
    autoLoad: true,
    url: "api/All_RepStatisticPersonnel.php",
    baseParams: { type: "po_parcel_officer", all: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name"],
    listeners: {
      load: function (t, records, options) {
        Ext.getCmp("po_parcel_officer_id").setValue("0");
      },
    },
  ...
  Ext.bg_expense = new Ext.data.JsonStore({
    ...
    autoLoad: true,
    url: "api/All_RepStatisticPersonnel.php",
    baseParams: { type: "bg_expense", all: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name"],
    listeners: {
      load: function (t, records, options) {
        Ext.getCmp("bg_expense_id").setValue("0");
      },
    },
  });
  ...
  LookReport = function (type) {
    ...
    if (msg == "") {
      ...
      resultUrl += "&bg_expense_id=" + Ext.getCmp("bg_expense_id").getValue();
      resultUrl += "&po_creditor_id=" + Ext.getCmp("po_creditor_id").getValue();
      resultUrl += "&dc_approve_id=" + Ext.getCmp("dc_approve_id").getValue();
      resultUrl += "&po_parcel_officer_id=" + Ext.getCmp("po_parcel_officer_id").getValue();
