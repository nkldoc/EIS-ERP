...
if ($_REQUEST["type"] == "dc_cost") {
	...
	$sqlMain	= "SELECT * FROM NMU.dbo.bg_expense WHERE i_last = 1 and i_enable = ? {$con} ORDER BY c_code_tree";
	$arrParam	= array(STATUS_ENABLE);
	$stmt = $db->QueryParam($sqlMain, $arrParam);
	if ($stmt) {
	...
	$sqlMain = "
		...
		FROM NMU.dbo.bg_expense a
			INNER JOIN NMU.dbo.bg_expense b ON LEFT(a.c_code_tree,2) = LEFT(b.c_code_tree,2) AND b.i_enable = 1 AND b.i_level = 1
	...
	$sqlMain = "
		select top 1
			...
			,a.dc_tax_customer_id
			,isnull((select top 1 c_name from ".DB_NMU."dc_tax_customer aa where aa.dc_tax_customer_id = a.dc_tax_customer_id),'') as c_name_tax_customer
			,isnull((select top 1 (select top 1 c_name from ".DB_NMU."dc_tax_income aaa where aaa.dc_tax_income_id = aa.dc_tax_income_id) from ".DB_NMU."dc_tax_customer aa where aa.dc_tax_customer_id = a.dc_tax_customer_id),'')as c_name_tax_income
			,isnull(a.tax_c_title,'') as tax_c_title
			,isnull(a.tax_c_name,'') as tax_c_name
			,isnull(a.c_name,'') as c_name
		...
		left join ".DB_NMU."dc_tambon b on a.dc_tambon_id = b.dc_tambon_id
