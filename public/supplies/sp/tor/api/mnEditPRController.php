...
";
    ...
    case "updateBillingDate":
        ...
        if ($_REQUEST && !empty($_REQUEST['sp_tor_hdr_period_id'])) {
            ...
            if (!empty($_REQUEST["d_doc_arrive_dt"])) {
                ...
                // อัพเดท sp_tor_contract (booking type)
                $sql1 = "UPDATE dbo.sp_check_period_hdr SET d_doc_arrive_dt = ? "
                    . " , dc_user_update_id =? "
                    . " , dc_user_update_cost_id =? "
                    . " , d_update = ?"
                    . " WHERE sp_tor_hdr_period_id = ?";
    ...
    case "UPDATE_RESERVE_PR1":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {

            $id = $_REQUEST['tor_id'];
            $i_pr_types = $_REQUEST["i_pr_type"];
            $dc_cost_id = $_REQUEST['dc_cost_id'];
            $dc_budget_type_id = $_REQUEST['dc_budget_type_id'];
            $bg_expense_id = $_REQUEST['bg_expense_id'];
            $f_amt = str_replace(',', '', $_REQUEST['f_amt']);
            $dc_cost_acc_id = $db->GetDataBySQL('SELECT dc_cost_acc_id FROM NMU_DATACENTER..dc_cost WHERE dc_cost_id = ?', array($dc_cost_id));
            $array = json_decode($i_pr_types);
            $val = intval($array[0]);

            $sql1 = "
                    INSERT INTO NMU_EIS..bg_reserve_money (
                        i_sys
                        ,pr_id
                        ,i_year
                        ,i_pr_type
                ...
                DECLARE @NewReserveID BIGINT = @@IDENTITY;

                UPDATE sp_tor SET bg_reserve_money1_id = @NewReserveID , 
                dc_expense_budget_type_id = " . $dc_budget_type_id . "
                , i_pr_type1 = " . $i_pr_types . "
                , f_type_amt = " . $f_amt . "
                
                WHERE tor_id = ?;
            ";

            $arrValue1 = array(1, $id, 2026, $i_pr_types, 1, $dc_cost_id, $dc_budget_type_id, $bg_expense_id, 0, 1, $f_amt, 1, date("Y-m-d H:i:s"), date("Y-m-d H:i:s"), $dc_cost_acc_id, $id);
            //   echo $sql1; print_r($arrValue1);
            $stmt1 = $db->QueryParam($sql1, $arrValue1);
        }
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            $sql1 = "
                ...
                DECLARE @NewReserveID BIGINT = @@IDENTITY;
                                ...
                                UPDATE sp_tor SET bg_reserve_money2_id = @NewReserveID , 
                dc_expense_budget_type2_id = " . $dc_budget_type_id . "
                , i_pr_type2 = " . $i_pr_types . "
                , f_type2_amt = " . $f_amt . "
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            $sql1 = "
                ...
                DECLARE @NewReserveID BIGINT = @@IDENTITY;
                                ...
                                UPDATE sp_tor SET bg_reserve_money3_id = @NewReserveID , 
                dc_expense_budget_type3_id = " . $dc_budget_type_id . "
                , i_pr_type3 = " . $i_pr_types . "
                , f_type3_amt = " . $f_amt . "
    ...
    case "CANCEL_RESERVE_PR1":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            $id = $_REQUEST['tor_id'];
            $bg_reserve_id = $_REQUEST["bg_reserve_id"];
            $array = json_decode($i_pr_types);
            $val = intval($array[0]);
            ...
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 1 AND i_sys = 1; 
                    UPDATE sp_tor SET bg_reserve_money1_id = 0 WHERE tor_id = ?;
            ";

            $arrValue1 = array($bg_reserve_id,  $id, $id);
            //           echo $sql1; print_r($arrValue1);
            $stmt1 = $db->QueryParam($sql1, $arrValue1);
        }
    ...
    case "CANCEL_RESERVE_PR2":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 1 AND i_sys = 1; 
                    UPDATE sp_tor SET bg_reserve_money2_id = 0 WHERE tor_id = ?;
            ";
    ...
    case "CANCEL_RESERVE_PR3":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_id'])) {
            ...
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 1 AND i_sys = 1; 
                    UPDATE sp_tor SET bg_reserve_money3_id = 0 WHERE tor_id = ?;
            ";
        ...
        $ret_id = $id;
        break;
    case "UPDATE_RESERVE_PO1":
    case "UPDATE_RESERVE_PO2":
    case "UPDATE_RESERVE_PO3":
        if (empty($_SESSION['user_id'])) {
            echo json_encode(array("success" => false, "msg" => "Session หมดอายุ"));
            exit();
        }
        ...
        if ($mode == "UPDATE_RESERVE_PR1") {
            $col_budget = "dc_expense_budget_type_id";
            $col_expense = "bg_expense_bg_id";
            $col_reserve = "bg_reserve_money1_id"; // Assuming 1 is PR1
        } else if ($mode == "UPDATE_RESERVE_PO1") {
            $col_budget = "dc_expense_budget_type_id_po";
            $col_expense = "bg_expense_bg_id2";
            $col_reserve = "bg_reserve_money2_id"; // Wait, check col names
        } else if ($mode == "UPDATE_RESERVE_PR2") {
            $col_budget = "dc_expense_budget_type2_id";
            $col_expense = "bg_expense_id2_pr";
            $col_reserve = "bg_reserve_money2_id"; // This is confusing, might need to check DB
        }

        // Actually, to be safe and avoid guessing wrong column names if I can't see the schema,
        // I will use the names found in FormAdd.js 'rec.get()' calls.

        $mapping = array(
            "UPDATE_RESERVE_PR1" => array("bt" => "dc_expense_budget_type_id", "ex" => "bg_expense_bg_id"),
            "UPDATE_RESERVE_PO1" => array("bt" => "dc_expense_budget_type_id_po", "ex" => "bg_expense_bg_id2"),
            "UPDATE_RESERVE_PR2" => array("bt" => "dc_expense_budget_type2_id", "ex" => "bg_expense_id2_pr"),
            "UPDATE_RESERVE_PO2" => array("bt" => "dc_expense_budget_type2_id_po", "ex" => "bg_expense_id2_po"),
            "UPDATE_RESERVE_PR3" => array("bt" => "dc_expense_budget_type3_id", "ex" => "bg_expense_id3_pr"),
            "UPDATE_RESERVE_PO3" => array("bt" => "dc_expense_budget_type3_id_po", "ex" => "bg_expense_id3_po")
        );

        $cols = $mapping[$mode];
        $sql = "UPDATE sp_tor SET " . $cols['bt'] . " = ?, " . $cols['ex'] . " = ?, dc_user_update_id = ?, dc_user_update_cost_id = ?, d_update = ? WHERE tor_id = ?";
        $params = array($dc_budget_type_id, $bg_expense_id, $_SESSION['user_id'], $_SESSION['dc_cost_id'], date('Y-m-d H:i:s'), $tor_id);
    ...
    case "CANCEL_RESERVE_PO1":
        ...
        if ($_REQUEST && !empty($_REQUEST['tor_contract_id'])) {

            $id = $_REQUEST['tor_contract_id'];
            $bg_reserve_id = $_REQUEST["bg_reserve_id"];
            $array = json_decode($i_pr_types);
            $val = intval($array[0]);
            ...
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "
                    UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ? AND i_reserve = 2 AND i_sys = 1; 
                    UPDATE sp_tor_contract SET bg_reserve_money1_id = 0 WHERE sp_tor_contract_id = ?;
            ";
    ...
    case "CANCELBOOKING1":
        ...
        if ($_REQUEST && !empty($_REQUEST['bg_reserve_money_id_pr'])) {

            $id = $_REQUEST['tor_id'];
            $val = $_REQUEST["bg_reserve_money_id_pr"];
            $array = json_decode($i_pr_types);
            $val = intval($array[0]);
            ...
            // อัพเดท sp_tor_contract (booking type)
            $sql1 = "
                UPDATE NMU_EIS..bg_reserve_money SET i_enable = 2 WHERE bg_reserve_money_id = ? AND pr_id = ?;

                UPDATE sp_tor SET bg_reserve_money1_id = ? WHERE tor_id = ?;
            ";

            $arrValue1 = array($val, $id, 0, $id);
            //           echo $sql1; print_r($arrValue1);
            $stmt1 = $db->QueryParam($sql1, $arrValue1);
        }
