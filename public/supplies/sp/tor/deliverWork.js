                                                                        ...
                                                                        {header: "ออกเลขรับของ", align: "left", width: 110, dataIndex: "c_arrive_code", id: "genCodeArrivalID",
                                                                            renderer: function (value, metaData, record, row, col, store, gridView) {
                                                                                if (record.data.i_status_checking != 2) {
                                                                                    metaData.attr = "style='cursor:pointer; text-align:center;';";
                                                                                    if (record.get('c_arrive_code') === "" || record.get('c_arrive_code') === null) {
                                                                                        return '<button ' + Ext.styleBu2 + '>เลขรับของ</button>';
                                                                                    } else {
                                                                                        record.set('c_arrive_code', value);
                                                                                        return '<button ' + Ext.styleBu2 + '>เลขรับของ (' + value + ')</button>';
                                                                                    }
                                                                                }
                                                                                return value;
                                                                            }
                                                                        ...
                                                                        afterrender: function () {
                                                                            ...
                                                                            this.on("cellclick", function (grid, rowIndex, columnIndex, e) {
                                                                                ...
                                                                                if (columnIndex === grid.getColumnModel().getIndexById("bookArrivalID") && rs.get('bg_reserve_money_id') == 0) { //&& record.get('i_period')== 1 && record.get('bg_reserve_money_id')==0
                                                                                    ...
                                                                                    if (Ext.isEmpty(rs.get('c_arrive_code'))) {
                                                                                        grid.getEl().mask("Please wait...", "x-mask-loading");
                                                                                        Ext.Ajax.request({
                                                                                            ...
                                                                                            success: function (response) {
                                                                                                ...
                                                                                                if (data.success && data.success !== 'Error') {
                                                                                                    ...
                                                                                                    Ext.Msg.alert("แจ้งเตือน", data.msg || 'ไม่สามารถสร้างเลขรับของได้');
                                                                                                }
                                                                                                grid.getEl().unmask();
                                                                                            },
                                                                                            failure: function (result) {
                                                                                                grid.getEl().unmask();
                                                                                                Ext.MessageBox.alert("Failed", result.responseText || 'Request failed');
                                                                                            }
                                                                                        });
                                                                                    } else {
                                                                                        Ext.Msg.alert("แจ้งเตือน", "เลขรับของนี้ถูกสร้างแล้ว : " + rs.get('c_arrive_code'));
                                                                                    }
...
//                                                      f_vat_amt3ID f_total_add_vat_amt3ID f_rate_vat3ID
                                                        ...
                                                        var sum_qty = 0;
                                                        var sum_qty_all = 0;
                                                        Ext.getCmp("tabpanelMain2ID").getEl().mask("Please wait...", "x-mask-loading");
                                                        Ext.store5.load({
                                                            callback: function (row, operation, success) {
                                                                if (success) {
                                                                    ...
                                                                    winPeriodID.show();
                                                                    Ext.getCmp("tabpanelMain2ID").getEl().unmask();
