...
Ext.leftSearch = function (tb, t, men) {
//var msgSearch = "<img src='./images/icons/billing-icon.png'>";
    let showSearchBox = (x, y) => {

        if (!Ext.isEmpty(Ext.getCmp("winSearchFrm")))
            Ext.getCmp("winSearchFrm").destroy();
        var s1 = Ext.SearchFrm(x, y);
        s1.show();
    };
    return [tb, t, men, '->',
        new Ext.form.TwinTriggerField({
            ...
            onClick: function () {
                ...
                showSearchBox(525, 45);
            },
            onTrigger1Click: function () {

                showSearchBox(510, 30);
            },
...
Ext.contextMenu = function () {
    var headerGroup = [{
            ...
            text: "สถานะของ PR Time line",
            icon: "../images/icons/text_list_bullets.png",
            handler: function (e) {

                if (!Ext.isEmpty(Ext.getCmp("winTimeline_torID")))
                    Ext.getCmp("winTimeline_torID").destroy();
                var s1 = new Ext.Window({
                    title: "สถานะของ PR Time line " + Ext.selectRow.get('c_code'),
                    icon: "text_list_bullets.png",
                    id: "winTimeline_torID",
                    modal: true,
                    plain: true,
                    collapsible: true,
                    maximizable: true,
                    border: false,
                    layout: "column",
                    width: Ext.getCmp("contenterCenter").getWidth() - 0,
                    height: Ext.getCmp("contenterCenter").getHeight() - 0,
                    items: [new Ext.FormPanel({
                            ...
                            columnWidth: 0.5,
                            height: Ext.getCmp("contenterCenter").getHeight() - 0,
                            html: '<iframe src="timeline.php?c_code=' + Ext.selectRow.get('c_code') + '" frameborder="0" width="100%" height="100%"></iframe>',
                        })],
                    buttonAlign: 'left',
                    buttons: [{text: 'ปิด', iconCls: "icon-back",
                            handler: function () {
                                Ext.getCmp("winTimeline_torID").destroy();
                            }
                        }]
            ...
            text: "อัพโหลด upload pdf",
            icon: "../images/icons/folder_up.png",
            handler: function (e) {
                console.log(Ext.selectRow);
                if (!Ext.isEmpty(Ext.getCmp("winUpload_torID")))
                    Ext.getCmp("winUpload_torID").destroy();
                var s1 = new Ext.Window({
                    title: "จัดการอัพโหลดไฟล์หลายไฟล์ " + Ext.selectRow.get('c_code'),
                    ...
                    border: false,
                    layout: 'fit',
                    width: Ext.getCmp("contenterCenter").getWidth() - 160,
                    height: Ext.getCmp("contenterCenter").getHeight() - 80,
                    html: '<iframe src="https://eis.vajira.ac.th/sp_mn/upload/index.php?code=' 
                            + (Ext.selectRow.get('c_code')?Ext.selectRow.get('c_code'):'PR00') 
                            + '&id=' + (Ext.selectRow.get('id')?Ext.selectRow.get('id'):'0') 
                            + '&step=' + (Ext.selectRow.get('step')?Ext.selectRow.get('step'):'step0') 
                            + '&v=' + (Ext.selectRow.get('version')?Ext.selectRow.get('version'):'v0') 
                            + '&y=' + (Ext.selectRow.get('year')?Ext.selectRow.get('year'): Ext.util.Format.date(new Date(), 'Y'))
                            + '" frameborder="0" width="100%" height="100%"></iframe>',
                    buttonAlign: 'left',
                    buttons: [{text: 'ปิด', iconCls: "icon-back", handler: function () {
                                Ext.getCmp("winUpload_torID").destroy();
                            }
                        }]
            ...
            text: "Cltr+C คัดลอก grid cell ใน column",
            icon: "../images/icons/page_copy.png",
            handler: function (e) {
                Ext.copyCellSel();
            }
//        }, {
//        text: "คัดลอกข้อมูลใน grid row ทั้งแถว  ",
//                icon: "../images/icons/page_copy.png",
//                handler: function (e) {
//                Ext.copyRowSel();
//                }
...
//                }
            ...
            text: 'เปิดออก', icon: "../images/icons/outline.png",
            handler: function () {
                window.open('#', 'Monitoring', 'fullscreen="yes"');
            }
        }, {
            text: 'เปิดดูเต็มจอ F11', icon: "../images/icons/application_start.png",
            handler: function () {
                requestFullScreen();
            }
        }, {
            text: 'Reload หน้าจอ', icon: "../images/icons/reload.png",
            handler: function () {
...
Ext.copyCellSel = function () {

    if (!Ext.isEmpty(Ext.dataIndex))
        if (window.clipboardData) {
            // Internet Explorer
            window.clipboardData.setData("Text", Ext.cellData);
        } else {
            var forExecElement = CreateElementForExecCommand(Ext.cellData);
...
Ext.AppConfig = function () {
    ...
    function uploadFrm() {
        ...
        return new Ext.Window({
            title: "จัดการอัพโหลดไฟล์หลายไฟล์",
            iconCls: "icon-application-view-list",
            ...
            html: '<iframe src="https://eis.vajira.ac.th/sp_mn/upload/" frameborder="0" width="100%" height="100%"></iframe>',
        });
    }
    function SearchFrm() {
                ...
                searchGrid.superclass.constructor.call(this, {
                    ...
                    buttons: [
                        ...
                        },
                        {
                            ...
                            iconCls: "icon-reset",
                            handler: function () {
                                Ext.getCmp("frm-serachID").getForm().reset();
                            },
                        },
                    ],
                });
...
//                            metaData.attr = "style='cursor:pointer; text-align:center;';";
    ...
    const search = function () {
        ...
        if (msg == "") {
            ...
            Ext.storeDtl.setBaseParam("filter", Ext.getCmp("value-box").getValue());
...
Ext.getBodyMultiBudget = function (rec, status) {
    var i_amount_bg = Ext.selectRow.get("i_amount_bg") || Ext.selectRow.json.i_amount_bg;
    console.log(Ext.selectRow.get('po_expense_id'));
    console.log(rec.get('po_expense_id'));
//     return false;
