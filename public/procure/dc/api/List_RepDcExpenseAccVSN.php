...
function List_QueryParam() {
	...
	$sqlMain = "SET NOCOUNT ON
				SELECT a.* 
					,(select c.c_code+' '+c.c_name from dc_expense_group_vsn c inner join dc_expense_vsn b on c.dc_expense_group_vsn_id = b.dc_expense_group_vsn_id where b.dc_expense_vsn_id=a.dc_expense_vsn_id) as c_group_full
 					,(select b.c_code+' '+b.c_name from dc_expense_vsn b where b.dc_expense_vsn_id=a.dc_expense_vsn_id) as c_sub_group_full
	...
	if( $stmt ) {
		while($row =$db->Fetch($stmt)) {
			$temp = array(	"no"						=> ++$totalCount,
							...
							"c_sub_group_full"			=> $row["c_sub_group_full"],
