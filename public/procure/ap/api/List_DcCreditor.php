...
if($_REQUEST["type"] == "cnt") {
	...
	$sqlTempTable = "	SELECT
						...
						FROM NMU.dbo.dc_creditor a LEFT JOIN dc_tax_customer b ON a.dc_tax_customer_id = b.dc_tax_customer_id
						WHERE a.i_delete = 2
						$con";
	...
	$con	= null;	
	$con	.= " AND a.dc_creditor_id = ".$_REQUEST["id"]." ";
	...
	while($row =$db->Fetch($stmt)){
		$temp = array(	"bank_no"						=> "{$row["numrow"]}",
						"bank_id"						=> "{$row["dc_bank_acc_creditor_id"]}",
						...
						"bank_dc_creditor_id"			=> "{$row["dc_creditor_id"]}",
