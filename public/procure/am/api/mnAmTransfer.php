...
switch ($mode) {
	case "ADD" : 
		...
		$data["inv_tran_type_id"] = AM_TRAN_TYPE_6;
		$data["i_enable"] = STATUS_ENABLE;
		$data["c_code_gen"] = $code_gen;
		$data["d_date_chg"] = $date->bc_to_ad($data["d_date_chg"]);
	...
	case "GENCODE":
		...
		// อัพเดท หน่วยงานใหม่ลงตาราง dc_asset
		$sql = "update a
				set a.dc_cost_id_ta = c.dc_cost_id_new
				, a.ta_date = c.d_date_chg
				from dc_asset a
