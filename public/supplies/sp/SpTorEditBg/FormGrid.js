Ext.onReady(function () {
  ...
  const controllTab = function (record, butt) {
    ...
    if (butt == "edit" || butt == "view") {
      ...
      Ext.selectRrow = record;
      console.log(Ext.selectRrow);
  ...
  cellClick = function (grid, rowIndex, columnIndex, e) {
    ...
    if (columnIndex == grid.getColumnModel().getIndexById("edit")) {
      ...
      if( record.data.check_edit_bg == 3 ){
        Ext.MessageBox.alert("แจ้งเตือน-", "กรุณาเลือกวิธีการใช้เงิน"); // alert massage error
      } else {
        approve(record)
      }
    }
    ...
    columns: [
      ...
      // },
      {
        ...
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
          let vv = record.data.i_status_last;
          metaData.attr = 'style="font-weight: bold; color: ' + Ext.COLOR_STATUS[vv] + ';"';
          return value+(record.get('c_comment_edit1') != null || ""?"/ <span style='color:red'>"+record.get('c_comment_edit1')+"</span>":"");        
        },
      },
      {
