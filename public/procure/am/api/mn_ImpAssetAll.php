	...
	case "EDIT":
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				...
				SET NOCOUNT ON
				INSERT INTO {$DATABASE_NAME} imp_assetall_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE {$DATABASE_NAME} imp_assetall_hdr SET " . substr($addField, 1) . " WHERE imp_assetall_hdr_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
		...
		if ($msg == "") {
			foreach ($Arr as $fldd) {
				...
				if ($fldd["imp_assetall_dtl_id"] > 0) { // EDIT
					...
					$sql = "UPDATE {$DATABASE_NAME} imp_assetall_dtl SET " . substr($addField, 1) . " WHERE imp_assetall_dtl_id = ?;
						{$con_acc}";
					//echo  $sql ; exit;
					$db->QueryParam($sql, $arrValue);
				} else { // ADD

					$data["imp_assetall_hdr_id"]          = $_REQUEST["id"];
					foreach ($data as $fld => $value) {
						$arrValue[] = ($value != "") ? $value : null;
						$addField .= ", {$fld}";
						$addValue .= ", ?";
					...
					$sql = "
						SET NOCOUNT ON
						INSERT INTO {$DATABASE_NAME} imp_assetall_dtl (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
						{$con_acc}";
					$para	= $db->QueryParam($sql, $arrValue);
				}
				// ============== //
