...
Ext.onReady(function () {
    ...
    // Grid 2
    ////////////////////////////////////////////////////////////////////////////////////////
    var userOnline = new Ext.data.JsonStore({
        autoLoad: true,
        storeId: "myStoreCost",
        url: "api/userOnline.php",
        baseParams: {type: "userlog"},
        root: "data",
        idProperty: "socket",
        totalProperty: "totalCount",
        fields: ["no", "id", "socket", "c_name", "menuname", "datetime"]
    });
    ...
    new xg.GridPanel({
        ...
        listeners: {
            beforerender: function () {
                var wsUri = "ws://" + Ext.ipServer + ":9001/demo/server.php";
                Ext.websocket = new WebSocket(wsUri);
            },
...
//                    var res_type = response.type;
                    ...
                    if (response.status === "disconnected"
                            ...
                            || response.status === "menuname"
                            || response.status === "logut"
                            ) {
                        Ext.getCmp('gd4ID').getEl().mask("ที่เรียกดู ณ ขณะนี้...", "x-mask-loading");

                        var task = new Ext.util.DelayedTask(function () {

                            Ext.getCmp('gd4ID').store.reload({
                                callback: function (record, operation, success) {
                                    if (success) {
                                        Ext.getCmp('gd4ID').getEl().unmask();
                                    }
                                }
                            });
                        });
