...
if( $_REQUEST["type"] == "imp_expense_vsn_dtl" ) {
...
} else if( $_REQUEST["type"] == "imp_expense_vsn_item" ) {
	...
	$sqlMain = "SET NOCOUNT ON
				SELECT b.*
					,CASE
						...
						ELSE d.c_name
					END AS dc_expense_group_vsn_name
				...
				FROM imp_expense_vsn_dtl a
					...
					LEFT JOIN vw_dc_expense_group_vsn d ON b.dc_expense_group_vsn_id = d.dc_expense_group_vsn_id
				...
				ORDER BY a.imp_expense_vsn_dtl_id;";
	
	$arrParam[]	= $_REQUEST["imp_expense_vsn_dtl_id"];
	...
	if( sqlsrv_has_rows( $stmt ) ) {
		...
		while( $row = $db->Fetch( $stmt ) ) {
			...
			$temp = array(
					...
					"dc_expense_group_vsn_name"		=> $row["dc_expense_group_vsn_name"],
					"dc_expense_acc_vsn_name"		=> $row["dc_expense_acc_vsn_name"],
			...
			$arr["f_prov_fund"]				+= $row["f_prov_fund"];
