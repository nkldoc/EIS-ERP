...
switch ($mode) {
    ...
    break;
    case "REGISTER" :
        $stmt = true; $stmt2 = true;	$stmt3 = true;
        $sql = "";
        $assetRegis = @$_REQUEST["chk"];
        $hdr_id = @$_REQUEST["am_tran_rg_hdr_id"];

        $y		= date("Y")+543;
        $year	= substr($y,2,2);
        if (is_array($assetRegis))
        {
            foreach($assetRegis as $dtl_id)
            {
                $sql = "declare @am_tran_rg_dtl_id as bigint;
                        declare @strY as varchar(2);
                        declare @user_id as bigint;
                        declare @cost_id as bigint;
                        ...
                        insert into dc_asset (am_tran_rg_dtl_id, dc_cost_id, f_unit_cost, c_cost_ruins
                                , d_doc_date, f_depreciate_cost, c_comment, i_period_year
                                , d_register_date, i_is_expense, ins_is_method, i_is_ins
                                , c_name, dc_asset_type_id, dc_unit_type_id, dc_acc_id
                                , f_quan, i_enable
                                , c_code
                                , dc_user_create_id, dc_user_create_cost_id, d_create
                                , dc_user_update_id, dc_user_update_cost_id, d_update)
                        select a.am_tran_rg_dtl_id, a.dc_cost_id, a.c_cost_asset, a.c_cost_ruins
                                , a.d_receive_date, a.f_depreciate, a.c_comment, a.i_period_year
                                , a.d_register_date, isnull(a.i_is_expense, ".ASSET_CAL_NO.") as i_is_expense, a.ins_is_method, a.i_is_ins
                                , b.c_name as asset_name, b.dc_asset_type_id, b.dc_unit_type_id, b.dc_acc_dr_id
                                , 1 as f_quan, ".STATUS_ENABLE." as i_enable
                                , case when (select count(dc_asset_id) from dc_asset where c_code like a.c_code+'-'+@strY+'-%') > 0 
                                        then a.c_code+'-'+@strY+'-'+ right('000000'+cast((select cast(max(right(c_code,6)) as bigint)+1 from dc_asset where c_code like a.c_code+'-'+@strY+'-%') as varchar(6)),6)
                                        else a.c_code+'-'+@strY+'-000001' end c_code
                                , @user_id, @cost_id, getdate()
                                , @user_id, @cost_id, getdate()
                        from am_tran_rg_dtl a
                            inner join dc_asset_type b on left(b.c_code,LEN(a.c_code))=a.c_code
                        where am_tran_rg_dtl_id = @am_tran_rg_dtl_id;
						
                        update am_tran_rg_dtl
                        set i_is_success = ".ASSET_STATUS_SUCCESS."
                        where am_tran_rg_dtl_id = @am_tran_rg_dtl_id;";
                        $stmt = $db->QueryParam($sql, array($dtl_id, $year, $data["dc_user_update_id"], $data["dc_user_update_cost_id"]));
            }// end foreach
            ...
            $sqlHdr = "
                        ...
                        , i_is_success = case when (select count(am_tran_rg_dtl_id) from am_tran_rg_dtl where isnull(i_is_success, 0) = 0 and am_tran_rg_hdr_id = @am_tran_rg_hdr_id) > 0 then 0  else 1 end
