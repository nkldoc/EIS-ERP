<?php
include("../api/List_RepGlBalanceCostPost.php");
...
if ($_REQUEST["i_type_report"]==1)
	$caption	= "รายงานยอดคงเหลือบัญชีแยกประเภท (ตามศูนย์ต้นทุน)"; 
else
	$caption	= "งบทดลอง (ตามศูนย์ต้นทุน)";
/*===============================================*/
...
if( is_array($data_dtl) && count($data_dtl["data"]) > 0 ) {
	...
	foreach($data_dtl["data"] as $index => $jObj) {
		...
		if($jObj["i_type"]==1) { //หน่วยงาน  
 			$tbody	.=	"<tr>";
			$tbody	.= "<td nowrap ".$style." align='center'>".$jObj["no"]."</td>";
			$tbody	.= "<td nowrap ".$style."><b>".$jObj["cost_code"]." ".$jObj["cost_name"]."</b></td>";
			...
			$tbody	.= "<td nowrap ".$style." align='right'><b>".number_format($jObj["f_end_cr"],2)."</b></td>";			
 			$tbody	.=	"</tr>";
		} else if (($jObj["i_type"] == 2)|| ($jObj["i_type"] == 7)) { //บัญชีคุม
 			$tbody	.=	"<tr>";
			$tbody	.= "<td nowrap ".$style." align='center'>".$jObj["no"]."</td>";
			$tbody	.= "<td nowrap ".$style.">&nbsp;&nbsp;&nbsp;&nbsp;".$jObj["acc_code_parent"]." ".$jObj["acc_name_parent"]."</td>";
			...
			$tbody	.= "<td nowrap ".$style." align='right'>".number_format($jObj["f_end_cr"],2)."</td>";
 			$tbody	.=	"</tr>";
 		} else if ($jObj["i_type"] == 3) { //บัญชีย่อย
 			$tbody	.=	"<tr>";
			$tbody	.= "<td nowrap ".$style." align='center'>".$jObj["no"]."</td>";
