...
Ext.sp_user = new Ext.data.JsonStore({
  ...
  baseParams: { type: "sp_emp_show", all: "all" },
  root: "data",
  idProperty: "id",
  fields: ["id", "c_name"],
  listeners: {
    load: function (t, records, options) {
      // if ([1,32,9, 19 ,13].includes(Ext.session.i_level)) {
...
Ext.SP_TOR_EVENT = new Ext.data.JsonStore({
  autoLoad: true,
  url: "tor/api/mnTorCheckList.php",
  baseParams: {
    mode: "LIST_SP_TOR_EVENT",
    // i_level: 4,
    // show: "all"
    all: "all",
  },
  ...
  fields: ["sp_tor_event_id", "event_type", "event_date", "event_title", "event_detail", "d_create", "user_name", "event_time_start", "event_time_end"],
});
...
Ext.dc_cost = new Ext.data.JsonStore({
  ...
  listeners: {
    load: function (t, records, options) {
      if ([0, 38, 3].includes(parseInt(Ext.session.dc_cost_id))) {
        Ext.getCmp("s_dc_cost_idID").setValue("0");
      } else {
        Ext.getCmp("s_dc_cost_idID").setValue(Ext.session.dc_cost_id);
      }
    },
  },
});
...
Ext.storeEven = new Ext.data.JsonStore({
  autoDestroy: false,
  autoLoad: false,
  url: "tor/api/mnTorCheckList.php",
  baseParams: {
...
Ext.sp_event_type = new Ext.data.JsonStore({
  autoDestroy: false,
  autoLoad: true,
  url: "api/All_PoWorkingImpHdr.php",
  baseParams: {
...
Ext.storeDtl = new Ext.data.JsonStore({
  ...
  baseParams: {
    ...
    i_type: 0,
    i_enabled: 1,
    dc_cost_id: Ext.session.dc_cost_id,
