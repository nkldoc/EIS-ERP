...
function List_QueryParam()
{
	...
	$sqlMain = "SET NOCOUNT ON 
					SELECT
						...
						-- ====== Result Set 4: เงินจองตรวจรับ (i_reserve=3, i_finish=0) + เบิกจ่ายแล้ว (i_reserve=3, i_finish=1) แยกตามหมวดค่าใช้จ่าย ======
						-- [เพิ่ม] ให้ตรงกับสูตรในหน้า Budget_Monitoring_Dashboard.php (List_Rep_Budget_Monitoring_Dashboard.php Result Set 2)
						-- [แก้ไข] เพิ่ม bg_expense_id / dc_expense_budget_type_id + GROUP BY (เดิมรวมยอดเดียวทั้งก้อน)
						--         เพื่อให้ JS ฝั่งกราฟ PR/PO นำยอดเบิกจ่ายแล้วไปแสดงแยกตามหมวดค่าใช้จ่ายได้เหมือน PO
						SELECT
							a.bg_expense_id
							,a.dc_expense_budget_type_id
							,(SELECT c_name FROM " . DB_NMU_EIS . "bg_expense WHERE bg_expense_id = a.bg_expense_id) AS bg_expense
							...
							, SUM(CASE WHEN a.i_reserve = 3 AND ISNULL(a.i_finish,0) = 1 THEN 1 ELSE 0 END) AS paid_count
						FROM #temp_bg_reserve_money a
						WHERE a.i_reserve = 3
							AND a.i_sys = 1
							{$whereFinal}
						GROUP BY a.bg_expense_id, a.dc_expense_budget_type_id
...
";
	...
	if ($stmt) {
		...
		// [แก้ไข] ตอนนี้ query คืนมาแยกเป็นหลายแถวต่อ bg_expense_id แล้ว (เดิมแถวเดียวรวมยอด)
		//         จึงต้องวนอ่านทุกแถวแล้วรวมยอด พร้อมเก็บ per-row ไว้ใน $paid_detail สำหรับ JS ฝั่งกราฟ
		$f_check_total = 0;
		$f_paid_total  = 0;
		$paid_detail   = [];   // [เพิ่ม] เก็บ per-row ต่อ bg_expense_id (เหมือน budget_detail/contract)
		if ($db->NextResult($stmt)) {
			while ($rowChk = $db->Fetch($stmt)) {
				$chk  = (float)($rowChk['f_reserve_check_total'] ?? 0);
				$paid = (float)($rowChk['f_paid_total']          ?? 0);
				$cnt  = (int)($rowChk['paid_count']              ?? 0);
				$f_check_total += $chk;
				$f_paid_total  += $paid;

				$paid_detail[] = array(
					"bg_expense_id"             => (int)$rowChk["bg_expense_id"],
					"bg_expense"                => $rowChk["bg_expense"],
					"dc_expense_budget_type_id" => (int)$rowChk["dc_expense_budget_type_id"],
					"f_reserve_check_total"     => $chk,
					"f_paid_total"              => $paid,
					"paid_count"                => $cnt,
				);
			}
	...
	return json_encode(array(
		...
		"paid"                  => $paid_detail,   // [เพิ่ม] เบิกจ่ายแล้ว แยกตามหมวดค่าใช้จ่าย สำหรับกราฟ PR/PO
	));
}
