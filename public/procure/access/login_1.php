...
if (isset($_REQUEST['ss_user_id'])) {
    ...
    $sql = "select a.dc_user_id, a.dc_cost_id, b.dc_emp_id, a.c_full_name
            ...
            where a.i_enable = ? and a.i_delete = ? and a.dc_user_id = ?";
    $stmt = $db->QueryParam($sql, array(STATUS_ENABLE, DELETE_FALSE, trim($_REQUEST['ss_user_id']) ?? null));
//    echo sql;
//    exit();
//    $stmt = $db->QueryParam($sql, array(STATUS_ENABLE, DELETE_FALSE, $_POST["userf"], md5($_POST["passwordf"])));
    ...
    while ($data = $db->Fetch($stmt)) {
      ...
      /**/
        $_SESSION["DOMAIN_LOGIN"] = "EIS"; //id ผู้ใช้งาน
        ...
        $_SESSION["super_user"] = intval($data["super_user"])?? null; //ผู้ใช้งานสามารถดูสิทธิ์ได้ทั้ง หน่วยงาน
