...
###################
 ...
 { 
		$sqlTempTable = "select ar_so_dtl_id
						...
						where ar_so_hdr_id=? and isnull(i_delete,2) = ? and isnull(i_enable,2) = ? ";
		/* echo $sqlTempTable; print_r($arrParam); exit; */				
	$arrParam       = array($_REQUEST['id'], DELETE_FALSE, STATUS_ENABLE);
	$arrCountParam 	= array($_REQUEST['id'], DELETE_FALSE, STATUS_ENABLE);
...
//
		$temp = array("no" => ($i++), 
						...
						|| ((isset($_REQUEST['mn']) && $_REQUEST['mn']=='editso') && (isset($_REQUEST['accessData']) && $_REQUEST['accessData']=='edit'))
