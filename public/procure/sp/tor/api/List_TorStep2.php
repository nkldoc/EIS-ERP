                  ...
                  , a.po_expense_id
                    , a.dc_expense_budget_type_id
                     , a.bg_budget_dtl_project_id
                        ...
                        from dbo.sp_tor a
                        left join sp_tor_contract  c on c.sp_tor_id = a.tor_id
                        where a.i_is_notor = 0    and c.i_is_monthly=1   and  a.i_purchase<>1 and a.i_enabled = 1" . $wh; //
    // echo $sqlTempTable;
//     exit;
    $arrParam[] = $start;
    $arrParam[] = $limit; 
    $sqlMain = "select a.* , s.c_code
                        ...
                        , (select SUM(isnull(aaa.i_not_do,99))
        ...
        "
            ...
            // . " inner join dbo.sp_tor_contract ss on ss.sp_tor_contract_id=c.sp_tor_contract_id"
...
// /********************/
    ...
    //  echo json_encode(array("debug" => true, "totalCount" => $totalCount, $root => ${$root}));
     ...
     exit();
 } else if ($type == "po_working_dtl_check") {
    $tor_status_id = $_REQUEST["tor_status_id"] ?? null; // status id
    $i_alarm = $_REQUEST["i_alarm"] ?? null; // status id
    $i_pa = $_REQUEST["i_pa"] ?? null; // status id
                 ...
                 , a.po_expense_id
                   , a.dc_expense_budget_type_id
                    , a.bg_budget_dtl_project_id
                       ...
                       from dbo.sp_tor a
                       left join sp_tor_contract  c on c.sp_tor_id = a.tor_id
                       where c.i_is_monthly=1  
                       and  a.i_purchase<>1 and a.i_enabled =  1  {$where}  " . $wh; //
...
//     exit;
   ...
   $sqlMain = "select a.* , s.c_code
                       ...
                       ,  (select SUM(isnull(aaa.i_not_do,99))
                            from sp_tor_contract aa
                            left join sp_gl_monthly_hdr bb on aa.sp_tor_contract_id = bb.sp_tor_contract_id and bb.i_enabled = 1
                            left join (select sp_gl_monthly_hdr_id
                                            , CASE WHEN count(*) < 0 THEN 0 when COUNT(*) = 1 then  3 
                                            ELSE 1 END as i_not_do from sp_gl_monthly_dtl
                                            where i_enabled = 1
                                            group by sp_gl_monthly_hdr_id) aaa 
                                on bb.sp_gl_monthly_hdr_id = aaa.sp_gl_monthly_hdr_id
                                where aa.sp_tor_id = a.tor_id) as i_not_do 
                       , isnull(s.tor_type_id,1) as tor_type_id
                       , s.f_period_amt
                       , s.f_total_amt
                  ...
                  , a.po_expense_id
                    , a.dc_expense_budget_type_id
                     , a.bg_budget_dtl_project_id
                        ...
                        , a.i_hire_type, a.i_product_type, a.i_is_inv, a.i_type_fix_rate,i_delivery_date
                        , row_number() over (order by a.tor_id DESC) as row
                        from dbo.sp_tor a  where a.i_enabled=1 and a.tor_status_id =" . $step . $wh . $util->viewDepartmentTor('a', $_SESSION["dc_department_id"]); //
//     echo $sqlTempTable;
//     exit;
     $arrParam[] = $start;
     $arrParam[] = $limit;
