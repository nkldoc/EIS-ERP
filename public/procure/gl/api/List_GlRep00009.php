<?php
include("../../conf/config.php");
include("../../lib/database/DatabaseServer.php"); 
include("../../lib/date/i_date.class.php");

$db 		= new DatabaseServer();
$date		= new i_date();

$root		= "data";
$data		= array();
$con		= null;

function List_QueryParam() {
	
	global $db,$date,$root,$data, $con,$arr_status;
 
	$totalCount		= 0;
	$conCheque		= "";
	$conDate		= "";
	$conDateEP		= "";
	$conDateVSN		= "";
	$conDCCheque	= "";

	if($_REQUEST["page"] == "GlRep00009") { // รายงาน บัญชีย่อยเงินฝากธนาคาร
		$conDateCancel = $conDateEP = $conDateVSN = "";
		$for_id = explode ( ";", $_REQUEST ["dc_bank_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque .= ($in != "") ? " AND e.dc_bank_id IN (" . $in . ")" : "";
			}
		}
		
		$for_id = explode ( ";", $_REQUEST ["dc_bank_acc_company_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque 		.= ($in != "") ? " AND e.dc_bank_acc_company_id IN (" . $in . ")" : "";
				$conDCCheque	.= ($in != "") ? " AND d.dc_bank_acc_company_id IN (" . $in . ")" : "";
			}
		}
		
		$conFixedAcc1 = " AND e.dc_bank_deposit_type_id=1"; //ออมทรัพย์
		$conFixedAcc2 = " AND e.dc_bank_deposit_type_id=2"; //กระแส
		
		
		if($_REQUEST["date_start"] != "" && $_REQUEST["date_end"] != "") {
			$conDateEP		.= " AND b.d_pay 			between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
			$conDateVSN		.= " AND b.d_cheque 		between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
			$conDateCancel 	.= " AND r.d_save_jv_cancel between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
		}
		
		$sqlMain = "SET NOCOUNT ON
					/*================= e-phys BANK (SUM) กระแส =================*/
					SELECT *, 1 AS i_type
					INTO #tb_cheque_sum
					FROM ( SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_pay, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(b.f_inv+ISNULL(b.f_vat,0)) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I'
						{$conDateEP} {$conCheque} {$conFixedAcc2}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, b.d_pay
					UNION ALL
					/*=============== vision net BANK (SUM) กระแส   ===============*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_cheque, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(b.f_inv) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I'
						{$conDateVSN}  {$conCheque}  {$conFixedAcc2}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, b.d_cheque 
					
					UNION ALL
					/*================= e-phys BANK (SUM) ออมทรัพย์ =================*/
					SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_pay, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(b.f_inv+ISNULL(b.f_vat,0)) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I'
						{$conDateEP} {$conCheque} {$conFixedAcc1}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, b.d_pay
					UNION ALL
					/*=============== vision net BANK (SUM) ออมทรัพย์  ===============*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_cheque, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(b.f_inv) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I'
						{$conDateVSN}  {$conCheque}  {$conFixedAcc1}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, b.d_cheque 
					
					) a
					
					/*==================== e-phys CHEQUE ====================*/
					SELECT *, 2 AS i_type
					INTO #tb_cheque
					FROM ( SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_pay, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(c.f_cheque,0)) AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN imp_expense_dtl_cheque c ON b.imp_expense_dtl_id = c.imp_expense_dtl_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						LEFT JOIN dc_cheque d ON c.dc_cheque_id = d.dc_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I'
						{$conDateEP}  {$conCheque}  {$conFixedAcc2} {$conDCCheque}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque, b.d_pay
					UNION ALL
					/*================== vision net CHEQUE ==================*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, b.d_cheque, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(c.f_cheque,0)) AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN imp_expense_vsn_dtl_cheque c ON b.imp_expense_vsn_dtl_id = c.imp_expense_vsn_dtl_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						LEFT JOIN dc_cheque d ON c.dc_cheque_id = d.dc_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I'
						{$conDateVSN}  {$conCheque}  {$conFixedAcc2} {$conDCCheque}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque, b.d_cheque ) a
					
					
					/*================= e-phys CANCEL-BANK (SUM) กระแส =================*/
					SELECT *, 3 AS i_type
					INTO #tb_cheque_sum_cancel
					FROM 
					( SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(chqs.f_cheque) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_dtl_id = b.imp_expense_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN imp_expense_dtl_cheque chqs ON chqs.imp_expense_dtl_cheque_id = r.imp_expense_dtl_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE."
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=1
						and chqs.i_status = 2
						{$conDateEP} {$conDateCancel} {$conCheque}   {$conFixedAcc2} {$conDateCancel}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel
					UNION ALL
					/*=============== vision net CANCEL-BANK (SUM) กระแส ===============*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(chqs.f_cheque) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_vsn_dtl_id = b.imp_expense_vsn_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN imp_expense_vsn_dtl_cheque chqs ON chqs.imp_expense_vsn_dtl_cheque_id = r.imp_expense_vsn_dtl_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE."
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=2
						and chqs.i_status = 2
						{$conDateVSN} {$conDateCancel} {$conCheque}   {$conFixedAcc2}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel 
					
					) a
										

					/*================= e-phys CANCEL-EXPENSE (SUM) =================*/
					SELECT *, 4 AS i_type
					INTO #tb_cheque_expense_cancel
					FROM ( SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันจ่าย' AS c_name
						, 0.00 AS f_dr
						, SUM(chqs.f_cheque) AS f_cr 
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_dtl_id = b.imp_expense_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_id_cancel = c.gl_tran_hdr_id
						INNER JOIN imp_expense_dtl_cheque chqs ON chqs.imp_expense_dtl_cheque_id = r.imp_expense_dtl_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE."
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=1
						and chqs.i_status = 2
						{$conDateEP}  {$conDateCancel} {$conCheque}   {$conFixedAcc2}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel
					UNION ALL
					/*=============== vision net CANCEL-EXPENSE (SUM) ===============*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันจ่าย' AS c_name
						, 0.00 AS f_dr
						, SUM(chqs.f_cheque) AS f_cr 
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_vsn_dtl_id = b.imp_expense_vsn_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_id_cancel = c.gl_tran_hdr_id
						INNER JOIN imp_expense_vsn_dtl_cheque chqs ON chqs.imp_expense_vsn_dtl_cheque_id = r.imp_expense_vsn_dtl_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE."
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=2
						and chqs.i_status = 2
						{$conDateVSN}  {$conDateCancel}   {$conCheque}   {$conFixedAcc2}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel ) a

					
					/*==================== e-phys CANCEL BY - CHEQUE ====================*/
					SELECT *, 5 AS i_type
					INTO #tb_cheque_cancel
					FROM ( SELECT
						b.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, r.d_save_jv_cancel, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(c.f_cheque,0)) AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN imp_expense_dtl_cheque c ON b.imp_expense_dtl_id = c.imp_expense_dtl_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_dtl_id = b.imp_expense_dtl_id
						LEFT JOIN dc_cheque d ON c.dc_cheque_id = d.dc_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I' 
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=1
						and c.i_status = 2
						{$conDateEP}  {$conDateCancel}  {$conCheque}   {$conFixedAcc2}
					GROUP BY b.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque, r.d_save_jv_cancel
					UNION ALL
					/*================== vision net CANCEL BY - CHEQUE ==================*/
					SELECT
						b.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(c.f_cheque,0)) AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN imp_expense_vsn_dtl_cheque c ON b.imp_expense_vsn_dtl_id = c.imp_expense_vsn_dtl_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_vsn_dtl_id = b.imp_expense_vsn_dtl_id
						LEFT JOIN dc_cheque d ON c.dc_cheque_id = d.dc_cheque_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I' 
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=2
						and c.i_status = 2
						{$conDateVSN}   {$conDateCancel}  {$conCheque}   {$conFixedAcc2}
					GROUP BY b.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque,r.d_save_jv_cancel) a
					 
					/*================= e-phys CANCEL-BANK (SUM) ออมทรัพย์  =================*/
					SELECT *, 6 AS i_type
					INTO #tb_cheque_sum_cancel_aomsub
					FROM 
					(  
					SELECT
						a.imp_expense_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(ff.f_dr) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense' AS c_system
					FROM imp_expense_hdr a
						INNER JOIN imp_expense_dtl b ON a.imp_expense_hdr_id = b.imp_expense_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_dtl_id = b.imp_expense_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN gl_tran_dtl ff ON ff.gl_tran_hdr_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE."  and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE." AND c.i_is_post>1 AND ISNULL(c.i_is_close_year,2)=2 and LEFT(c.c_code,1)='G'
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=1 
						{$conDateEP}  {$conDateCancel}   {$conCheque}   {$conFixedAcc1}
					GROUP BY a.imp_expense_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel
					UNION ALL
					/*=============== vision net CANCEL-BANK (SUM) ออมทรัพย์  ===============*/
					SELECT
						a.imp_expense_vsn_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(ff.f_dr) AS f_dr
						, 0.00 AS f_cr
						,'imp_expense_vsn' AS c_system
					FROM imp_expense_vsn_hdr a
						INNER JOIN imp_expense_vsn_dtl b ON a.imp_expense_vsn_hdr_id = b.imp_expense_vsn_hdr_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target
						INNER JOIN imp_cancel_doc_expense r on r.imp_expense_vsn_dtl_id = b.imp_expense_vsn_dtl_id
						INNER JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN gl_tran_dtl ff ON ff.gl_tran_hdr_id = c.gl_tran_hdr_id
					WHERE a.i_enable = ".STATUS_ENABLE." and left(a.c_code,1)='I' 
						and b.i_status=2
						and c.i_enable = ".STATUS_ENABLE." AND c.i_is_post>1 AND ISNULL(c.i_is_close_year,2)=2 and LEFT(c.c_code,1)='G'
						and r.i_enable = ".STATUS_ENABLE." and r.i_type_doc=2 
						{$conDateVSN}   {$conDateCancel}  {$conCheque}   {$conFixedAcc1}
					GROUP BY a.imp_expense_vsn_hdr_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel ) a
						 
					SELECT *
					INTO #cheque FROM
					(SELECT
						RIGHT('0'+CAST(DAY(a.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(a.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(a.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum a
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(b.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(b.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(b.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque b 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(c.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(c.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(c.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum_cancel c 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(d.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(d.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(d.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_expense_cancel d 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(e.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(e.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(e.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_cancel e  
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(f.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(f.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(f.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum_cancel_aomsub f 
					) a;
					
					SELECT
						dc_bank_acc_company_id AS source_id
						,*
					FROM #cheque
					ORDER BY dc_bank_acc_company_id, d_cheque_date, pk_id, i_type, c_cheque;";
		
	} else if($_REQUEST["page"] == "RepBankAccountDetail") { // รายงาน บัญชีย่อยเงินฝากธนาคาร (รายได้)
		
		$for_id = explode ( ";", $_REQUEST ["dc_bank_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque .= ($in != "") ? " AND aa.dc_bank_id IN (" . $in . ")" : "";
			}
		}
		
		$for_id = explode ( ";", $_REQUEST ["dc_bank_acc_company_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque .= ($in != "") ? " AND bb.dc_bank_acc_company_id IN (" . $in . ")" : "";
			}
		}
		
		if($_REQUEST["date_start"] != "" && $_REQUEST["date_end"] != "") {
			$conDate	.= " AND a.d_doc_date between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
		}
		
		switch ($_REQUEST["i_book_type"]) {
			case 1: // สมุดรายวันรับ
				$cc	= " AND a.i_book_type = 1";
				break;
			case 2: // สมุดรายวันทั่วไป
				$cc	= " AND a.i_book_type = 2";
				break;
			case 3: // สมุดรายวันจ่าย
				$cc	= " AND a.i_book_type = 3";
				break;
			default:
				$cc	= "";
		}
		
		$sqlMain = "SET NOCOUNT ON
					SELECT
						a.imp_bank_account_detail_hdr_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, a.d_doc_date, 120) AS d_cheque_date
						,a.c_doc AS c_cheque
						,CASE a.i_book_type
							WHEN 1 THEN 'สมุดรายวันรับ'
							WHEN 2 THEN 'สมุดรายวันทั่วไป'
							WHEN 3 THEN 'สมุดรายวันจ่าย'
							ELSE ''
						END AS c_name
						,b.f_dr AS f_dr
						,b.f_cr AS f_cr
					INTO #tb_cheque
					FROM imp_bank_account_detail_hdr a
						INNER JOIN imp_bank_account_detail_dtl b ON a.imp_bank_account_detail_hdr_id = b.imp_bank_account_detail_hdr_id
						INNER JOIN (
							SELECT
								bb.dc_bank_acc_company_id
								,bb.c_code
								,bb.c_name
							FROM dc_bank aa
								INNER JOIN dc_bank_acc_company bb ON aa.dc_bank_id = bb.dc_bank_id
							WHERE 1=1
								{$conCheque}
						) e ON a.dc_bank_acc_company_id = e.dc_bank_acc_company_id
					WHERE
						a.i_enable = ".STATUS_ENABLE."
							{$conDate}
							{$cc};
					
					SELECT
						RIGHT('0'+CAST(DAY(b.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(b.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(b.d_cheque_date)+543 c_yyyy
						,*
					INTO #cheque
					FROM #tb_cheque b;
					
					SELECT
						ROW_NUMBER() OVER (PARTITION BY dc_bank_acc_company_id,c_yyyy,c_mm,c_dd ORDER BY c_yyyy,c_mm,c_dd) AS i_type
						,dc_bank_acc_company_id AS source_id
						,*
					FROM #cheque
					ORDER BY dc_bank_acc_company_id, d_cheque_date, c_cheque;";
							
	} else if($_REQUEST["page"] == "RepGlBank") { // รายงาน บัญชีเงินฝากธนาคาร(ไม่มีรายละเอียดค่าใช้จ่าย)
		$conDate = $conDateCancel = "";
		$for_id = explode ( ";", $_REQUEST ["dc_bank_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque .= ($in != "") ? " AND e.dc_bank_id IN (" . $in . ")" : "";
			}
		}
	
		$for_id = explode ( ";", $_REQUEST ["dc_bank_acc_company_id"] );
		if (! in_array ( "0", $for_id )) {
			$in = "";
			if (is_array ( $for_id )) {
				foreach ( $for_id as $val ) {
					$in .= ($in == "") ? $val : ", " . $val;
				}
				$conCheque .= ($in != "") ? " AND e.dc_bank_acc_company_id IN (" . $in . ")" : "";
			}
		}

		$conFixedAcc1 = " AND e.dc_bank_deposit_type_id=1"; //ออมทรัพย์
		$conFixedAcc2 = " AND e.dc_bank_deposit_type_id=2"; //กระแส
		
		if($_REQUEST["date_start"] != "" && $_REQUEST["date_end"] != "") {
			$conDate		.= " AND a.d_save_jv_date between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
			$conDateCancel 	.= " AND r.d_save_jv_cancel between '{$_REQUEST["date_start"]}' and '{$_REQUEST["date_end"]}' ";
		}
	
		$sqlMain = "SET NOCOUNT ON
					/*================= GL_BANK (SUM) =================*/
					SELECT *, 1 AS i_type
					INTO #tb_cheque_sum
					FROM ( SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, a.d_save_jv_date, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(a.f_money) AS f_dr
						,0.00 AS f_cr
					FROM gl_bank a
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source  
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.c_code IS NOT NULL
						AND a.i_enable = ".STATUS_ENABLE."
						{$conDate} {$conCheque} {$conFixedAcc2}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, a.d_save_jv_date 
					UNION ALL
					SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, a.d_save_jv_date, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(a.f_money) AS f_dr
						,0.00 AS f_cr
					FROM gl_bank a
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target  
						LEFT JOIN gl_tran_hdr c ON a.gl_tran_hdr_id_bank_id = c.gl_tran_hdr_id
					WHERE a.c_code IS NOT NULL
						AND a.i_enable = ".STATUS_ENABLE."
						{$conDate} {$conCheque} {$conFixedAcc1}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc, a.d_save_jv_date 
					) a
					
					/*================= CHEQUE of GL_BANK  BY  ITEMS =================*/
					SELECT *, 2 AS i_type
					INTO #tb_cheque
					FROM ( SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR, a.d_save_jv_date, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(b.f_cheque,0)) AS f_cr
					FROM gl_bank a
						INNER JOIN gl_bank_cheque b ON a.gl_bank_id = b.gl_bank_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source 
						LEFT JOIN dc_cheque d ON b.dc_cheque_id = d.dc_cheque_id
					WHERE a.c_code IS NOT NULL
						AND a.i_enable = ".STATUS_ENABLE."
						{$conDate}  {$conCheque} {$conFixedAcc2}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque, a.d_save_jv_date ) a

					
					/*================= GL_BANK CANCLE GX/GL - BANK กระแส =================*/
					SELECT *, 3 AS i_type
					INTO #tb_cheque_sum_cancel
					FROM ( SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(chqs.f_cheque) AS f_dr
						,0.00 AS f_cr
					FROM gl_bank a
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source 
						INNER JOIN imp_cancel_doc_expense r on r.gl_bank_id=a.gl_bank_id
						LEFT JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN gl_bank_cheque chqs ON chqs.gl_bank_cheque_id = r.gl_bank_cheque_id
					WHERE a.i_status=2 AND a.c_code IS NOT NULL  AND a.i_enable = ".STATUS_ENABLE."
						AND c.i_enable = ".STATUS_ENABLE." AND c.i_is_post>1
						AND r.i_enable = ".STATUS_ENABLE." AND r.i_type_doc=3						
						and chqs.i_status = 2
						{$conDateCancel}  {$conCheque} {$conFixedAcc2}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel ) a

					/*================= CANCEL CHEQUE of GL_BANK  ALL ITEMS =================*/
					SELECT *, 5 AS i_type
					INTO #tb_cheque_cancel
					FROM ( SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,d.c_cheque
						,'สมุดรายวันเงินจ่าย' AS c_name
						,0.00 AS f_dr
						,SUM(ISNULL(b.f_cheque,0)) AS f_cr
					FROM gl_bank a
						INNER JOIN gl_bank_cheque b ON a.gl_bank_id = b.gl_bank_id
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_source  
						INNER JOIN imp_cancel_doc_expense r on r.gl_bank_id=a.gl_bank_id 
							AND b.gl_bank_cheque_id = r.gl_bank_cheque_id 
						LEFT JOIN dc_cheque d ON b.dc_cheque_id = d.dc_cheque_id
					WHERE  a.i_status=2 AND a.c_code IS NOT NULL AND a.i_enable = ".STATUS_ENABLE."
						AND d.i_enable = ".STATUS_ENABLE."  
						AND r.i_enable = ".STATUS_ENABLE." AND r.i_type_doc=3	 
						{$conDateCancel}  {$conCheque} {$conFixedAcc2}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, d.c_cheque, r.d_save_jv_cancel ) a

					/*================= GL_BANK CANCLE GX/GL - BANK ออมทรัพย์ =================*/
					SELECT *, 6 AS i_type
					INTO #tb_cheque_sum_cancel_aomsub_btn
					FROM ( SELECT
						a.gl_bank_id AS pk_id
						,e.dc_bank_acc_company_id
						,e.c_code AS company_code
						,e.c_name AS company_name
						,CONVERT(VARCHAR,r.d_save_jv_cancel, 120) AS d_cheque_date
						,c.c_ref_doc AS c_cheque
						,'สมุดรายวันทั่วไป' AS c_name
						,SUM(chqs.f_cheque) AS f_dr
						,0.00 AS f_cr
					FROM gl_bank a
						INNER JOIN dc_bank_acc_company e ON e.dc_bank_acc_company_id = a.dc_bank_acc_company_id_target 
						INNER JOIN imp_cancel_doc_expense r on r.gl_bank_id=a.gl_bank_id
						LEFT JOIN gl_tran_hdr c ON r.gl_tran_hdr_bank_id_cancel = c.gl_tran_hdr_id
						INNER JOIN gl_bank_cheque chqs ON chqs.gl_bank_cheque_id = r.gl_bank_cheque_id
					WHERE a.i_status=2 AND a.c_code IS NOT NULL  AND a.i_enable = ".STATUS_ENABLE."
						AND c.i_enable = ".STATUS_ENABLE." AND c.i_is_post>1
						AND r.i_enable = ".STATUS_ENABLE." AND r.i_type_doc=3						
						and chqs.i_status = 2
						{$conDateCancel}  {$conCheque} {$conFixedAcc1}
					GROUP BY a.gl_bank_id, e.dc_bank_acc_company_id, e.c_code, e.c_name, c.c_ref_doc,r.d_save_jv_cancel ) a
					
					
					SELECT *
					INTO #cheque FROM
					(SELECT
						RIGHT('0'+CAST(DAY(a.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(a.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(a.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum a
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(b.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(b.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(b.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque b 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(c.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(c.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(c.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum_cancel c 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(d.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(d.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(d.d_cheque_date)+543 c_yyyy
						,* 
					FROM #tb_cheque_cancel d 
					UNION ALL
					SELECT
						RIGHT('0'+CAST(DAY(e.d_cheque_date) AS varchar(2)) ,2) AS c_dd
						,RIGHT('0'+CAST(MONTH(e.d_cheque_date) AS varchar(2)) ,2) AS c_mm
						,YEAR(e.d_cheque_date)+543 c_yyyy
						,*
					FROM #tb_cheque_sum_cancel_aomsub_btn e  ) a;
						
					SELECT
						dc_bank_acc_company_id AS source_id
						,*
					FROM #cheque
					ORDER BY dc_bank_acc_company_id, d_cheque_date, pk_id, i_type, c_cheque;";
	
	}
	
	$arrParam[]	= "";
  
    //  echo $sqlMain;exit;
  
	$stmt = $db->QueryParam( $sqlMain, $arrParam );
	if( $stmt ) {
		
		$comArr	= array();
		$ArrDtl	= array();
		
		// ยอดยกมา
		$date_start		= date( "Y-m-d", strtotime( "{$_REQUEST["date_start"]} -1 month" ) );
		list($yyyy,$mm,$dd)		= explode("-", $date_start);
		list($yyyy2,$mm2,$dd2)	= explode("-", $_REQUEST["date_end"]);
		
		while($row = $db->Fetch($stmt)) {
			
			if( @!array_key_exists( $row["c_yyyy"], $comArr[$row["source_id"]]) ) {
				$comArr[$row["source_id"]][$row["c_yyyy"]]	= $row["c_yyyy"];
				$f_end_dr = 0;  $f_end_cr = 0;
				$sqll	= "	SELECT
								SUM(ISNULL(a.f_end_dr,0)-ISNULL(a.f_end_cr,0)) AS f_end_dr
								,SUM(ISNULL(a.f_end_cr,0)-ISNULL(a.f_end_dr,0)) AS f_end_cr
							FROM gl_balance_cost a
								INNER JOIN (
									SELECT
										bb.dc_acc_id
										,bb.dc_bank_acc_company_id
									FROM dc_bank aa
										INNER JOIN dc_bank_acc_company bb ON aa.dc_bank_id = bb.dc_bank_id
									WHERE 1=1
								) b ON a.dc_acc_id = b.dc_acc_id
							WHERE a.c_mm={$mm} AND a.c_yyyy={$row["c_yyyy"]}-543
								AND b.dc_bank_acc_company_id = {$row["source_id"]};";
						
				$data_f_end	= $db->GetDataBySQL($sqll, array());
					
				if ( $data_f_end["f_end_dr"] >= 0 ) {
					$f_end_dr	= $data_f_end["f_end_dr"];
					$f_end_cr	= 0;
				} else if ( $data_f_end["f_end_dr"] < 0 ) {
					$f_end_dr	= 0;
					$f_end_cr	= $data_f_end["f_end_cr"];
				}
			}
			
			if($row["source_id"].$row["c_yyyy"].$row["c_mm"].$row["c_dd"] != @$s_dd) {
				$s_dd	= $row["source_id"].$row["c_yyyy"].$row["c_mm"].$row["c_dd"];
				$c_dd	= $row["c_dd"];
			} else { $c_dd	= ""; }
			
			
			if($row["c_cheque"] != @$s_cheque) {
				$s_cheque		= $row["c_cheque"];
				$c_cheque		= $row["c_cheque"];
			} else { $c_cheque	= ""; }
			
			$ArrDtl[$row["source_id"]]["company_code"]	= $row["company_code"];
			$ArrDtl[$row["source_id"]]["company_name"]	= $row["company_name"];
			$ArrDtl[$row["source_id"]]["c_name"]		= $date->s_month_thai[$mm]." ".($yyyy+543)." - ".$date->s_month_thai[$mm2]." ".($yyyy2+543);
			
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["c_yyyy"]		= $row["c_yyyy"];
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["f_end_dr"]		= $f_end_dr;
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["f_end_cr"]		= $f_end_cr;
			
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["c_mm"]	= $date->s_month_thai[$row["c_mm"]];

			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["c_dd"]		= $c_dd;
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["c_cheque"]	= $c_cheque;
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["c_name"]		= $row["c_name"];
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["f_dr"]		= $row["f_dr"];
			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["f_cr"]		= $row["f_cr"];

			$ArrDtl[$row["source_id"]]["data"][$row["c_yyyy"]]["data"][$row["c_mm"]]["data"][$row["c_dd"]]["data"][$row["i_type"]]["data"][$row["c_cheque"]]["i_type"]		= $row["i_type"];
			
			
		};
	}

	return json_encode(array("debug"=>true, "totalCount"=>$totalCount, $root=>$ArrDtl));
}
?>