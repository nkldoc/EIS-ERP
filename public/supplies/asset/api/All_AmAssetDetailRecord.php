	...
	if ($stmt) {
		...
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				...
				"c_name_mode"       => $row['c_name'],
				"c_code"            => $row['c_code'],
				"i_period_year"     => $row['i_period_year'] == '' ? '0.00' : $row['i_period_year'],
...
if ($_POST["type"] == "imp_assetall_dtl") {
	...
	$sqlTempTable = "select 
					...
					from am_asset_hdr a
                    where 1=1 and a.parent_id is null";
	//    echo $sqlTempTable ;
	//    exit ;
	if ($mode == "SEARCH") {
		if ($filter && $filter !== "") {
	...
	$sqlMain = "
		...
		LEFT JOIN am_asset_dtl b ON a.am_asset_hdr_id = b.am_asset_hdr_id
		WHERE
