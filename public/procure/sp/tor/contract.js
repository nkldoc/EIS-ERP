...
const win_request_end_contract = function (rec, i_type) {
    new Ext.Window({
        title: "ยืนยันการปิดสัญญา",
        id: "win-pop_end",
        layout: "fit",
        modal: true,
        fileUpload: true, // ✅ ตรงนี้เท่านั้น
                        ...
                        items: [
                            {
                                ...
                                items: [
                                    ...
                                    {
                                        xtype: "hidden",
                                        value: i_type == 2 ? "EDIT_UPDATE_OVERLAP" : "UPDATE_OVERLAP",
                                        name: "mode",
                                    },
                                    {
                                        xtype: "hidden",
...
const genLinkBg = function (event, rec) {
    ...
    //END TEST
    switch (event) {
        case "c_overlap": // เช็คใบกัน
            link =
                    Ext.session.IPAPIBG +
                    "/?/bg/BgBudgetAllSupplies" +
                    "/i_year/" +
                    (parseInt(Ext.getCmp("i_yyyy_overlapID").getValue()) - setYear) + //((parseInt(rec.get('i_yyyy')) - setYear)-543)   test +1
        ...
        case "c_overlap_book": // จองใบกัน
            link =
                    ...
                    rec.data.sp_tor_id +
                    "/po_id/" +
                    rec.data.sp_tor_contract_id +
                    "/chk_id/0" +
                    "/i_year/" +
                    Ext.getCmp("i_yyyy_overlapID").getValue() + //+((parseInt(rec.get('i_yyyy')) - setYear)-543)    test 1
                    "/i_pr_type/" +
                    rec.data.i_pr_type1 + //  plan or period
                    "/i_reserve/2" + // step 1 PR step 2 po step3 checking
                    "/dc_cost_id/" +
                    rec.data.dc_cost2_id +
                    "/dc_budget_type_id/" +
                    rec.data.dc_expense_budget_type_id +
                    "/bg_expense_id/" +
                    rec.data.po_expense_id +
                    "/i_last/" +
                    (rec.data.i_type_contract === 3 ? 0 : 1) + // pr มี สัญญาเดียว = 1
                    "/c_code_overlap/" +
                    encodeURIComponent(Ext.c_overlap) +
                    "/f_amt/" +
                    rec.data.f_total_amt;
            break;
    }
    return link;
};

Ext.c_overlap_close = function (rec, event, c_overlap) {
    var i_yyyy_overlap = rec.i_yyyy_overlap;
    var link = genLinkBg(event, rec);
    Ext.c_overlap = c_overlap;
    Ext.Ajax.request({
        ...
        success: function (result, request) {
            ...
            var bg_budget_dtl_overlap_id = jsonData.data[0].bg_budget_dtl_overlap_id;
            if (jsonData.debug == true) {
                var f_return = jsonData.data[0].f_overlap_total;
                var f_total = rec.data.f_total_amt.replace(/,/g, "") * 1;
                if (f_total <= f_return) {
                    var link2 = genLinkBg("c_overlap_book", rec);
                    Ext.Ajax.request({
                        url: link2, //record,linkGetMoney
                        method: "GET", //POST
                        disableCaching: false,
                        success: function (result, request) {
                            var jsonData = Ext.util.JSON.decode(result.responseText); //decode json
                            var bg_reserve_overlap_id = jsonData.bg_reserve_overlap_id;
                            Ext.Ajax.request({
                                url: "tor/api/mnTorController.php",
                                params: {
                                    mode: "UPDATE_CONTRACT_BG_OVERLAP_NEW", //หลังยิง API จองใบกัน กลับมา อัพเดท
                                    sp_tor_contract_id: rec.data.sp_tor_contract_id, //rec.data.sp_tor_contract_id
                                    bg_reserve_overlap_id: bg_reserve_overlap_id,
                                    bg_budget_dtl_overlap_id: bg_budget_dtl_overlap_id,
                                    i_overlap: 2,
                                    c_overlap: c_overlap,
                                    f_type_amt: rec.data.f_total_amt,
                                },
                                method: "GET", //POST
                                disableCaching: false,
                                success: function (result, request) {
                                    Ext.MessageBox.alert("แจ้งเตือน", "บันทึกรายการเรียบร้อย");
                                    Ext.storeDtl.reload({
                    ...
                    Ext.MessageBox.alert("แจ้งเตือน", "ยอดเงินใบกันไม่เพียงพอ ติดต่องบประมาณ");
                }
                return;
                ...
                Ext.MessageBox.alert("Success", "เรียบร้อยแล้ว", function () {
                    Ext.Ajax.request({
                        ...
                        params: {
                            ...
                            c_booking: event == "c_overlap_close" ? rec.c_contract_overlap : Ext.getCmp("c_bookingID").lastSelectionText,
                        },
                        success: function (result, request) {
                                ...
                                buttons: [
                                    {
                                        ...
                                        handler: function () {
                                            ...
                                            var getSum = parseFloat(Ext.sum + Ext.getCmp("period_f_net_unit_price").getValue().replace(/,/g, "") / 1);
                                            ...
                                            if (Ext.getCmp("i_pr_type2ID").getValue() == null) {
                                                Ext.Msg.alert("แจ้งเตือน", "<span style='white-space: nowrap;'>ยังไม่ได้ระบุประเภท</span>");
                                                return false;
                                            }
...
Ext.AppUx = function (app, menu) {
    ...
    function controller(rec, evt) {
        ...
        if (Ext.isEmpty(rec)) {
            ...
            // } else if (Ext.selectRow.get("i_type_bg") == 13 && i_type_bg == 1 ) {
            //     Ext.Msg.alert("แจ้งเตือน", "ยังไม่ได้ทำการจองเงิน", function (bu, action) {
            //         return false;
            //     });
        } else {
        ...
        var colPeriod = [
            ...
            },
            {
                ...
                renderer: function (value, metaData, record, row, col, store, gridView) {
                    ...
                    if (record.get("no") > 9996)
                        return "";
                    else if (Ext.selectRow.get("i_type_contract") == 3)
                        return "สัญญา " + record.get("c_contract_code") + lastTxt;
                    else
                        return "งวด " + value + lastTxt;
                },
            },
            {
        ...
        function popOverlap(rec) {
            Ext.storeDepartment = new Ext.data.JsonStore({
                ...
                baseParams: {
                    ...
                    limit: 20,
                    mode: null,
                    dc_cost_id: rec.data.dc_cost2_id,
                    i_yyyy_overlap: Ext.yyyy,
                    dc_expense_budget_type_id: rec.data.c_dc_expense_budget_type_id,
                }, //Permission i_read
                idProperty: "id",
                totalProperty: "totalCount",
            ...
            Ext.booking_store = new Ext.data.JsonStore({
                ...
                autoLoad: true,
                url: "api/All_PoWorkingImpHdr.php",
                baseParams: {type: "booking_store", dc_cost_id: rec.data.dc_cost2_id, i_yyyy_overlap: Ext.yyyy},
                root: "data",
                idProperty: "id",
                ...
                return new Ext.Window({
                    ...
                    items: new Ext.FormPanel({
                        ...
                        items: [
                            ...
                            {
                                xtype: "textfield",
                                fieldLabel: "จำนวนเงิน ที่จะใช้",
                                name: "f_total_amt_overlap",
                                id: "f_total_amt_overlapID",
                                readOnly: true,
                                value: rec.get("f_total_amt"),
                                listeners: {
                                    blur: function () {
                                        var f_total = parseFloat(this.getValue().replace(/,/g, "") / 1);
                                        this.setValue(Ext.floatRenderer(f_total));
                        ...
                        buttons: [
                            {
                                ...
                                handler: function () {
                                    ...
                                    var msg = "";
                                    console.log(Ext.getCmp("i_yyyy_overlapID").getValue());
                                    // return ;
                                    if (["", 0, null, undefined].includes(Ext.getCmp("c_bookingID").getValue())) {
                                        msg += "<span style='white-space: nowrap;'>- กรุณาเลือกใบกันเหลื่อม</span><br>";
                                    }
                                    if (["ยังไม่ได้ระบุ", "", 0, null, undefined].includes(Ext.getCmp("i_yyyy_overlapID").getValue())) {
                                        msg += "<span style='white-space: nowrap;'>- กรุณาเลือกใบกันเหลื่อม</span><br>";
                                    }
                                    if (msg == "") {
                                        form.submit({
                                            ...
                                            success: function (form, action) {
                                                // Ext.Msg.alert("Success", action.result.msg, function (form, action) {
                                                Ext.c_overlap_close(rec, "c_overlap", Ext.getCmp("c_bookingID").lastSelectionText);
                                            },
                                            failure: function (form, action) {
                                                switch (action.failureType) {
                                                    case Ext.form.Action.CLIENT_INVALID:
                            ...
                            items: [
                                {
                                    ...
                                    handler: function () {
                                        ...
                                        console.log();
                                        Ext.Ajax.request({
                                            url: "http://" + Ext.session.ip_booking + "/procure/po_send",
                                            method: "POST",
                                            params: {
                                                mode: "CONTRACT_ERP",
                                    ...
                                    items: [
                                        {
                                            ...
                                            items: [
                                                ...
                                                },
                                                {
                                                    ...
                                                    name: "mode",
                                                    id: "i_year_enID",
                                                    value: Ext.yyyy, //1 ก่อนจอง 2 จองแล้ว
                                                },
                                                {
                                                    xtype: "hidden",
                                                    name: "dc_expense_budget_type_id",
                                                                    ...
                                                                    if (record.data.sp_check_period != null) {
                                                                        ...
                                                                        Ext.store4.reload({
                                                                            callback: function (rec, operation, success) {
                                                                                if (success) {
                                                                                    console.log(Ext.store4);
                                                                                    console.log(rec[0].get("i_hire_type"));
                                                                                    // i_product_type
                                                                                    // period_i_hire_type
                                                                                    ...
                                                                                    record.set("period_i_hire_type", rec[0].get("i_hire_type"));
                    ...
                    },
                    {
                        ...
                        renderer: function (val, metaData, record, rowIndex, colIndex, store) {
                            ...
                            if (record.data.i_status_overlap == 2) {
                                ...
                                console.log(record.get('c_dc_expense_budget_type_id'));
                                if (([4, 5].includes(record.get('c_dc_expense_budget_type_id')) && record.get('i_pr_type1') != 2)
                                        || ([4, 5].includes(record.get('c_dc_expense_budget_type2_id')) && record.get('i_pr_type2') != 2)
                                        || ([4, 5].includes(record.get('c_dc_expense_budget_type3_id')) && record.get('i_pr_type2') != 3)
                                        ) {
                                    return "แก้ไขแบบจองแบบงวดก่อนจองเงิน";
                                } else {
                                    return "จองเงิน";
                                }
                        ...
                        contextmenu: function (e) {
                            ...
                            var mymenu = new Ext.menu.Menu({
                                items: [{
                                    ...
                                    {
                                        text: "ปิดสัญญา/เบิกงวดสุดท้ายแล้ว",
                                        icon: "../images/icons/book_addresses_key.png",
                                        hidden: Ext.session.dc_center_user == 1 ? false : true,
                                        handler: function (e) {
                                            win_request_end_contract(Ext.selectRow, "end"); // app,data.load
                                        },
                                        scope: this,
                                    },
                                    {
