...
const changePrice = function () {
  ...
  Ext.store.each(function (record, id) {
    if (record.id > 0 && document.getElementById(Ext.objChk[record.id]).checked == true) {
      f_cut = record.get("f_cut").replace(/,/g, "");
      f_select += parseFloat(f_cut, 2);
    }
  });
...
Ext.onReady(function () {
  ...
  Ext.objChk = [];
  /*===============================================*/
  Ext.title_panel = "ตรวจสอบตัดชำระ";

  const Preview = function (id) {
    let url = "http://localhost/nmu/ar/report/Rep_ArCut.php?type=preview&preview_id=" + id;
  ...
  const gridMain = new Ext.grid.GridPanel({
    ...
    columns: [
      ...
      },
      {
        ...
        align: "center",
        width: 50,
        dataIndex: "ar_cut_hdr_id",
        renderer: function (value, metaData, record, row, col, store, gridView) {
          if (record.get("ar_cut_hdr_id") > 0) {
