...
//        exit();
    ...
    case "GENCODECHECKING_MONTHLY":
        ...
        case "GENCODECHECKING":
            ...
            $cf = $db->GetDataBySQL("SELECT c_sub as c_code FROM NMU_DATACENTER.dbo.dc_cost aa WHERE aa.dc_cost_id =(  
            SELECT  dc_cost_id FROM NMU_DATACENTER.dbo.dc_cost WHERE c_code = (SELECT LEFT(c_code,2) + '000000' 
            FROM NMU_DATACENTER.dbo.dc_cost WHERE dc_cost_id = ?))", array($_REQUEST['dc_cost_origin_id']));
            $ap_code = $arr_gen_code["c_code"];
            $c_code = $arr_gen_code["c_code"]."/".$cf;
            $i_product  = $_REQUEST["i_product_type"]>0?1:0; //สนแค่มีของ 
            $i_product_type = $_REQUEST["i_product_type"]??null; //สนแค่มีของs
            $ref_id = $arr_gen_code["sp_check_period_hdr_id"];
            unset($arrValue);
        if ($ret_id == $ref_id) {
            //อัพเดท
            $sql2 = "UPDATE {$table} SET i_menu = 2 ,c_code= '{$c_code}', ap_code = '{$ap_code}'  ,i_register_status = {$i_product}, i_register =  {$i_product_type} , sp_emp_id   = {$_SESSION['sp_emp_id']} WHERE {$keyName} = ?;";
            //update code
