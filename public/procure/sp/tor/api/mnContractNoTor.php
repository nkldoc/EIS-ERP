...
switch ($mode) {
    ...
    case "UPDATENEXTSTEP":
        ...
        unset($data);
        unset($arrValue);
        $data["tor_status_id"] = $_REQUEST["tor_status_id"];
        ...
        unset($data);
        unset($arrValue);
        $data["i_contract_status"] = $_REQUEST["i_contract_status"];
        foreach ($data as $fldA => $value) {
            $arrValue[]    = ($value != "") ? $value : null;
            $addField    .= ",
            {$fldA} = ?";
        ...
        $sql        = "UPDATE sp_tor_contract SET " . substr($addField, 1) . " WHERE sp_tor_id = ? ";
        $arrValue[] = $_REQUEST["id"];
        $stmt   = $db->QueryParam($sql, $arrValue);
        break;
    case "UP_CONTRACT_NO_TOR":
        ...
        if (@$_REQUEST["i_is_bank_warranty1"] == 1) {
            ...
            $data["i_yyyy_overlap"] = $_REQUEST["i_yyyy_overlap"];
            $data["c_overlap"] = $_REQUEST["c_booking"]??null;
            $data["bg_budget_dtl_overlap_id"] = $_REQUEST["bg_budget_dtl_overlap_id"]??null;
...
//            exit(); 
        ...
        if (@$_REQUEST["i_is_bank_warranty1"] == 1) {
            ...
            $data["i_yyyy_overlap"] = $_REQUEST["i_yyyy_overlap"];
            $data["c_overlap"] = $_REQUEST["c_overlap"]??null;
            $data["bg_budget_dtl_overlap_id"] = $_REQUEST["bg_budget_dtl_overlap_id"]??null;
            $data["i_overlap"] = $_REQUEST["i_overlap"];
            $data["dc_cost_id"] = $_SESSION["dc_cost_id"];
            $data["sp_emp_id"] = $_SESSION["sp_emp_id"];
            $data["c_code"] = $_REQUEST["c_code"];
            $data["dc_user_create_id"] = $_SESSION["user_id"];
