<?php
include("../api/List_RepDcExpenseAccVSN.php");
...
$caption	= "รายงานรายการจ่ายบัญชี (Vision Net)";

if($_REQUEST["type"] == "excel") { $export->headerExcel($caption); }

$thead[]	= "ที่";
$thead[]	= "หมวดรายจ่าย"; 
$thead[]	= "หมวดรายจ่ายย่อย"; 
$thead[]	= "รหัส"; 
$thead[]	= "รหัสเดิม"; 
$thead[]	= "ชื่อ"; 
$thead[]	= "ผังบัญชี";
$thead[]	= "ผังบัญชี (เหลื่อมปี)";
$thead[]	= "สถานะ";
...
if( is_array($data_dtl) && count($data_dtl["data"]) > 0 ) {
	...
	foreach($data_dtl["data"] as $index => $jObj) {
		...
		$style		= " nowrap ";
		$style		.= (($jObj["the_dc_acc_id"]=="0") || ($jObj["the_dc_acc_id_overlap"]=="0")) ? " bgcolor=#FFFF99" : "";
		...
		$tbody	.= "<td ".$style." align='left'>".$jObj["c_group_full"]."</td>";
		$tbody	.= "<td ".$style." align='left'>".$jObj["c_sub_group_full"]."</td>";
		$tbody	.= "<td ".$style." align='center'>".$jObj["c_code"]."</td>"; 
		$tbody	.= "<td ".$style." align='center'>".$jObj["c_map_code"]."</td>";
		$tbody	.= "<td ".$style.">".$jObj["c_name"]."</td>"; 
		$tbody	.= "<td ".$style.">".$jObj["c_acc_full"]."</td>"; 
		$tbody	.= "<td ".$style.">".$jObj["c_acc_full_overlap"]."</td>"; 
