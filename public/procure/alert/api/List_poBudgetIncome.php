...
if ($_REQUEST["type"] == "po_budget_income_hdr") {
	...
	$sqlMain = "
        ...
        FROM #TemData a
			INNER JOIN dbo.po_budget_income_hdr b ON a.po_budget_income_hdr_id = b.po_budget_income_hdr_id
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"								=> $row["numrow"],
				"id"								=> $row["po_budget_income_hdr_id"],
				"i_year"							=> $row["i_year"],
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"										=> $row["numrow"],
				"id"										=> $row["po_budget_income_dtl_id"],
