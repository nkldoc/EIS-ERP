...
//===========================================
$reportColumn = 12;
$dataAssetGroup = $db->GetDataBySQL("select c_code, c_name, asset_type from dc_asset_type where c_code = ?", array($asset_type));
...
$data_hdr["4"]	= "<tr bgcolor=#A5BAD6>"
                ...
                ."              <th width=10% style=\"{$css_gl_top_bottom_left}\" nowrap>ยี่ห้อ</th>"
...
if ($cal_line == 0)
{
    $data_hdr["2"] = "<td colspan=3 nowrap align=right style=\"{$css_gl_information}\">หน้า 1 / 1</td></tr>";
    $data .= $data_hdr["0"].$data_hdr["1"].$data_hdr["2"].$data_hdr["3"].$data_hdr["4"]; 
}
