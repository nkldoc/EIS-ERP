...
###########################################
        ...
        $sqlMain = "SELECT a.sp_tor_dtl_period_id
            , a.sp_tor_hdr_period_id
            , a.sp_tor_dtl_id
            , a.i_qty AS i_qty_all
            ,(
            SELECT sum(b.i_qty) FROM sp_check_period_dtl b
            inner join sp_check_period_hdr sh on sh.sp_check_period_hdr_id = b.sp_check_period_hdr_id
                WHERE sh.i_enabled=1 and  b.sp_tor_dtl_period_id = a.sp_tor_dtl_period_id and b.sp_tor_hdr_period_id = a.sp_tor_hdr_period_id
            ) as i_qty_used
            ...
            , a.f_net_unit_price
            , a.f_net_total_price
            , d.f_vat_amt ,sum(d.f_total_add_vat_amt) as f_total_add_vat_amt,d.f_rate_vat
            ...
            and (select sp_victor_id from sp_tor_victory where d.sp_tor_bidder_dtl_id =  sp_tor_bidder_dtl_id ) > 0
            ...
            WHERE a.sp_tor_hdr_period_id = ? and a.i_enabled = 1 and  isnull(e.dc_bg_budget_type_id,a.dc_bg_budget_type_id) = ?
            GROUP BY  a.sp_tor_dtl_period_id, a.sp_tor_hdr_period_id, a.sp_tor_dtl_id,
                a.i_qty, a.dc_unit_type_id, a.c_name,a.dc_bg_budget_type_id,
                a.dc_creditor_id, a.f_net_unit_price, a.f_net_total_price,
                a.po_expense_id, a.i_hire_type, a.i_is_inv, a.i_product_type,
                d.f_vat_amt, d.f_rate_vat
            ";
//    echo $sqlMain;
//    exit();
...
// }
        $sqlMain = "SELECT a.sp_tor_contract_id
            ...
            from ( select
                        ...
                        left join sp_check_period_hdr b on a.sp_tor_hdr_period_id  = b.sp_tor_hdr_period_id
                        left join sp_check_billing_items c on b.sp_check_period_hdr_id = c.sp_check_period_hdr_id
                        left join sp_withdraw d on d.sp_check_period_hdr_id = b.sp_check_period_hdr_id
...
//set start
        ...
        $data["sp_emp_id"] = $_REQUEST["sp_emp_id"] ?? null;
