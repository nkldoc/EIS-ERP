...
function headerX($t='',$rd){
    ...
    $title= 'รายงานสรุปแจ้งประกันภัยสินทรัพย์(ตามรายการสินทรัพย์)';
...
{
    $str .= genReportByType($row["dc_ins_method_id"], $dc_building_id, $dc_ins_town_hdr_id, $row["c_name"], $txtBuild, $txtTownHdr, $i_month, $i_year);
}// end while
//-------------------------------------
...
function genReportByType($i_type, $dc_building_id, $dc_ins_town_hdr_id, $type_name, $txtBuild, $txtTownHdr, $i_month, $i_year)
{
    $db 	= new DatabaseServer();
    $date 	= new i_date();
    ...
    //============ gen where ===========
    $c_yyyy_mm1 = sprintf("%04d%02d", $i_year, $i_month);
    $c_yyyy_mm2 = sprintf("%04d%02d", $i_year, 12);
    ...
    $sqlGetMoney = "select d.am_tran_rg_dtl_id
                        , c.i_is_ins as dc_ins_group_id 
                        , isnull(d.c_cost_asset,0)-isnull(d.f_depreciate,0) as acc_cost
                    from am_ins_hdr a
    ...
    $str = '<table cellspacing="0" cellpadding="0" width="100%" border="0" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
            ...
            <table cellspacing="0" cellpadding="3" width="100%" border="1" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
                <tr bgcolor="#A5BAD6">
                    ...
                    <th align="center" rowspan="2" nowrap><b>สถานที่ใช้งาน/ประเภทครุภัณฑ์</b></th>
                    <th align="center" rowspan="2" nowrap><b>ยี่ห้อ/Serial No</b></th>
                    <th align="center" rowspan="2" nowrap><b>รุ่น-แบบ</b></th>
                    <th align="center" rowspan="2" nowrap><b>วันที่ได้มา</b></th>
    ...
    $sql = "select d.c_name as town_name
                , (select c_code+' '+c_name from dc_cost where dc_cost_id = b.dc_cost_id) as cost_name
                , e.am_tran_rg_dtl_id
                , e.c_name as dc_name
                , e.c_brand
                , e.c_serial
                , e.c_model
                , isnull(convert(varchar(10), e.d_receive_date,120), '') as d_receive_date
            from am_ins_hdr a
                ...
                inner join am_tran_rg_dtl e on c.am_tran_rg_dtl_id = e.am_tran_rg_dtl_id
            where a.i_enable= ? 
                and b.i_enable= ? 
                and d.i_enable= ?
                and a.i_is_method= ?
                {$sqlWhere}
            order by town_name, cost_name";
    ...
    while ($row = $db->Fetch($stmt))
    {
        if ($tempTown != $row["town_name"])
        {
            $str .= "<tr bgcolor='#f0f4fa'>"
                    ."<td colspan='{$columnAll}'>ชื่ออาคาร : {$row["town_name"]}</td>"
                 ."</tr>";
            $tempTown = $row["town_name"];
        }// end if 
        
        if ($tempCost != $row["cost_name"])
        {
            $str .= "<tr bgcolor='#E2E8E9'>"
                    ."<td colspan='{$columnAll}'>หน่วยงาน : {$row["cost_name"]}</td>"
                 ."</tr>";
            $tempCost = $row["cost_name"];
        }// end if 
        ...
        $str .= "<tr>"
                ...
                ."<td align='left' nowrap>&nbsp;{$row["dc_name"]}</td>"
                . "<td align='left' nowrap>&nbsp;{$row["c_brand"]}/{$row["c_serial"]}</td>"
                . "<td align='left' nowrap>&nbsp;{$row["c_model"]}</td>"
