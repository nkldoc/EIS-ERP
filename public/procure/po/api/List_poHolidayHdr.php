...
if ($_REQUEST["type"] == "po_holiday_hdr") {
	...
	$sqlMain = "
        ...
        FROM #TemData a
            INNER JOIN dbo.po_holiday_hdr b ON a.po_holiday_hdr_id = b.po_holiday_hdr_id
	...
	$sqlMain = "
		...
		SELECT
			...
			,a.c_comment
		INTO #TemData
		FROM dbo.po_holiday_dtl a
		WHERE a.po_holiday_hdr_id = ?;
	...
	if (sqlsrv_has_rows($stmt)) {
		while ($row = $db->Fetch($stmt)) {
			$temp = array(
				"no"										=> $row["numrow"],
				"id"										=> $row["po_holiday_dtl_id"],
				"po_holiday_hdr_id"							=> $row["po_holiday_hdr_id"],
				"c_name"									=> $row["c_name"],
