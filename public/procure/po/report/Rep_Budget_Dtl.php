...
if (is_array($data_dtl) && count($data_dtl["data"]) > 0) {
	...
	foreach ($data_dtl["data"] as $index => $jObj) {
		...
		if ($jObj["i_type"] == 1) {
			...
			$span = ($jObj["i_success"] == 1) ? "" : "<font color=red> *</font>";

			// GEN TBODY
			$tbody .= "<tr>";
			$tbody .= "<td style='" . $style . "' align='center'>" . $jObj["no"] . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='center'>" . $jObj["c_code_ref"] . $span . "</td>";
			$tbody .= "<td style='mso-number-format:\@;" . $style . "' align='center'>" . $jObj["c_approve"] . "</td>";
			$tbody .= "<td style='" . $style . "' align='left'>" . $jObj["budget_name"] . "</td>";
			$tbody .= "<td style='" . $style . "' align='left'>" . $jObj["c_name_lv4"] . "</td>";
			$tbody .= "<td style='" . $style . "' align='right'>" . number_format($jObj["f_total"], 2)  . "</td>";
			$tbody .= "</tr>";
		} else {
			$style = "background-color:#EEE;";
			// GEN TBODY
			$tbody .= "<tr>";
			$tbody .= "<td colspan=5 style='" . $style . "' align='right'><b>{$jObj["c_name"]}</b></td>";
	...
	<div class="outer">
		...
		<div style="position: relative; font-size: 11px; margin: 5px 10px;">
			<div style='position: relative; left: 2px;'>
				<font color=red>*</font> ยังไม่ทำทะเบียนจ่าย
			</div>
		</div>
			...
			<table width="100%" class="table_report" border="0" cellspacing="1" cellpadding="0">
				<thead valign="top">
					<tr>
						...
						<th style="vertical-align:middle;" nowrap>เลขที่ใบขอเบิก</th>
						<th style="vertical-align:middle;" nowrap>เลขที่ฏีกา</th>
