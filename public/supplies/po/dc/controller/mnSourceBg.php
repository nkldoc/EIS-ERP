...
$mode    = $_REQUEST[ "mode" ] ;
$table   = "po_bg_type_hdr" ;
$keyName = "po_bg_type_hdr_id" ;
...
//        $arrParam[] = $data[ "i_config" ] ;
        ...
        $sql = "insert into {$table} (c_code, c_name, i_seq
                                        , i_parent, c_comment, i_enable
    ...
    case "EDIT" :
        ...
        $sql        = "UPDATE {$table} SET  c_name = ? , i_seq = ? , c_comment = ? , i_enable = ? , dc_user_update_id = ?, dc_user_update_cost_id = ? , d_update = ? WHERE {$keyName} = ?" ;
        $arrParam[] = $data[ "id" ] ;
        $stmt       = $db -> QueryParam ( $sql , $arrParam ) ;
        /* @ log update */
        $db -> logs ( "po-DcPuStatus"
