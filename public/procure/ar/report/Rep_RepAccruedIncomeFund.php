<?php
include("../api/List_RepAccruedIncomeFund.php");
...
$caption	= "รายงาน รายได้ค้างรับ (แยกตามกองทุน)";
...
if (is_array($data_dtl) && count($data_dtl["data"]) > 0) {
	...
	foreach ($data_dtl["data"] as $index => $jObj) {
		...
		if ($jObj["i_type"] == 1) {
			...
			$a = "<a href='./Rep_RepAccruedIncomeFund_DTL.php' target='Rep_RepAccruedIncomeFund_DTL'>" . $jObj["c_fund"] . "</a>";

			$tbody	.=	"<tr>";
			$tbody	.= "<td nowrap align='left'>{$a}</td>";
			$tbody	.= "<td nowrap align='left'>" . $jObj["c_name"] . "</td>";
			$tbody	.= "<td nowrap align='center'>" . number_format($jObj["i_hn"], 0) . "</td>";
		...
		// }
			...
			$tbody	.= "<tr>";
			$tbody	.= "<td {$style} colspan=2 nowrap align='right'><b>รวมรายได้ค้างรับ {$jObj["c_fund"]}</b></td>";
			$tbody	.= "<td nowrap align='center' {$style}><b>" . number_format($jObj["i_hn"], 0) . "</b></td>";
			$tbody	.= "<td nowrap align='right' {$style}><b>" . number_format($jObj["f_charge"], 2) . "</b></td>";
			$tbody	.= "</tr>";
		}
	}
	$tbody	.= "</tbody>";
...
<body>
	...
	<table width=100% class="table_report" border="0" cellspacing="1" cellpadding="0" style="page-break-after: always;">
		<thead valign="top">
			<tr>
				<th nowrap rowspan=2 style='vertical-align:middle;'>กองทุน</th>
				<th nowrap rowspan=2 style='vertical-align:middle;'>สิทธิการรักษา</th>
				<th nowrap colspan=2 style='vertical-align:middle;'>รวม</th>
			</tr>
			<tr>
				<th nowrap style='vertical-align:middle;'>ผู้ป่วย/คน</th>
