...
if ($_REQUEST["type"] == "ar_cut") {
	...
	$sqlMain = "	
		...
		SELECT
			a.ar_cut_hdr_id
			,ROW_NUMBER() OVER (PARTITION BY a.c_code_cut ORDER BY a.c_code_cut, a2.ar_treat_right_group_name DESC) AS i_show
			,a.c_code_cut
		...
		WHERE b.i_status IN (0, 1)
			AND 0 = (SELECT DISTINCT i_success_approve FROM dbo.ar_cut_log aa WHERE aa.d_action_date BETWEEN CONVERT(DATETIME,'{$_REQUEST["d_start"]}',102) AND CONVERT(DATETIME,'{$_REQUEST["d_end"]}'+' 23:59:59',102))
			{$con}
		GROUP BY
			a.ar_cut_hdr_id
			,a.c_code_cut
		...
		SELECT * INTO #TemData FROM (
			...
			SELECT
				...
				,SUM(ISNULL(a.f_cut,0)) AS f_cut
				,a.i_status
			FROM #temp_data a
			GROUP BY a.ar_cut_hdr_id, a.c_code_cut, a.i_status
		) aa;
		
		SELECT * FROM #TemData ORDER BY i_status, c_code_cut, i_type, i_show;
	...
	if (sqlsrv_has_rows($stmt)) {
		...
		while ($row = $db->Fetch($stmt)) {
			...
			$temp = array(
				"no"								=> ++$numrow,
				"id"								=> ($row["i_type"] == 2 && $row["i_status"] != 1) ? $row["ar_cut_hdr_id"] : "",
				"ar_cut_hdr_id"						=> ($row["i_type"] == 2) ? $row["ar_cut_hdr_id"] : "",
				"preview_id"						=> $row["i_show"] == 1 ? $row["ar_cut_hdr_id"] : "",
				"i_type"							=> $row["i_type"],
				"c_code_cut"						=> ($row["i_show"] == 1) ? $row["c_code_cut"] : "",
				"d_cut_date"						=> ($row["i_show"] == 1) ? $date->extDateBuddha($row["d_cut_date"]) : "",
	...
	$sqlMain = "
		SET NOCOUNT ON	
		SELECT
			ISNULL(SUM(ISNULL(c.f_cut,0)),0) AS f_cut
		FROM dbo.ar_cut_hdr a
			INNER JOIN dbo.ar_cut_dtl b ON a.ar_cut_hdr_id = b.ar_cut_hdr_id
