...
function List_QueryParam() {
	...
	if ($limit=="") { $limit 	= 20; }else{ $limit=($limit+$start); }
   	...
   	if ($i_type_report==2) // งบทดลอง (ตามศูนย์ต้นทุน)
 		$i_is_post			=  " AND b.i_is_post=".BOOK_ACC_GL;
 	else if ($i_type_report==1)  //รายงานยอดคงเหลือบัญชีแยกประเภท (ตามศูนย์ต้นทุน)
...
$sqlMain = "DECLARE @start int = ?;
			...
			GROUP BY b.cost_code, b.cost_name;
			...
			ORDER BY cost_code,acc_code_parent, acc_code, i_type;
