Ext.onReady(function () {
    Ext.QuickTips.init();
    /*===============================================*/
    Ext.title_panel = "PR  <font color=red>*(Admin Only)</font>";
    /*===============================================*/
    // pagingBar
            ...
            if (record.data.c_booking != null) {
                Ext.bg_budget_overlap.load({
                    ...
                    callback: function (records, operation, success) {
                        ...
                        document.getElementById(Ext.getCmp("dis_dc_expense_budget_type_name").label.id).innerHTML = "แหล่งเงิน : " + record_overlap.get("dc_expense_budget_type_id") + " : ";
            ...
            if (record.data.c_booking == null && (record.data.i_sav_by_sys == 5 || Ext.select_row.i_sav_by_sys == 4)) {
                Ext.bg_reserve_money.load({
                    ...
                    callback: function (records, operation, success) {
                        ...
                        document.getElementById(Ext.getCmp("dis_re_dc_expense_budget_type_name").label.id).innerHTML = "แหล่งเงิน : " + record_reserve.get("dc_expense_budget_type_id") + " : ";
    ...
    ///
    var setEabled = (id, val, url) => {
        console.log(id, val);
        console.log(url, url);
        Ext.Ajax.request({
            url: url, // เปลี่ยนเป็น URL ของไฟล์ PHP คุณ
            method: 'POST',
            params: {
                mode: 'SETDISABLED',
                id: id, // ไอดีของแถวที่ต้องการอัปเดต
                i_enabled: val               // ตัวอย่าง: ส่ง 0 เพื่อปิดใช้งาน (SETDISABLED)
            },
            success: function (response, opts) {
                // ทำงานเมื่ออัปเดตสำเร็จ เช่น สั่ง store.reload() เพื่ออัปเดตหน้า Grid
                Ext.Msg.alert('สำเร็จ', 'บันทึกการเปลี่ยนสถานะเรียบร้อยแล้ว');
                Ext.store.reload();
            },
            failure: function (response, opts) {
                Ext.Msg.alert('ล้มเหลว', 'เกิดข้อผิดพลาดของระบบ: ' + response.status);
            }
        });
    ...
    rowContextmenu = function (grid, rowIndex, e) {
        ...
        if (record) {
//            D:\ERP\nmu_supplies\src\main\webapp\sp\adminupdatepr\FormGrid.js
            new Ext.menu.Menu({
                items: [{
                        ...
                        handler: function (e) {
                            ...
                            setEabled(record.data.id
                                    , record.data.i_enabled === 1 ? 2 : 1
                                    , './adminupdatepr/php/mn_spWorkingAdvancedEdit.php'
                                    );
                        },
                    },
                    {
    ...
    const gridMain = new Ext.grid.GridPanel({
        ...
        viewConfig: {
            ...
            getRowClass: function (record, rowIndex, rowParams, store) {
                ...
                return ''; // ถ้าเป็น 1 ให้แสดงผลปกติ
            },
        },
        tbar: [
            {
