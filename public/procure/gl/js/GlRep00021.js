Ext.onReady(function() {
  Ext.QuickTips.init();

  /* =============================================== */
  title_panel = "งบทดลอง (ตามบัญชี - ช่วงเวลา)";
  /* =============================================== */

  var dc_cost_all = new Ext.data.JsonStore({
    autoLoad: true,
    url: "api/ALL_GlRep00021.php",
    baseParams: { type: "dc_cost", show: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name"]
  });

  var store_acc_all_parent = new Ext.data.JsonStore({
    autoLoad: true,
    url: "api/ALL_GlRep00021.php",
    baseParams: { type: "dc_acc_main", show: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name", "cut_name"]
  });

  var store_acc_all_parent_lv5 = new Ext.data.JsonStore({
    autoLoad: true,
    url: "api/ALL_GlRep00021.php",
    baseParams: { type: "dc_acc_main_lv5", show: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name", "cut_name"]
  });

  var store_acc_all = new Ext.data.JsonStore({
    autoLoad: true,
    url: "api/ALL_GlRep00021.php",
    baseParams: { type: "dc_acc", show: "all" },
    root: "data",
    idProperty: "id",
    fields: ["id", "c_name", "cut_name"]
  });

  var store_month = new Ext.data.JsonStore({
    fields: ["id", "c_name"],
    data: [
      { id: "01", c_name: "มกราคม" },
      { id: "02", c_name: "กุมภาพันธ์" },
      { id: "03", c_name: "มีนาคม" },
      { id: "04", c_name: "เมษายน" },
      { id: "05", c_name: "พฤษภาคม" },
      { id: "06", c_name: "มิถุนายน" },
      { id: "07", c_name: "กรกฎาคม" },
      { id: "08", c_name: "สิงหาคม" },
      { id: "09", c_name: "กันยายน" },
      { id: "10", c_name: "ตุลาคม" },
      { id: "11", c_name: "พฤศจิกายน" },
      { id: "12", c_name: "ธันวาคม" }
    ]
  });

  // storeYear
  var years = [];
  var currentTime = new Date();
  var now = currentTime.getFullYear() + 1;
  var yy_en = Ext.START_YEAR_ACC;
  while (yy_en <= now) {
    years.push({ id: yy_en, c_name: yy_en + 543 });
    yy_en++;
  }

  store_year = new Ext.data.JsonStore({
    fields: ["id", "c_name"],
    data: years
  });

  LookReport = function(type) {
    var msg = "";

    var dc_cost_id = "";
    var dc_acc_id_parent = "";
    var dc_acc_id_parent_lv5 = "";
    var dc_acc_id = "";

    if (Ext.getCmp("dc_cost_id").getValue() == "") {
      msg += "- กรุณาเลือก หน่วยงาน อย่างน้อย 1 รายการ<br>";
    } else {
      dc_cost_id = Ext.getCmp("dc_cost_id").getValue();
    }
 
    if (Ext.getCmp("i_show_acc").getValue().inputValue == 1) {
      if (Ext.getCmp("dc_acc_id_parent").getValue() == "") {
        msg += "- กรุณาเลือก บัญชีคุม Lv4 อย่างน้อย 1 รายการ<br>";
      } else {
        dc_acc_id_parent = Ext.getCmp("dc_acc_id_parent").getValue();
      }
    } else if (Ext.getCmp("i_show_acc").getValue().inputValue == 3) {
      if (Ext.getCmp("dc_acc_id_parent_lv5").getValue() == "") {
        msg += "- กรุณาเลือก บัญชีคุม Lv5 อย่างน้อย 1 รายการ<br>";
      } else {
        dc_acc_id_parent_lv5 = Ext.getCmp("dc_acc_id_parent_lv5").getValue();
      }
    } else if (Ext.getCmp("i_show_acc").getValue().inputValue == 2) {
      if (Ext.getCmp("dc_acc_id").getValue() == "") {
        msg += "- กรุณาเลือก บัญชีย่อยอย่างน้อย 1 รายการ<br>";
      } else {
        dc_acc_id = Ext.getCmp("dc_acc_id").getValue();
      }
    }

    if (msg == "") {
      var href = "report/Rep_GlRep00021.php";
      var resultUrl = "";

      var c_mm1 =
        Ext.getCmp("month")
          .getValue()
          .toString().length > 1
          ? Ext.getCmp("month").getValue()
          : "0" +
            Ext.getCmp("month")
              .getValue()
              .toString();
      var c_mm2 =
        Ext.getCmp("month2")
          .getValue()
          .toString().length > 1
          ? Ext.getCmp("month2").getValue()
          : "0" +
            Ext.getCmp("month2")
              .getValue()
              .toString();

      resultUrl += "&type=" + type;
      resultUrl += "&month=" + c_mm1;
      resultUrl += "&month2=" + c_mm2;
      resultUrl += "&year=" + Ext.getCmp("year").getValue();
      resultUrl += "&dc_cost_id=" + dc_cost_id;
      resultUrl += "&i_show_cost=" + (Ext.getCmp("i_show_cost").getValue() ? 1 : 2);
      resultUrl += "&i_show=" + Ext.getCmp("i_show").getValue().inputValue;
      resultUrl += "&i_show_acc=" + Ext.getCmp("i_show_acc").getValue().inputValue;
      resultUrl += "&dc_acc_id_parent=" + dc_acc_id_parent;
      resultUrl += "&dc_acc_id_parent_lv5=" + dc_acc_id_parent_lv5;
      resultUrl += "&dc_acc_id=" + dc_acc_id;

      resultUrl = resultUrl != "" ? "?" + resultUrl.substring(1) : "";

      window.open(href + resultUrl, href);
      window.focus();
    } else {
      Ext.MessageBox.alert("แจ้งเตือน", msg);
    }
  };

  var panelForm = new Ext.Panel({
    region: "center",
    title: title_panel,
    border: false,
    stripeRows: true,
    loadMask: true,
    items: [
      {
        xtype: "form",
        frame: true,
        labelAlign: "right",
        labelWidth: 200,
        bodyStyle: { padding: "10px 20px" },
        defaults: {
          anchor: "100%",
          msgTarget: "side",
          allowBlank: false
        },
        items: [
          {
            xtype: "container",
            layout: "hbox",
            align: "stretch",
            RemoveHeight: true,
            defaults: {
              xtype: "fieldset",
              flex: 1,
              margins: "0px 3px",
              autoHeight: true
            },
            items: [
              {
                title: "เมนู " + title_panel,
                RemoveCls: "x-box-item",
                defaults: {
                  labelStyle: "width:200px;",
                  allowBlank: true
                },
                items: [
                  new Ext.form.ComboBox({
                    id: "month",
                    fieldLabel: "เดือน",
                    width: 150,
                    mode: "local",
                    store: store_month,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    emptyText: "กรุณาเลือก...",
                    value: ("00" + (new Date().getMonth() + 1)).substr(-2),
                    listeners: {
                      change: function(combo, newValue) {
                        if (newValue == "") {
                          combo.reset();
                        }
                      },
                      beforequery: function(q) {
                        if (q.query) {
                          var length = q.query.length;
                          q.query = new RegExp(Ext.escapeRe(q.query));
                          q.query.length = length;
                        }
                      },
                      blur: function() {
                        this.getStore().clearFilter();
                      }
                    }
                  }),
                  new Ext.form.ComboBox({
                    id: "month2",
                    fieldLabel: "ถึง",
                    width: 150,
                    mode: "local",
                    store: store_month,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    emptyText: "กรุณาเลือก...",
                    value: ("00" + (new Date().getMonth() + 1)).substr(-2),
                    listeners: {
                      change: function(combo, newValue) {
                        if (newValue == "") {
                          combo.reset();
                        }
                      },
                      beforequery: function(q) {
                        if (q.query) {
                          var length = q.query.length;
                          q.query = new RegExp(Ext.escapeRe(q.query));
                          q.query.length = length;
                        }
                      },
                      blur: function() {
                        this.getStore().clearFilter();
                      }
                    }
                  }),
                  new Ext.form.ComboBox({
                    id: "year",
                    fieldLabel: "ปีงบประมาณ",
                    width: 150,
                    mode: "local",
                    store: store_year,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    emptyText: "กรุณาเลือก...",
                    value: new Date().getFullYear(),
                    listeners: {
                      change: function(combo, newValue) {
                        if (newValue == "") {
                          combo.reset();
                        }
                      },
                      beforequery: function(q) {
                        if (q.query) {
                          var length = q.query.length;
                          q.query = new RegExp(Ext.escapeRe(q.query));
                          q.query.length = length;
                        }
                      },
                      blur: function() {
                        this.getStore().clearFilter();
                      }
                    }
                  }),
                  {
                    xtype: "compositefield",
                    fieldLabel: "หน่วยงาน",
                    anchor: "100%",
                    msgTarget: "under",
                    items: [
                      new Ext.ux.form.LovCombo({
                        id: "dc_cost_id",
                        fieldLabel: "หน่วยงาน",
                        width: 500,
                        mode: "local",
                        store: dc_cost_all,
                        valueField: "id",
                        displayField: "c_name",
                        triggerAction: "all",
                        forceSelection: true,
                        selectOnFocus: true,
                        typeAhead: false,
                        emptyText: "กรุณาเลือก..."
                      }),
                      {
                        xtype: "checkbox",
                        id: "i_show_cost",
                        boxLabel: "แสดงยอดเงินหน่วยงาน",
                        inputValue: 1,
                        checked: true
                      }
                    ]
                  },
                  {
                    xtype: "radiogroup",
                    id: "i_show_acc",
                    fieldLabel: "รายการบัญชี",
                    columns: [90, 90, 100,100],
                    items: [
                      {
                        boxLabel: "บัญชีคุม Lv4",
                        name: "i_show_acc",
                        inputValue: 1,
                        checked: true
                      },
                      {
                        boxLabel: "บัญชีคุม Lv5",
                        name: "i_show_acc",
                        inputValue: 3
                      },
                      {
                        boxLabel: "บัญชีย่อย",
                        name: "i_show_acc",
                        inputValue: 2
                      },
                      {
                        boxLabel: "ทั้งหมด",
                        name: "i_show_acc",
                        inputValue: 9
                      }
                    ],
                    listeners: {
                      change: function(obj, value) {
                        if (value.inputValue == 1) {
                          Ext.getCmp("dc_acc_id").hide();
                          Ext.getCmp("dc_acc_id_parent").show();
                          Ext.getCmp("dc_acc_id_parent_lv5").hide();
                        } else if (value.inputValue == 3) {
                          Ext.getCmp("dc_acc_id").hide();
                          Ext.getCmp("dc_acc_id_parent").hide();
                          Ext.getCmp("dc_acc_id_parent_lv5").show();
                        } else if (value.inputValue == 9) {
                          Ext.getCmp("dc_acc_id").hide();
                          Ext.getCmp("dc_acc_id_parent").hide();
                          Ext.getCmp("dc_acc_id_parent_lv5").hide();
                        }else {
                          Ext.getCmp("dc_acc_id").show();
                          Ext.getCmp("dc_acc_id_parent").hide();
                          Ext.getCmp("dc_acc_id_parent_lv5").hide();
                        }
                      }
                    }
                  },
                  new Ext.ux.form.LovCombo({
                    id: "dc_acc_id_parent",
                    fieldLabel: "รายการบัญชีคุม Lv4",
                    width: 500,
                    mode: "local",
                    store: store_acc_all_parent,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    emptyText: "กรุณาเลือก..."
                  }),
                  new Ext.ux.form.LovCombo({
                    id: "dc_acc_id_parent_lv5",
                    fieldLabel: "รายการบัญชีคุม Lv5",
                    width: 500,
                    mode: "local",
                    store: store_acc_all_parent_lv5,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    hidden: true,
                    emptyText: "กรุณาเลือก..."
                  }),
                  new Ext.ux.form.LovCombo({
                    id: "dc_acc_id",
                    fieldLabel: "รายการบัญชีย่อย",
                    width: 700,
                    mode: "local",
                    store: store_acc_all,
                    valueField: "id",
                    displayField: "c_name",
                    triggerAction: "all",
                    forceSelection: true,
                    selectOnFocus: true,
                    typeAhead: false,
                    hidden: true,
                    emptyText: "กรุณาเลือก..."
                  }),
                  {
                    xtype: "radiogroup",
                    id: "i_show",
                    fieldLabel: "รูปแบบแสดงข้อมูล",
                    columns: [80, 110],
                    items: [
                      {
                        boxLabel: "ทุกรายการ",
                        name: "i_show",
                        inputValue: 1
                      },
                      {
                        boxLabel: "ยอดคงเหลือ",
                        name: "i_show",
                        inputValue: 2,
                        checked: true
                      }
                    ]
                  }
                ]
              }
            ]
          }
        ],
        buttonAlign: "left",
        buttons: [
          {
            text: Ext.GLOBAL_BU_SHOW_TH + "สำหรับ HTML",
            iconCls: "page_magnify",
            handler: function() {
              LookReport("html");
            } // End Handle
          },
          {
            text: Ext.GLOBAL_BU_SHOW_TH + "สำหรับ Excel",
            iconCls: "icon-excel",
            handler: function() {
              LookReport("excel");
            } // End Handle
          }
        ]
      }
    ]
  }); // panelForm

  /* ====================== CENTER ====================== */
  var center = new Ext.TabPanel({
    region: "center",
    border: false,
    activeTab: 0, // default Tab
    id: "contenterCenter",
    defaults: { autoScroll: true },
    items: [panelForm]
  });

  /* ====================== RENDER ====================== */
  new Ext.Viewport({
    layout: "border",
    items: [center]
  });
});
