...
if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="SEARCH")
{
        ...
        $wh = "isnull({$table}.i_is_status,0)=1"
                . " AND ISNULL({$table}.i_enable,".STATUS_DISABLE.") =?"
...
/*         $arrParam[]       = $d_begin_dateID;
        ...
        $arrCountParam[]  = $d_end_dateID; */
		 ...
		 if(isset($_REQUEST['dc_debtor_id']) && $_REQUEST['dc_debtor_id']>0){
			...
			$wh .=" and dc_debtor_id =?";
			$arrParam[] =$_REQUEST['dc_debtor_id'];	
			$arrCountParam[] =$_REQUEST['dc_debtor_id']; 
		 }
 
}else{  
 
    
        $wh = "isnull({$table}.i_is_status,0)=1 and isnull({$table}.i_parent,0)=0 and ISNULL({$table}.i_enable,".STATUS_DISABLE.") =?"; // Search No Enable = 1  
