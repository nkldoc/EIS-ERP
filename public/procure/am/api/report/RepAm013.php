...
$sql = "select d.c_code as cost_code
                ...
                , a.c_code_gen
                , a.c_code as ccode
                , isnull(convert(varchar(10), a.d_date_chg, 120), '')  as d_date_chg
                ...
                , case when a.dc_cost_id_new is not null then (select c_name from dc_cost where dc_cost_id = a.dc_cost_id_new) else '&nbsp;' end as new_cost_name
                , a.c_comment
        from am_tf_hdr a 
        ...
        where  a.c_code_gen!='TA' 
                and a.c_code_gen like 'TA%'  
                and  (d_date_chg between convert(datetime,? ,102) and convert(datetime,? ,102)) 
                {$sqlWhere}
...
while ($row = $db->Fetch($stmt))
{
    if ($tempCostCode != $row["cost_code"])
    {
        if ($sumCost_cost> 0)
        {
            $str .= "<tr bgcolor='#f0f4fa'>"
                ."<td align='right' colspan='6'>รวมหน่วยงาน : {$tempCostCode}: {$tempCostName}</td>"
    ...
    $countItems++;
    $d_date_chg = ($row["d_date_chg"] != "")? $date->shot_date_from_db($row["d_date_chg"]) : '';
    ...
    $str .= "<tr>"
                ...
                ."<td align='center' nowrap>&nbsp;{$d_date_chg}</td>"
                ...
                ."<td align='left'>{$row["new_cost_name"]}</td>"
...
$str = '<table cellspacing="0" cellpadding="0" width="100%" border="0" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
                        ...
                        <tr><th colspan="11"><b>หน่วยงานที่โอน :</b> '.$costName.'</th></tr>
		...
		<table cellspacing="0" cellpadding="3" width="100%" border="1" style="border-collapse:collapse;border:none;mso-border-alt:solid windowtext .1pt;mso-padding-alt:0cm 1.0pt 0cm 1.0pt">
                    <tr bgcolor="#A5BAD6">
                        ...
                        <th align="center" nowrap><b>เลขที่โอนย้าย</b></th>
                        <th align="center" nowrap><b>เลขที่เอกสาร</b></th>
                        <th align="center" nowrap><b>วันที่โอนย้าย</b></th>
                        <th align="center" nowrap><b>หมายเลขครุภัณฑ์</b></th>
                        <th align="center" nowrap><b>รายการครุภัณฑ์</b></th>
                        ...
                        <th align="center" nowrap><b>ผู้รับโอน</b></th>
