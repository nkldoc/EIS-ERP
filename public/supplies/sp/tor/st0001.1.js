...
Ext.AppUx = function (app, menu) {
    ...
    process: function (menuCode, record) {
      Ext.Ajax.request({
        ...
        params: {
          ...
          i_in_advance: record.get("i_type_bg") == 12 ? 1 : 0,
          id: record.get("id"),
  ...
  function controller(rec, status) {
    if (status === "processUpdate") {
      ...
      if (rec.get("i_is_register") != 1) {
        Ext.Msg.alert("แจ้งเตือน", "<span style='white-space: nowrap;'>กรุณาบันทึกรายการก่อนผ่านรายกการ</span><br>", function (bu, action) {
          return false;
        });
        return;
      }
                          ...
                          listeners: {
                            afterrender: function () {
                              this.fn = function () {
                                value = this.getValue();
                                console.log(value);
                                if (value == 4 || value == 2 || value == 5 || value == 8 || value == 11 || value == 12) {
                                  // Ext.getCmp("button1").hide();
                                  Ext.getCmp("buttongroup1").hide();
                                  Ext.getCmp("buttongroup2").hide();
                                  Ext.getCmp("buttongroup3").hide();
  ...
  var MenuButton = function () {
    ...
    tb.add({
      ...
      bodyStyle: "padding:0px 0px 0px 0px !important;",
      handler: function () {
        var link = "tor/api/List_TorStep.php?mode=EXPORT_EXCEL";
        var params = Ext.storeDtl.baseParams;
    ...
    (gridMain = function () {
      var colmnn = [
        ...
        },
        {
          ...
          width: 220,
          renderer: function (value, metaData, record, row, col, store, gridView) {
              var cc = record.get('tor_delete_comment');
              var ccTxt =(cc!==null)?"<span style='font-weight:bold;color:red'>("+cc+")</span>":"";
            return value+ccTxt;
          },
        },
        {
