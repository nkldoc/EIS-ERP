...
Ext.onReady(function() {
	...
	/* =============================================== */
	if (Ext.I_MENU_CANCEL=="1")
		title_panel		= "ยกเลิกใบเบิก-GL  (e-PHIS)";
	else if (Ext.I_MENU_CANCEL=="2")
		title_panel		= "ยกเลิกใบเบิก-GL  (Vision Net)";
	/* =============================================== */
	...
	Ext.store = new Ext.data.JsonStore({
	    ...
	    autoDestroy: true,
		autoLoad: false, 
	    url: "api/List_CancelRQE.php",
	    baseParams: { type: "imp_expense_approve", i_read: user_right_read ,i_type_menu : Ext.I_MENU_CANCEL}, // Permission i_read
	    root: "data",
	    idProperty: "id",
		...
		new Ext.Window({
			...
			items:[{ // column 1
	            ...
	            items: [new Ext.FormPanel({
	                ...
	                items: [{
	                    xtype: "fieldset",
	                    title: "ยกเลิกรายการตั้งหนี้ "+((arr.data.table_name == "imp_request_ephis_hdr")? "ใบเบิก e-PHIS" : "ใบเบิก Vision Net"),
	                    defaults: { xtype: "displayfield", width: "100%", readOnly: true },
	                    items :[
	                            { fieldLabel: "<b>เลขที่ใบเบิก</b>", style: "color:red; font-weight: bold;", value: arr.data.c_request }
								,{ fieldLabel: "<b>ชื่อผู้รับเงิน</b>", value : arr.data.c_creditor }
								,{ fieldLabel: "<b>รหัสบัญชีตั้งหนี้</b>", value : arr.data.c_code_g }
		                        ,{ fieldLabel: "<b>ข้อมูลนำเข้า</b>", value : ((arr.data.table_name == "imp_request_ephis_hdr")? "ใบเบิก e-PHIS" : "ใบเบิก Vision Net") }
		                    	,{ fieldLabel: "<b>วันที่ตั้งหนี้</b>", value : shortThaiDate(arr.data.d_doc_date) }
		                    	,{ fieldLabel: "<b>ชื่อรายการ</b>", value : arr.data.c_acc_item }
		                    	,{ fieldLabel: "<b>จำนวนขอเบิกทั้งสิ้น</b>", value : floatRenderer(arr.data.f_inv) }
		                    	,{
									xtype: "compositefield"
									,fieldLabel: "<b>วันที่บันทึกบัญชียกเลิกใบเบิก</b>"
									,anchor: "100%"
									,msgTarget: "under"
									,readOnly: false
									,items: [
									        { id: "d_save_jv_cancel", fieldLabel: "วันที่บันทึกบัญชียกเลิกใบเบิก", xtype: "datefield", width: 128, readOnly: false }
	                ...
	                buttons: [{
	                	...
	                	handler : function() {
	                		...
	                		var check	= false;
			   				...
			   				if(Ext.getCmp("d_save_jv_cancel").getValue() == "") { msg	+= "กรุณากรอก วันที่บันทึกบัญชียกเลิกใบเบิก<br>"; }
	                    	...
	                    	} else { Ext.Msg.minWidth = 300;	Ext.Msg.alert("แจ้งเตือน", msg); }
	                    }
	                },{
	                    text: Ext.GLOBAL_BU_BACK_TH,
	...
	cellClick	= function( grid, rowIndex, columnIndex, e ) {
		...
		if (columnIndex == grid.getColumnModel().getIndexById("cancel")) {
			...
			if(record.data.gl_tran_hdr_rq_id > 0) { Preview(record.data.gl_tran_hdr_rq_id); }
		}
	}; // cellClick
	...
	gridMain = new Ext.grid.GridPanel({
		...
		{ id: "cancel", header: "-", sortable: false, align: "center", width:120, dataIndex: "id",
			renderer: function(value, metaData, record, row, col, store, gridView) {
				if(record.data.i_status == 9) { 
					...
					return "<button style='font-size:11px; cursor:pointer; color: red;'>ยกเลิกใบเบิก (GL)</button>";
				}
			}
		},
		{ header: "ยกเลิกบันทึกบัญชีตั้งหนี้(ใบเบิก)", id:"printBank", sortable: true, width:200, dataIndex: "c_code_cancel",
		...
		{ header: "บันทึกบัญชีตั้งหนี้", id:"print", sortable: true, width:200, dataIndex: "c_code_bank_cancel",
			renderer: function(value, metaData, record, rowIndex, colIndex, store) {
				metaData.attr = "align='center'";
        ...
        { header: "ข้อมูลนำเข้า", sortable: true, align: "center", dataIndex: "table_name",
	    	renderer: function(value, metaData, record, rowIndex, colIndex, store) {
				if (value == "imp_request_ephis_hdr") {
					return "ใบเบิก e-PHIS";
				  } else if (value == "imp_request_vsn_hdr") {
					return "ใบเบิก Vision Net";
				  }
	    	}
		},
		{ header: "เลขที่นำเข้าใบเบิก", sortable: true, width: 120, align: "center", dataIndex: "c_code_request" }, 
		{ header: "เลขที่ใบเบิก", sortable: true, width: 110, align: "center", dataIndex: "c_request" },
		{ header: "วันที่ตั้งหนี้", sortable: true, align: "center", dataIndex: "d_doc_date",
			renderer: function(value, metaData, record, rowIndex, colIndex, store) {
				return (value != "")? shortThaiDate(value) : "";
			}
		},
		{ id: "c_acc_item", header: "ชื่อรายการ", sortable: true, width: 300, dataIndex: "c_acc_item" },
		{ header: "จำนวนขอเบิกทั้งสิ้น", sortable: true, dataIndex: "f_inv",
			renderer: function(value, metaData, record, rowIndex, colIndex, store) {
				metaData.attr = "style= 'cursor:pointer; text-align:right; color: blue;';";
				return floatRenderer(value);
			}
		} 
		,{ id: "i_status", header: "สถานะใบเบิก", sortable: false, align: "center", width:120, dataIndex: "i_status",
			renderer: function(value, metaData, record, row, col, store, gridView) {
				if (value == 3) {
					return "<font color=#CC3300>บันทึกบัญชีสมบูรณ์</font>";
				} else {
					return "<font color=red>ยกเลิกใบเบิก</font>";
				}
			}
		}		
