genTextButton1 = function (val) {
  ...
  if (val == 1) {
    ...
    text = "<text style='font-size:11px; cursor:pointer; color: red; width: 100%;'disabled>คำนวณค่าเสื่อมประจำเดือนแล้ว</text>";
  } else if (val == 2) {
    text = "-";
  } else {
    text = "<text style='font-size:11px; cursor:pointer; color: green; width: 100%;'disabled>ยังไม่ได้คำนวณค่าเสื่อม</text>";
  }
  return text;
};
...
genTextButton2 = function (val) {
  ...
  if (val == 1) {
    ...
    text = "<text style='font-size:11px; cursor:pointer; color: red; width: 100%; 'disabled>คำนวณสินทรัพย์บริจาคแล้ว</text>";
  } else if (val == 2) {
    text = "-";
  } else {
  ...
  const gridMain = new Ext.grid.GridPanel({
    ...
    columns: [
      ...
      },
      {
        ...
        sortable: true,
        width: 160,
        dataIndex: "i_am_send_donate",
        renderer: function (value, metaData, record, rowIndex, colIndex, store) {
          metaData.attr = 'style="text-align:center;"';
          return genTextButton2(value);
        },
      },
      { width: 120, dataIndex: "" },
    ],
