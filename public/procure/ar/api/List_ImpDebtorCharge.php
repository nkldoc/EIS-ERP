...
if ($_REQUEST["type"] == "imp_debtor_charge_hdr") {
    ...
    $sqlMain = "
        ...
        FROM #TemData a
            INNER JOIN dbo.imp_debtor_charge_hdr b ON a.imp_debtor_charge_hdr_id = b.imp_debtor_charge_hdr_id
    ...
    if (sqlsrv_has_rows($stmt)) {
        while ($row = $db->Fetch($stmt)) {
            ...
            $temp = array(
                "no"                                => $row["numrow"],
                "id"                                => $row["imp_debtor_charge_hdr_id"],
                "c_code"                            => $row["c_code"],
    ...
    exit;
} else if ($_REQUEST["type"] == "imp_debtor_charge_dtl") {
    ...
    if ($mode == "excel") { } else {
        ...
        FROM dbo.imp_debtor_charge_dtl a
        WHERE a.imp_debtor_charge_hdr_id = ?
        ORDER BY a.imp_debtor_charge_dtl_id;";
    ...
    if (count($arr) > 0) {
        foreach ($arr as $row) {
            $temp = array(
                "no"                                    => ++$no,
                "id"                                    => $row["imp_debtor_charge_dtl_id"],
                "hdr_id"                                => $row["imp_debtor_charge_hdr_id"],
