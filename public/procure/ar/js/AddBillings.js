	...
	*/
	...
	/* store
		...
		*
		*/
	Ext.storeOrders = new Ext.data.JsonStore({
...
//
	...
	*/
	...
	function controllTab(record,butt){ 
		...
		if(butt=='add'){  
			...
			var frmSoHdr = new formSoHdr();  
								...
								record.set('ar_so_hdr_id',record.get('id'));
								record.set('so_code',record.get('c_code'));
	...
	function clickRemoveProductBl(grid, rowIndex, columnIndex, e) { 
		var record = grid.getStore().getAt(rowIndex);   
		...
		var win = new Ext.Window({
				...
				width : 250,
				height : 130,
				html: "ท่านต้องการที่จะลบข้อมูล ? <p style='color:red;font-weight:bold;'>* กรุณากดบันทึกรายการ อีกครั้งเพื่อบันทึกภาษี</p>",
				buttons : [
					{
						text : "Confirm",
						handler : function() {
							Ext.Ajax.request({
								url : 'api/mnSoBillingDtl.php' , 
								params : { 
									mode : 'DELETE', 
									id : record.get('id'),
								}, 
								...
								success: function ( result, request ) { 
									...
									Ext.getCmp('tabBlDtlGrid').getStore().reload();		// reload grid & store 
	...
	// Field hdr
	function selectProductBilling(record){
		
		//console.log(record); //'BL'
			Ext.storeDtl2 = new Ext.data.JsonStore({
			storeId: 'myStoreDtl2', 
			url : 'api/ListAddSoBillingDtl.php', 
			root: 'data', 
			idProperty: 'id',
			totalProperty: 'totalCount', 	
			fields: [
 			...
 			var gridDtl = {  
				...
				autoHeight: true,
				store:Ext.storeDtl2, 
				//tbar:[ { xtype:'hidden', name:'dc_product_type_id',value:record=="BL"?Ext.getCmp('dc_product_type_id2ID').getValue():Ext.getCmp('dc_product_type_idID').getValue(), }],  
				...
				columns:[
						...
						{ align:'center', width:65, header: "เลือกวางบิล", dataIndex: 'soDtlBIlling' },  
						{ header: "รายการสินค้า", dataIndex: 'c_name',id:'CproductID' },
						{ header: "จำนวน/ครั้ง", align:'right', dataIndex: 'f_quan', width:80},
 						{ header: "ราคา/หน่วย", align:'right', dataIndex: 'f_unit_cost', width:80, },
						{ header: "จำนวนเงินรวม<br/>ก่อนหัก ณ ที่จ่าย", align:'right', dataIndex: 'f_total_cost', width:150, },  
						{ header: "หัก ณ ที่จ่าย", align:'right', dataIndex: 'f_tax_amt', width:100, }, 
 						{ header: "จำนวนเงินสุทธิ", align:'right', dataIndex: 'f_net_cost' , width:130},
						{ header: "หมายเหตุ",  dataIndex: 'c_comment',  width:130},   
						], 
 	...
 	var itemsSoDtl = [{
							...
							xtype:'hidden',name:'ar_bill_invoice_hdr_id',value:Ext.getCmp('hdrID').getValue(),
						} ,gridDtl,
	...
	var winFrm = new Ext.Window({
		...
		listeners: {
					afterrender: function( obj, eOpts )
					{
						...
						if(record=="BL"){
							Ext.storeDtl2.setBaseParam("mode",'GETDATA');
							Ext.storeDtl2.setBaseParam("id",Ext.getCmp('soHdrID').getValue());//GETDATA
							Ext.storeDtl2.load(); 
							
							Ext.storeBlDtl.setBaseParam("mode",'GETDATA');
							Ext.storeBlDtl.setBaseParam("id",Ext.getCmp('hdrID').getValue());//GETDATA
							Ext.storeBlDtl.load();	 
						} 
					},
					 "maximize": function(window, opts) { //when property minimizable 
	...
	function windowOrders(record,butt){
		...
		var gridOrders = { 
				...
				{ header: "ลูกค้า", width:210, sortable: true, dataIndex: 'c_debtor_name' }, 
				{ header: "หน่วยงาน", sortable: true, dataIndex: 'c_cost_name' ,width:120, },
				{
					header: "วันที่แจ้ง Order",  
					sortable:false,
					align: 'center', 
		...
		var winFrm = new Ext.Window({
			id : "win-pop-lov"+id,
			title : "เลือกรายการ Order เพื่อวางบิล/แจ้งหนี้",
			modal: true, 
			plain: true,  
			frame: true,
			layout: "fit", 
	...
	var itemsSoHdr = [{
			...
			xtype:'textfield', fieldLabel:'เลขที่ Order'  ,name:'so_code', value:'0',readOnly:true,
		} 
		,
			Ext.PopCntForm.mini 
		...
		
	{
		...
		xtype:'displayfield', fieldLabel:'หน่วยธุรกิจ ' ,name:'c_area_name',
			...
			defaults:{ width:'78%',}, 
			items: [
			{xtype:'textfield', fieldLabel:'ชื่อลูกค้าที่วางบิล/แจ้งหนี้ ' ,name:'c_name_inv',},
			{xtype:'textfield', fieldLabel:'ที่อยู่ ' ,name:'c_address_inv',},
			{xtype:'textfield', fieldLabel:'กำหนดการวางบิล  ' ,name:'due_bill',},
			{xtype:'textfield', fieldLabel:'เงื่อนไขการชำระเงิน ' ,name:'condition_pay',},
			]
	},{ 
		xtype: 'datefield', 
	...
	var filters = {
						...
						store: new Ext.data.SimpleStore({
							  fields: ["id", "c_name"],  
							  data:[['debtor_name', "ชื่อลูกค้า"],['c_code', "รหัส Order"]],
						}),
						value: 'debtor_name',
						valueField: 'id',
						displayField: 'c_name',
	...
	searchGridOrders.superclass.constructor.call(this, { 
		...
		buttons: [ {
				...
				handler: function() { 
					...
					Ext.storeOrders.setBaseParam("mode", "SEARCH");
					Ext.storeOrders.setBaseParam("filter",Ext.getCmp("filterOrders-ID").getValue()); 
					Ext.storeOrders.setBaseParam("value", Ext.getCmp("valOrders-ID").getValue());  
					Ext.storeOrders.setBaseParam("d_begin_dateID", Ext.getCmp("startDateOrdersID").getValue());  
					Ext.storeOrders.setBaseParam("d_end_dateID", Ext.getCmp("endDateOrdersID").getValue());  
		...
		formBlDtlEdit.superclass.constructor.call(this, {   
			...
			items:[{
					...
					{ 
						...
						id:'buAddProID',
						handler: function() { selectProductBilling("BL"); }
					} ,
					gridDtl2,//grid
					{ 
						html: "<p>&nbsp;</p>",
	...
	*/
	...
	function printForm(record){
		...
		var storeDtl =new Ext.data.JsonStore({
			...
			url : 'api/ListBillingPrintDtl.php', 
			root: 'data', 
			idProperty: 'id',
...
//Close
	...
	Ext.onReady(function(){
		...
		var gridMain = {
			region: 'center',
			title: 'แสดงข้อมูลรายการใบวางบิล',
			xtype: 'grid',
			id:'tabpanel1',
			border: false,
			stripeRows: true,
