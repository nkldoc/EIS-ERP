...
if($_REQUEST["type"] == "gl_rep_acc_dtl") {
	...
	$sqlTempTable = "SELECT
						ROW_NUMBER() OVER (ORDER BY a.c_name, b.i_sequence) AS numrow
						,b.gl_rep_acc_dtl_id
						,b.gl_rep_acc_hdr_id
						,b.c_name
						,b.i_sequence
	...
	$dtl_id			= ($_REQUEST["gl_rep_acc_dtl_id"] > 0)? $_REQUEST["gl_rep_acc_dtl_id"] : 0;
	...
	$sqlTempTable = "SELECT
					...
					FROM vw_dc_acc a
						LEFT JOIN (	SELECT bb.dc_acc_id FROM gl_rep_acc_dtl aa
							INNER JOIN gl_rep_acc_map bb ON aa.gl_rep_acc_dtl_id=bb.gl_rep_acc_dtl_id
						WHERE aa.gl_rep_acc_dtl_id = ?) c ON a.dc_acc_id=c.dc_acc_id
