...
switch ($mode) {
	...
	case "EDIT":
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				SET NOCOUNT ON
				INSERT INTO gl_map_acc_hdr (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE gl_map_acc_hdr SET " . substr($addField, 1) . " WHERE gl_map_acc_hdr_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
	...
	case "SAVE_DTL":
		...
		if ($msg == "") {
			foreach ($Arr as $fldd) {
				...
				$data["c_code_map"]						= $fldd["c_code_map"];
				$data["c_name_map"]						= $fldd["c_name_map"];
				$data["c_comment"]						= $fldd["c_comment"];
				$data["i_system"]						= $fldd["i_system"]; // (1 = V-Net, 2 = E-Phis)
				$data["c_system"]						= ($data["i_system"]=="1") ? "Vision Net" : "E-Phis"; // (1 = V-Net, 2 = E-Phis)
				$data["dc_user_update_id"]				= $_SESSION["user_id"];
				$data["dc_user_update_cost_id"]			= $_SESSION["dc_cost_id"];
				$data["d_update"]						= date("Y-m-d H:i:s");
 

				if ($fldd["gl_map_acc_dtl_id"] > 0) { // EDIT
					...
					$arrValue[] = $fldd["gl_map_acc_dtl_id"];
					$sql		= "UPDATE gl_map_acc_dtl SET " . substr($addField, 1) . " WHERE gl_map_acc_dtl_id = ?";
					$db->QueryParam($sql, $arrValue);
				} else { // ADD

					$data["gl_map_acc_hdr_id"]								= $_REQUEST["id"];
					$data["dc_user_create_id"]								= $_SESSION["user_id"];
					$data["dc_user_create_cost_id"]							= $_SESSION["dc_cost_id"];
					$data["d_create"]										= date("Y-m-d H:i:s");
					...
					$sql = "
						SET NOCOUNT ON
						INSERT INTO gl_map_acc_dtl (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
						SELECT @@IDENTITY as id;";
