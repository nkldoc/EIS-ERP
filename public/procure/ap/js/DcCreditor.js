Ext.onReady(function() {
	...
	var gridMain = new Ext.grid.GridPanel({
		...
		columns: [
			...
			{ header: "รหัสลูกค้า/เจ้าหนี้", sortable: true, align:"center", width: 70, dataIndex: "c_code" },
			{ id: "c_name", header: "ชื่อลูกค้า/เจ้าหนี้", sortable: true, dataIndex: "c_name" },
	...
	function win_method (mode, dc_creditor_id) {
...
//									dc_acc_id: Ext.getCmp("bank_dc_acc_id").getValue(),
									c_comment: Ext.getCmp("bank_c_comment").getValue(),
								...
								success: function ( result, request ) {
									var jsonData = Ext.util.JSON.decode(result.responseText);	//decode json
									if (jsonData.success) {
										Ext.MessageBox.alert("แจ้งเตือน", "บันทึกเรียบร้อย");		// alert massage success
										store_account.load();
...
//	                			}
	    	                		...
	    	                		Ext.Ajax.request({
	    								...
	    								success: function ( result, request ) {
	    									...
	    									if (obj.success) { // ใช้งานได้
	    										Ext.getCmp("chk_ref_value").setValue(true);
	    										Ext.Msg.alert("Warning", "เลขบัตรประชาชนใช้งานได้");
	    									} else { // ซ้ำ
	    										Ext.getCmp("chk_ref_value").setValue(false);
	    	                		...
	    	                		if(Ext.getCmp("c_tax_value").getValue() == "") {
		    	                		...
		    	                		Ext.Ajax.request({
		    								...
		    								success: function ( result, request ) {
		    									...
		    									if (obj.success) { // ใช้งานได้
		    										Ext.getCmp("chk_tax_value").setValue(true);
		    										Ext.Msg.alert("Warning", "เลขประจำตัวผู้เสียภาษีอากรใช้งานได้");
		    									} else { // ซ้ำ
		    										Ext.getCmp("chk_tax_value").setValue(false);
		    										$.each(obj.data, function( index, value ) {
			    										sss	+= value+"<br>";
		    			...
		    			mode: "local",
		    			triggerAction: "all",
		    			emptyText: "- กรุณาเลือกบัญชีเจ้าหนี้ -",
						forceSelection: true,
						selectOnFocus: true
					}, {
						xtype: "checkbox",
			...
			buttons: [{
				...
				handler : function() {
					...
					if(Ext.getCmp("c_firstname").getValue() == "") { msg	+= "- กรุณากรอก ชื่อ<br>"; }
					...
					if(Ext.getCmp("dc_acc_id").getValue() == "") { msg	+= "- กรุณาเลือก บัญชีเจ้าหนี้<br>"; }
					...
					if (msg == "") {
						Ext.Ajax.request({
							...
							params: { 
								...
								c_firstname: Ext.getCmp("c_firstname").getValue(),
