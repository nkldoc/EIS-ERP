...
switch ($mode) {
	...
	case "EDIT":
		...
		if ($mode == "ADD") {
			...
			$sql	= "
				SET NOCOUNT ON
				INSERT INTO po_budget_hdr_overlap (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
			...
			$sql		= "UPDATE po_budget_hdr_overlap SET " . substr($addField, 1) . " WHERE po_budget_hdr_overlap_id = ?";
			$para		= $db->QueryParam($sql, $arrValue);
			$id			= $_REQUEST["id"];
		}
	...
	case "SAVE_DTL":
		...
		if ($msg == "") {
			foreach ($Arr as $fldd) {
				...
				$data["c_code_ref"]										= $fldd["c_code_ref"];
				$data["po_expense_id"]									= $fldd["po_expense_id"];
				$data["f_total"]										= $fldd["f_total"];
				$data["f_cancel"]										= $fldd["f_cancel"];
				$data["c_comment"]										= $fldd["c_comment"];
				$data["dc_user_update_id"]								= $_SESSION["user_id"];
				$data["dc_user_update_cost_id"]							= $_SESSION["dc_cost_id"];
				$data["d_update"]										= date("Y-m-d H:i:s");

				if ($fldd["po_budget_dtl_overlap_id"] > 0) { // EDIT
					...
					$arrValue[] = $fldd["po_budget_dtl_overlap_id"];
					$sql		= "UPDATE po_budget_dtl_overlap SET " . substr($addField, 1) . " WHERE po_budget_dtl_overlap_id = ?";
					$db->QueryParam($sql, $arrValue);
				} else { // ADD

					$data["po_budget_hdr_overlap_id"]						= $_REQUEST["id"];
					$data["dc_user_create_id"]								= $_SESSION["user_id"];
					$data["dc_user_create_cost_id"]							= $_SESSION["dc_cost_id"];
					$data["d_create"]										= date("Y-m-d H:i:s");
					...
					$sql = "
						SET NOCOUNT ON
						INSERT INTO po_budget_dtl_overlap (" . substr($addField, 1) . ") VALUES (" . substr($addValue, 1) . ");
						SELECT @@IDENTITY as id;";
