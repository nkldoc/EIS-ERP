...
$sqlTempTable = "select dc_building_id
                    ,c_code
                    ,c_name
                    ,c_addr
                    ,i_type_region
...
if($mode=="SEARCH"){
	...
	if ($_REQUEST["c_name"] != "")
	{
            ...
            $arrParam[] = "%".$_REQUEST["c_name"]."%";
	}
	
	if ($_REQUEST["i_type_region"] > 0)
	{
            $sqlTempTable .= " and i_type_region = ?";
            $arrParam[] = $_REQUEST["i_type_region"];
	}
	
	if ($_REQUEST["i_enable"] > 0)
	{
            $sqlTempTable .= " and i_enable = ?";
...
while($row =$db->Fetch($stmt))				
{
    $temp = array("no" => ($i++), 
                    ...
                    "c_name" => $row["c_name"],
                    "c_addr" => $row["c_addr"],
                    "i_type_region" => $row["i_type_region"],
