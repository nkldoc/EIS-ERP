...
function po_tracking($i_status)
{
	...
	if ($i_status == 2) {
		...
		CONVERT(VARCHAR,
			CASE
				WHEN c.d_receive_date3 IS NOT NULL THEN c.d_receive_date3
				ELSE c.d_inv_date
			END
		, 120) AS d_doc_date
		,dbo.fn_minus_to_0(DATEDIFF(DAY, CASE WHEN c.d_receive_date3 IS NOT NULL THEN c.d_receive_date3 ELSE c.d_inv_date END, GETDATE())
			- (SELECT COUNT(DISTINCT bb.d_holiday) FROM dbo.po_holiday_hdr aa INNER JOIN dbo.po_holiday_dtl bb ON aa.po_holiday_hdr_id = bb.po_holiday_hdr_id WHERE aa.i_enable = 1 AND bb.d_holiday BETWEEN DATEADD(DAY, 1, CASE WHEN c.d_receive_date3 IS NOT NULL THEN c.d_receive_date3 ELSE c.d_inv_date END) AND GETDATE())
		) AS count_date";
	} else if ($i_status == 3) {
		$fld = "
		CONVERT(VARCHAR, c.d_doc_date3, 120) AS d_doc_date
		,dbo.fn_minus_to_0(DATEDIFF(DAY, c.d_doc_date3, GETDATE())
			- (SELECT COUNT(DISTINCT bb.d_holiday) FROM dbo.po_holiday_hdr aa INNER JOIN dbo.po_holiday_dtl bb ON aa.po_holiday_hdr_id = bb.po_holiday_hdr_id WHERE aa.i_enable = 1 AND bb.d_holiday BETWEEN DATEADD(DAY, 1, c.d_doc_date3) AND GETDATE())
		) AS count_date";
	} else if ($i_status == 4) {
		$fld = "
		CONVERT(VARCHAR, c.d_approve_date, 120) AS d_doc_date
		,dbo.fn_minus_to_0(DATEDIFF(DAY, c.d_approve_date, GETDATE())
			- (SELECT COUNT(DISTINCT bb.d_holiday) FROM dbo.po_holiday_hdr aa INNER JOIN dbo.po_holiday_dtl bb ON aa.po_holiday_hdr_id = bb.po_holiday_hdr_id WHERE aa.i_enable = 1 AND bb.d_holiday BETWEEN DATEADD(DAY, 1, c.d_approve_date) AND GETDATE())
		) AS count_date";
	} else if ($i_status == 7) {
		$fld = "
		CONVERT(VARCHAR, c.d_doc_date7, 120) AS d_doc_date
		,dbo.fn_minus_to_0(DATEDIFF(DAY, c.d_doc_date7, GETDATE())
			- (SELECT COUNT(DISTINCT bb.d_holiday) FROM dbo.po_holiday_hdr aa INNER JOIN dbo.po_holiday_dtl bb ON aa.po_holiday_hdr_id = bb.po_holiday_hdr_id WHERE aa.i_enable = 1 AND bb.d_holiday BETWEEN DATEADD(DAY, 1, c.d_doc_date7) AND GETDATE())
		) AS count_date";
	} else if ($i_status == 10) {
		$fld = "
		CONVERT(VARCHAR, c.d_doc_date10, 120) AS d_doc_date
		,dbo.fn_minus_to_0(DATEDIFF(DAY, c.d_doc_date10, GETDATE())
			- (SELECT COUNT(DISTINCT bb.d_holiday) FROM dbo.po_holiday_hdr aa INNER JOIN dbo.po_holiday_dtl bb ON aa.po_holiday_hdr_id = bb.po_holiday_hdr_id WHERE aa.i_enable = 1 AND bb.d_holiday BETWEEN DATEADD(DAY, 1, c.d_doc_date10) AND GETDATE())
		) AS count_date";
	}
	$sqlMain = "
		SET NOCOUNT ON;
		SELECT
			a.c_code_ref
			,ISNULL(b.c_approve,'') AS c_approve
			,{$fld}
		INTO #tempData
		FROM po_working_hdr a
			INNER JOIN po_working_dtl b ON a.po_working_hdr_id = b.po_working_hdr_id
			INNER JOIN vw_po_working_track_status c ON b.po_working_hdr_id = c.po_working_hdr_id
		WHERE a.i_status_last = {$i_status}
			AND a.i_enable = 1;
		
		SELECT * FROM #tempData ORDER BY count_date DESC;
		DROP TABLE #tempData;";
...
echo json_encode(array(
	"debug"				=> true,
	"B1"				=> po_tracking(2), // รออนุมัติฏีกา
	"B2"				=> po_tracking(3), // รอทักท้วง
	"B3"				=> po_tracking(4), // รอหักงบประมาณ
	"B4"				=> po_tracking(7), // รอจัดทำเช็ค
	"B5"				=> po_tracking(10), // รอทำทะเบียนจ่าย
));
