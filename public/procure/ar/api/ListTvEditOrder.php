...
$sta = array(0=>"แก้ไข",
 ...
 if(isset($_REQUEST['mode']) && $_REQUEST['mode']=="SEARCH")
 {
		...
		$wh .=" isnull({$table}.i_no_order,0)!=1 AND {$table}.i_class_type=".AR_CLASS_TYPE_TV." and b.d_doc_date between ? and ?";
		...
		$wh = " isnull({$table}.i_no_order,0)!=1 AND {$table}.i_class_type=".AR_CLASS_TYPE_TV." and ISNULL({$table}.i_enable,".STATUS_DISABLE.") != ?";
	...
	$sqlTempTable = "select {$table}.{$table}_id 
                            ...
                            ,{$table}.pj_hdr_id
							,(select top 1 c_code+' '+c_name from ar_package  where ar_package_id={$table}.ar_package_id) as ar_package_name
							,isnull(ar_package_id,0) as ar_package_id
