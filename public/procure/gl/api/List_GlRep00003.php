...
function List_QueryParam() {
	...
	if (!get($limit)) { $limit 	= 50; }else{ $limit=($limit+$start); }
	...
	//--------------------------------------------------------------------------------------//
 	...
 	$sqlMain = "DECLARE @start int = {$start};
				...
				DECLARE @Dc_acc TABLE( 
						...
						c_sub VARCHAR(3) NULL,
						i_level tinyint NULL,
						dc_acc_lv1_id INT NULL, 
						c_code_lv1 VARCHAR(250) NULL, 
						c_name_lv1 VARCHAR(250) NULL 
					);
				...
				/*dc_acc*/
				INSERT INTO @Dc_acc  
				SELECT DISTINCT a.dc_acc_id,a.c_code,a.c_name,a.i_group,left(a.c_code,3) as c_sub,a.i_level
					,b.dc_acc_lv1_id,b.c_code_lv1,b.c_name_lv1
				FROM dc_acc a INNER JOIN vw_dc_acc_with_parent b ON a.dc_acc_id=b.dc_acc_lv2_id
					WHERE a.i_group IN (4,5) AND a.i_level IN(2) AND a.i_enable = 1 AND a.i_delete = 2; 
				...
				, CASE WHEN c_mm BETWEEN '10' AND '12' THEN
						CASE WHEN b.i_group = ".GL_ACC_GROUP4_REVENUE." THEN SUM(f_cr-f_dr) ELSE SUM(f_dr-f_cr) END
				...
				SELECT
					...
					NULL,
					NULL, 
					aa.dc_acc_id as dc_acc_lv2_id,
					aa.c_code  as c_code_lv2,
					aa.c_name as c_name_lv2,
					aa.dc_acc_lv1_id,
					aa.c_code_lv1,
					aa.c_name_lv1, 
					ISNULL((SELECT SUM(bb.f_money1) FROM @Gl_data bb WHERE bb.dc_acc_lv2_id=aa.dc_acc_id GROUP BY bb.dc_acc_lv2_id),0) AS f_money1,
					ISNULL((SELECT SUM(bb.f_money2) FROM @Gl_data bb WHERE bb.dc_acc_lv2_id=aa.dc_acc_id GROUP BY bb.dc_acc_lv2_id),0) AS f_money2,
					ISNULL((SELECT SUM(bb.f_money3) FROM @Gl_data bb WHERE bb.dc_acc_lv2_id=aa.dc_acc_id GROUP BY bb.dc_acc_lv2_id),0) AS f_money3,
					ISNULL((SELECT SUM(bb.f_money4) FROM @Gl_data bb WHERE bb.dc_acc_lv2_id=aa.dc_acc_id GROUP BY bb.dc_acc_lv2_id),0) AS f_money4				
				FROM @Dc_acc aa 
				WHERE aa.i_level=2
				GROUP BY  aa.i_level,aa.dc_acc_id,aa.c_code,aa.c_name, aa.dc_acc_lv1_id, aa.c_code_lv1, aa.c_name_lv1;				
				...
				*/
				...
				SELECT
					...
					NULL,
					'9999999999',
					'รายได้สูง(ต่ำ)กว่าค่าใช้จ่าย',
					SUM(case when left(c_code_lv1,1) = '4' then f_money1 else f_money1 * -1 end),
					SUM(case when left(c_code_lv1,1) = '4' then f_money2 else f_money2 * -1 end),
					SUM(case when left(c_code_lv1,1) = '4' then f_money3 else f_money3 * -1 end),
					SUM(case when left(c_code_lv1,1) = '4' then f_money4 else f_money4 * -1 end)
				FROM @Gl_show
				WHERE i_type = 4;
