...
{
		$orderType = array('-1'=>"", 'i_center_sale'=>" and isnull(i_center_sale,0)=1 ", 'i_is_cluster'=>" and isnull(i_is_cluster,0)=1 ", 'i_is_imc'=>" and isnull(i_is_imc,0)=1 ", 'i_is_barter'=>" and isnull(i_is_barter,)=1"); 
		...
		$wh .=" i_type_region=".AR_PROCUT_TYPE_REGION." AND isnull(i_no_order,0)!=1 AND i_class_type=".AR_CLASS_TYPE_RADIO.$orderType[$_REQUEST["orderType"]]." and d_doc_date between ? and ?";
        ...
        $wh = " i_type_region=".AR_PROCUT_TYPE_REGION." AND isnull(i_no_order,0)!=1 AND i_class_type=".AR_CLASS_TYPE_RADIO." and ISNULL(i_enable,".STATUS_DISABLE.") != ?";
	...
	$sqlTempTable = "select {$table}_id 
                            ...
                            ,isnull(i_is_cluster,0) as i_is_cluster 
                            ,isnull(i_center_sale,0) as i_center_sale
...
while($row =$db->Fetch($stmt))				
{
	...
	$temp = array("no" => ($i++), 
                    ...
                    "i_is_cluster"      => $row["i_is_cluster"],
                    "i_center_sale" 	=> $row["i_center_sale"],
